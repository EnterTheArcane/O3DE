...
def get(recipe, url, md5=None, sha1=None, sha256=None, destination=".", filename="",
    ...
    if not filename:  # deduce filename from the URL
        ...
        if "?" in url_base or "=" in url_base:
            raise RecipeException("Cannot deduce file name from the url: '{}'. Use 'filename' "
                                 "parameter.".format(url_base))
        filename = os.path.basename(url_base)

    download(recipe, url, filename, verify=verify,
             retry=retry, retry_wait=retry_wait, auth=auth, headers=headers,
             md5=md5, sha1=sha1, sha256=sha256)
    unzip(recipe, filename, destination=destination, keep_permissions=keep_permissions,
...
def download(recipe, url, filename, verify=True, retry=None, retry_wait=None,
...
def rename(recipe, src, dst):
    ...
    if os.path.exists(dst):
        raise RecipeException("rename {} to {} failed, dst exists.".format(src, dst))
    ...
    if platform.system() == "Windows" and which("robocopy") and os.path.isdir(src):
        ...
        if process.returncode > 7:  # https://ss64.com/nt/robocopy-exit.html
            raise RecipeException("rename {} to {} failed.".format(src, dst))
    else:
        try:
            os.rename(src, dst)
        except Exception as err:
            raise RecipeException("rename {} to {} failed: {}".format(src, dst, err))
...
def chmod(recipe, path: str, read: Optional[bool] = None, write: Optional[bool] = None,
    ...
    if not os.path.exists(path):
        raise RecipeException(f"Could not change permission: Path \"{path}\" does not exist.")
...
def unzip(recipe, filename, destination=".", keep_permissions=False, pattern=None,
    ...
    with FileProgress(filename, msg="Unzipping", mode="r") as file, zipfile.ZipFile(file) as z:
        ...
        if strip_root:
            ...
            if not common_folder and len(names) > 1:
                raise RecipeException("The zip file contains more than 1 folder in the root")
            if len(names) == 1 and len(names[0].split("/", 1)) == 1:
                raise RecipeException("The zip file contains a file in the root")
                ...
                if strip_root:
                    ...
                    if not common_folder:
                        ...
                        if member.isdir() or len(splits) > 1:
                            ...
                            raise RecipeException("Can't untar a tgz containing files in the root with strip_root enabled")
                    if not name.startswith(common_folder):
                        raise RecipeException("The tgz file contains more than 1 folder in the root")
...
def replace_in_file(recipe, file_path, search, replace, strict=True, encoding="utf-8"):
    ...
    """
    output = recipe.output
    content = load(recipe, file_path, encoding=encoding)
    if -1 == content.find(search):
        message = "replace_in_file didn't find pattern '%s' in '%s' file." % (search, file_path)
        if strict:
            raise RecipeException(message)
        else:
            output.warning(message)
            return False
    content = content.replace(search, replace)
    save(recipe, file_path, content, encoding=encoding)
    return True
    ...
    **folder** directory relative to the package folder. Useful to collect not inter-dependent
    ...
    for lib_folder in lib_folders:
        if not os.path.exists(lib_folder):
            recipe.output.warning("Lib folder doesn't exist, can't collect libraries: "
                                     "{0}".format(lib_folder))
            continue
