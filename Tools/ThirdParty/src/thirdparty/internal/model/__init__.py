...
class CPSComponent:
    ...
    def serialize(self):
        ...
        if self.location:
            component["location"] = self.location.replace("\\", "/")
        if self.link_location:
            component["link_location"] = self.link_location.replace("\\", "/")
    ...
    def from_cpp_info(cpp_info, conanfile, libname=None):
        ...
        if len(cpp_info.libs) > 1 and not libname:
            cps_comp.type = CPSComponentType.INTERFACE
            return cps_comp
...
class CPS:
    ...
    def serialize(self):
        ...
        if self.cps_path is not None:
            cps["cps_path"] = self.cps_path
        elif self.prefix is not None:
            cps["prefix"] = self.prefix

        for attr in ("license", "description", "website"):
            if getattr(self, attr, None):
                cps[attr] = getattr(self, attr)
        ...
        cps["components"] = {name: comp.serialize() for name, comp in self.components.items()}
    ...
    def deserialize(data):
        ...
        cps.cps_path = data.get("cps_path")
    ...
    def from_conan(dep):
        ...
        prefix = dep.package_folder.replace("\\", "/").rstrip("/")
        cps.cps_path = "@prefix@"
        ...
        if not dep.cpp_info.has_components:
            if dep.cpp_info.libs and len(dep.cpp_info.libs) > 1:
                comp = CPSComponent.from_cpp_info(dep.cpp_info, dep)
                ...
                for lib in dep.cpp_info.libs:
                    ...
                    comp.requires.insert(0, f":{base_name}")
            ...
            if cps.name not in cps.components:
                virtual = CPSComponent()
                virtual.type = CPSComponentType.INTERFACE
                virtual.requires = [f":{c}" for c in cps.default_components]
                cps.components[cps.name] = virtual

        def _relocate(path):
            if path is None:
                return None
            return path.replace("\\", "/").replace(prefix, "@prefix@")

        for comp in cps.components.values():
            comp.includes = [_relocate(p) for p in comp.includes]
            comp.location = _relocate(comp.location)
            comp.link_location = _relocate(comp.link_location)
    ...
    def to_conan(self):
        ...
        def definitions(defs):
            ...
            return list(f"{k}={v}" if v is not None else k for k, v in aggregated.items())
        ...
        for comp_name, comp in self.components.items():
            ...
            if comp.link_location:
                lib_location(comp.link_location, cpp_comp)
                location = comp.location.replace("@prefix@/", "")
                cpp_comp.bindirs = [os.path.dirname(location)]
            elif comp.location:
                lib_location(comp.location, cpp_comp)
    ...
    def load(file):
        with open(file, encoding="utf-8") as f:
            base = CPS.deserialize(json.loads(f.read()))
        ...
        for conf in "release", "debug":
            ...
            if os.path.exists(full_conf):
                with open(full_conf, encoding="utf-8") as f:
