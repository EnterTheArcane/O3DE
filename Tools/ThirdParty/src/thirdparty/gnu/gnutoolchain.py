...
class GnuToolchain:
    ...
    def __init__(self, recipe, namespace=None, prefix="/"):
        ...
        self.apple_extra_flags = apple_extra_flags(recipe)
    ...
    def _msvc_extra_flags(self):
        if is_msvc(self._recipe) and check_min_vs(self._recipe, "180", raise_invalid=False):
            return ["-FS"]
        return []

    def _add_msvc_flags(self, flags):
        # This is to avoid potential duplicate with users recipes -FS (alreday some in RecipeCenter)
