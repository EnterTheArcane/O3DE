...
def check_with_algorithm_sum(algorithm_name, file_path, provided_hash):
    ...
    if real_hash != provided_hash.lower():
        raise RecipeException("%s hash failed for '%s' file. \n"
                             " Provided hash: %s  \n"
    ...
    def rmdir(path):
        ...
        for i in range(retries):
            try:
                ...
                if i == retries - 1:
                    raise RecipeException(f"Couldn't remove folder: {path}\n{str(err)}\n"
    ...
    def renamedir(old_path, new_path):
        ...
        for i in range(retries):
            try:
                ...
                if i == retries - 1:
                    raise RecipeException(f"Couldn't move folder: {old_path}->{new_path}\n"
                                         f"{str(err)}\n"
                                         "Folder might be busy or open. "
                                         "Close any app using it and retry.")
                time.sleep(delay)
