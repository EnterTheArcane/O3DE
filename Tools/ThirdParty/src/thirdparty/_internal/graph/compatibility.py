...
from thirdparty._internal.util.files import GENERATED_FILE_COMMENT, load, save

_default_compat = """\
from thirdparty.build import supported_cppstd, supported_cstd
from thirdparty.errors import RecipeException


def cppstd_compat(recipe):
    # It will try to find packages with all the cppstd versions
    extension_properties = getattr(recipe, "extension_properties", {})
    ...
    if cppstd is not None and extension_properties.get("compatibility_cppstd") is not False:
        cppstd_possible_values = supported_cppstd(recipe)
        if cppstd_possible_values is None:
            recipe.output.warning(f'No cppstd compatibility defined for compiler "{compiler}"')
...
def migrate_compatibility_files(cache_folder):
    ...
    def _is_migratable(file_path):
        ...
        return GENERATED_FILE_COMMENT in first_line
    ...
    if _is_migratable(compatibility_file) and _is_migratable(cppstd_compat_file):
        ...
        if needs_update:
            ...
            if compatibility_exists:
                Output().success("Migration: Successfully updated compatibility.py")
...
class BinaryCompatibility:
    ...
    def __init__(self, compatibility_plugin_folder, hook_manager):
        ...
        if not os.path.exists(compatibility_file):
            raise RecipeException("The 'compatibility.py' plugin file doesn't exist. If you want "
    ...
    def compatibles(self, recipe):
        compat_infos = []
        if hasattr(recipe, "compatibility"):
            with recipe_exception_formatter(recipe, "compatibility"):
                recipe_compatibles = recipe.compatibility()
                compat_infos.extend(self._compatible_infos(recipe, recipe_compatibles))

        try:
            plugin_compatibles = self._compatibility(recipe)
        except Exception as e:
            msg = f"Error while processing 'compatibility.py' plugin for '{recipe}'"
            msg = scoped_traceback(msg, e, scope="plugins/compatibility")
            raise RecipeException(msg)
        compat_infos.extend(self._compatible_infos(recipe, plugin_compatibles))
        if not compat_infos:
            return {}

        result = {}
        original_info = recipe.info
        original_settings = recipe.settings
        original_settings_target = recipe.settings_target
        original_options = recipe.options
        for c in compat_infos:
            # we replace the recipe, so ``validate()`` and ``package_id()`` can
            # use the compatible ones
            recipe.info = c
            recipe.settings = c.settings
            recipe.settings_target = c.settings_target
            recipe.options = c.options
            run_validate_package_id(recipe, self._hook_manager)
            pid = c.package_id()
            if pid not in result and not c.invalid:
                result[pid] = c
        # Restore the original state
        recipe.info = original_info
        recipe.settings = original_settings
        recipe.settings_target = original_settings_target
        recipe.options = original_options
        return result

    @staticmethod
    def _compatible_infos(recipe, compatibles):
        result = []
        if compatibles:
            for elem in compatibles:
                compat_info = recipe.original_info.clone()
