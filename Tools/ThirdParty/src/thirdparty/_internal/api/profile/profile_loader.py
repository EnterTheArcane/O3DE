...
from thirdparty._internal.model.conf import ConfDefinition, CORE_CONF_PATTERN
...
def _check_correct_cppstd(settings):
    ...
    if cppstd and version:
        ...
        from thirdparty.build.cppstd import supported_cppstd
        ...
        if supported is not None and cppstd not in supported:
            from thirdparty.errors import RecipeException
            raise RecipeException(f"The provided compiler.cppstd={cppstd} is not supported by {compiler} {version}. "
...
def _check_correct_cstd(settings):
    ...
    if cstd and version:
        compiler = settings.get("compiler")
        from thirdparty.build.cstd import supported_cstd
        supported = supported_cstd(None, compiler, version)
        # supported is None when we don't have information about the compiler
        # but an empty list when no flags are supported for this version
        if supported is not None and cstd not in supported:
            from thirdparty.errors import RecipeException
            raise RecipeException(f"The provided compiler.cstd={cstd} is not supported by {compiler} {version}. "
                                 f"Supported values are: {supported}")
"""
...
class ProfileLoader:
    ...
    def from_cli_args(self, profiles, settings, options, conf, cwd, context=None):
        ...
        if conf and any(CORE_CONF_PATTERN.match(c) for c in conf):
            raise RecipeException("[conf] 'core.*' configurations are not allowed in profiles.")
    ...
    def _load_profile(self, profile_name, cwd, context, root_profile_name):
        ...
        try:
            ...
            raise RecipeException(f"Cannot load profile:\n{e}")
        ...
        # Always include the root Recipe home "profiles" folder as secondary route for loading
        ...
        try:
            ...
            raise RecipeException(f"Error while rendering the profile template file '{profile_path}'. "
        ...
        try:
            ...
            raise RecipeException("Error reading '%s' profile: %s" % (profile_name, exc))
    ...
    def _recurse_load_profile(self, text, profile_path, context, root_profile_name):
        ...
        try:
            ...
            raise RecipeException("Error parsing the profile text file: %s" % str(exc))
    ...
    def get_profile_path(profiles_path, profile_name, cwd, exists=True):
        def valid_path(_profile_path, _profile_name=None):
            if exists and not os.path.isfile(_profile_path):
                raise RecipeException("Profile not found: {}".format(_profile_name or _profile_path))
        ...
        if exists:
            if not os.path.isfile(profile_path):
                profile_path = os.path.abspath(os.path.join(cwd, profile_name))
            if not os.path.isfile(profile_path):
                raise RecipeException("Profile not found: %s" % profile_name)
        return profile_path
        ...
        for counter, line in enumerate(text.splitlines()):
            ...
            if line.startswith("["):
                ...
                if not include.endswith(")"):
                    raise RecipeException("Invalid include statement")
                include = include[:-1]
                self.includes.append(include)
            else:
                raise RecipeException("Error while parsing line %i: '%s'" % (counter, line))
...
class _ProfileValueParser:
    ...
    def get_profile(profile_text, base_profile=None):
        ...
        doc = TextINIParse(profile_text, allowed_fields=["tool_requires",
                                                         ...
                                                         "runner"])
        # Parse doc sections into Recipe model, Settings, Options, etc
        ...
        def parse_replaces(replaces):
            ...
            if errors:
                raise RecipeException("Profile [platform_requires]/[platform_tool_requires] must "
        ...
        def load_replace(doc_replace_requires):
            ...
            for r in doc_replace_requires.splitlines():
                ...
                try:
                    ...
                    raise RecipeException(f"Error in [replace_xxx] '{r}'.\nIt should be in the form"
    ...
    def _parse_settings(doc):
        def get_package_name_value(item):
            ...
            if ":" in item:
                ...
                if "[" in packagename:
                    Output().warning(f"Settings pattern {packagename} contains a version range, which has no effect. "
                                          f"Only '&' for consumer and '*' as wildcard are supported in this context.",
                                          warn_tag="risk")
        ...
        for setting in doc.settings.splitlines():
            ...
            if not setting or setting.startswith("#"):
                continue
            if "=" not in setting:
                raise RecipeException("Invalid setting line '%s'" % setting)
...
def _profile_parse_args(settings, options, conf):
    ...
    def _get_tuples_list_from_extender_arg(items):
        ...
        for item in items:
            chunks = item.split("=", 1)
            if len(chunks) != 2:
                raise RecipeException("Invalid input '%s', use 'name=value'" % item)
