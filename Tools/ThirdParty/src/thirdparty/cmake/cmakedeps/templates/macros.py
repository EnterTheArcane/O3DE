...
class MacrosTemplate(CMakeDepsFileTemplate):
    ...
    def __init__(self):
        super(MacrosTemplate, self).__init__(cmakedeps=None, require=None, recipe=None)
    ...
    def template(self):
       ...
       macro(recipe_find_apple_frameworks FRAMEWORKS_FOUND FRAMEWORKS FRAMEWORKS_DIRS)
           if(APPLE)
               foreach(_FRAMEWORK ${FRAMEWORKS})
                   # https://cmake.org/pipermail/cmake-developers/2017-August/030199.html
                   find_library(RECIPE_FRAMEWORK_${_FRAMEWORK}_FOUND NAMES ${_FRAMEWORK} PATHS ${FRAMEWORKS_DIRS} CMAKE_FIND_ROOT_PATH_BOTH)
                   if(RECIPE_FRAMEWORK_${_FRAMEWORK}_FOUND)
                       list(APPEND ${FRAMEWORKS_FOUND} ${RECIPE_FRAMEWORK_${_FRAMEWORK}_FOUND})
       ...
       function(recipe_package_library_targets libraries package_libdir package_bindir library_type
           ...
           foreach(_LIBRARY_NAME ${libraries})
               ...
               find_library(RECIPE_FOUND_LIBRARY NAMES ${_LIBRARY_NAME} PATHS ${package_libdir}
               ...
               if(RECIPE_FOUND_LIBRARY)
                   message(VERBOSE "Recipe: Library ${_LIBRARY_NAME} found ${RECIPE_FOUND_LIBRARY}")

                   # Create a micro-target for each lib/a found
                   # Allow only some characters for the target name
                   string(REGEX REPLACE "[^A-Za-z0-9.+_-]" "_" _LIBRARY_NAME ${_LIBRARY_NAME})
                   set(_LIB_NAME RECIPE_LIB::${package_name}_${_LIBRARY_NAME}${config_suffix})
                   ...
                   if(is_host_windows AND library_type STREQUAL "SHARED")
                     ...
                     find_library(RECIPE_SHARED_FOUND_LIBRARY NAMES ${_LIBRARY_NAME} PATHS ${package_bindir}
                                  NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
                     set(CMAKE_FIND_LIBRARY_SUFFIXES ${_OLD_CMAKE_FIND_LIBRARY_SUFFIXES})
                     if(NOT RECIPE_SHARED_FOUND_LIBRARY)
                       message(DEBUG "DLL library not found, creating UNKNOWN IMPORTED target, searched for: ${_LIBRARY_NAME}")
                       if(NOT TARGET ${_LIB_NAME})
                          add_library(${_LIB_NAME} UNKNOWN IMPORTED)
                       endif()
                       set_target_properties(${_LIB_NAME} PROPERTIES IMPORTED_LOCATION${config_suffix} ${RECIPE_FOUND_LIBRARY})
                     else()
                        if(NOT TARGET ${_LIB_NAME})
                          add_library(${_LIB_NAME} SHARED IMPORTED)
                        endif()
                        set_target_properties(${_LIB_NAME} PROPERTIES IMPORTED_LOCATION${config_suffix} ${RECIPE_SHARED_FOUND_LIBRARY})
                        set_target_properties(${_LIB_NAME} PROPERTIES IMPORTED_IMPLIB${config_suffix} ${RECIPE_FOUND_LIBRARY})
                        message(DEBUG "Found DLL and STATIC at ${RECIPE_SHARED_FOUND_LIBRARY}, ${RECIPE_FOUND_LIBRARY}")
                     endif()
                     unset(RECIPE_SHARED_FOUND_LIBRARY CACHE)
                   else()
                     if(NOT TARGET ${_LIB_NAME})
                     ...
                     set_target_properties(${_LIB_NAME} PROPERTIES IMPORTED_LOCATION${config_suffix} ${RECIPE_FOUND_LIBRARY} IMPORTED_NO_SONAME ${no_soname_mode})
                   endif()
                   list(APPEND _out_libraries_target ${_LIB_NAME})
                   message(VERBOSE "Recipe: Found: ${RECIPE_FOUND_LIBRARY}")
               else()
               ...
               unset(RECIPE_FOUND_LIBRARY CACHE)
           endforeach()
