...
class Recipe(RecipeBase):
    ...
    def generate(self):
        ...
        tc.cache_variables["CMAKE_POSITION_INDEPENDENT_CODE"] = self.options.get("fPIC", True)
        ...
        tc.cache_variables["CMAKEMODULES_VERSION"] = "1.27"  # Prevents cmake from overwriting CMAKEMODULES_PATH with empty env var
    ...
    def _patch_sources(self):
        ...
        replace_in_file(os.path.join(self.source_folder, "pxshared", "include", "foundation", "PxPreprocessor.h"),
        ...
        replace_in_file(os.path.join(physx_source_cmake_dir, "CMakeLists.txt"),
        ...
        for cmake_file in (
            ...
            replace_in_file(os.path.join(physx_source_cmake_dir, cmake_file),
                            f"SET_TARGET_PROPERTIES({target} PROPERTIES POSITION_INDEPENDENT_CODE TRUE)", "")

        # No error for compiler warnings
        replace_in_file(os.path.join(physx_source_cmake_dir, "windows", "CMakeLists.txt"),
                              "/WX", "")
        for cmake_os in ("linux", "mac", "android", "ios"):
            replace_in_file(os.path.join(physx_source_cmake_dir, cmake_os, "CMakeLists.txt"),
                                  "-Werror", "")
    ...
    def package(self):
        ...
        save(os.path.join(self.package_folder, "licenses", "LICENSE"), self._get_license())
        ...
        copy(pattern="*.a", dst=package_dst_lib_dir, src=cmake_installation_dir, keep_path=False)
        copy(pattern="*.so", dst=package_dst_lib_dir, src=cmake_installation_dir, keep_path=False)
        copy(pattern="*.dylib*", dst=package_dst_lib_dir, src=cmake_installation_dir, keep_path=False)
        copy(pattern="*.lib", dst=package_dst_lib_dir, src=cmake_installation_dir, keep_path=False)
        copy(pattern="*.dll", dst=package_dst_bin_dir, src=cmake_installation_dir, keep_path=False)
    ...
    def _copy_external_bin(self):
        ...
        if self.is_linux and False:  # Linux x86_64 GPU support -- not used on Windows
            ...
            copy(pattern="*PhysXGpu*.so", dst=package_dst_lib_dir, src=physx_gpu_dir, keep_path=False)
            ...
            for dll_info in dll_info_list:
                ...
                copy(pattern=dll_info.get("pattern"), dst=package_dst_bin_dir, src=dll_dir, keep_path=False)
