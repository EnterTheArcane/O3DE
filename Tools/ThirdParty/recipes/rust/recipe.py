...
class Recipe(RecipeBase):
    ...
    def build(self):
        ...
        for component in _COMPONENTS:
            ...
            component_folder = os.path.join(self.build_folder, component)
    ...
    def package(self):
        triple = self._target_triple
        os.makedirs(os.path.join(self.package_folder, ".cargo"), exist_ok=True)
        ...
        for component in _COMPONENTS:
            ...
            copy(self, "*", src=payload_folder, dst=self.package_folder)
            copy(
                self,
                "LICENSE*",
                src=component_folder,
                dst=os.path.join(self.package_folder, "licenses", component),
                keep_path=False,
            )
    ...
    def package_info(self):
        ...
        self.buildenv_info.define_path("CARGO_HOME", os.path.join(self.package_folder, ".cargo"))
        self.buildenv_info.define_path("CARGO", cargo)
        self.buildenv_info.define_path("RUSTC", rustc)
        self.conf_info.define("tools.rust:dir", self.package_folder)
