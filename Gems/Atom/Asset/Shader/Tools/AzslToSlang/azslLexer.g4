...
// AzslToSlang addition (not present in the AZSLC grammar).
//
// AZSLC lexes post-MCPP input, so its grammar only ever needed #pragma and #line. This tool lexes
// *raw* AZSL, where #include/#define/#if are everywhere, and without this rule a bare '#' produces a
// lexer error and the directive body is lexed as ordinary code. Routing whole directive lines to the
// PREPROCESSOR channel keeps them out of the scanner's way while the rewriter preserves them verbatim.
// Declared after PragmaDirective/LineDirective so those keep priority for their own forms.
// The escaped-newline alternative keeps multi-line #define bodies in a single token.
