...
namespace AZ
{
    ...
    public:
        ...
        struct FontRenderingPass
        {
            ...
            AZ::Vector2 m_posOffset{ 0.0f, 0.0f };
    ...
    public:
        ...
        bool Load(
            ...
            unsigned int width,
            unsigned int height,
            unsigned int widthNumSlots,
            unsigned int heightNumSlots,
            unsigned int flags,
            float sizeRatio) override;
        ...
        AZ::Vector2 GetTextSize(const char* str, const bool asciiMultiLine, const TextDrawContext& ctx) override;
        ...
        AZ::Vector2 GetMaxEffectOffset(unsigned int effectId) const override;
        bool DoesEffectHaveTransparency(unsigned int effectId) const override;
        void AddCharsToFontTexture(
            const char* chars, int glyphSizeX = ICryFont::defaultGlyphSizeX, int glyphSizeY = ICryFont::defaultGlyphSizeY) override;
        AZ::Vector2 GetKerning(uint32_t leftGlyph, uint32_t rightGlyph, const TextDrawContext& ctx) const override;
        ...
        uint32_t WriteTextQuadsToBuffers(
            SVF_P2F_C4B_T2F_F4B* verts,
            uint16_t* indices,
            uint32_t maxQuads,
            float x,
            float y,
    ...
    private:
        ...
        AZ::Vector2 GetTextSizeUInternal(
            const RHI::Viewport& viewport, const char* str, const bool asciiMultiLine, const TextDrawContext& ctx);
        AZ::Vector2 GetKerningInternal(
            const RHI::Viewport& viewport, uint32_t leftGlyph, uint32_t rightGlyph, const TextDrawContext& ctx) const;
        ...
        using AddFunction = AZStd::function<bool(
            const AZ::Vector3&,
            const AZ::Vector3&,
            const AZ::Vector3&,
            const AZ::Vector3&,
        ...
        {
            TextScaleInfoInternal(const AZ::Vector2& _scale, float _rcpCellWidth)
                : scale(_scale)
        ...
        AZ::Vector2 GetRestoredFontSize(const TextDrawContext& ctx) const;
        ...
        DrawParameters ExtractDrawParameters(
            const AzFramework::TextDrawParameters& params, AZStd::string_view text, bool forceCalculateSize);
