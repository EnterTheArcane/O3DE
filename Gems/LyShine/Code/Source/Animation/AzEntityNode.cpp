    ...
    size_t baseOffset)
{
    ...
    if (!element || element->m_typeId != paramData.GetTypeId())
    {
        ...
        if (element)
        {
            // Allow AZ::Vector2 types to be assigned AZ::Vector2 animation data and AZ::Color types
            // to be assiged AZ::Vector3 animation data
            if (((element->m_typeId == AZ::SerializeTypeInfo<AZ::Vector2>::GetUuid()) && (paramData.GetTypeId() == AZ::SerializeTypeInfo<AZ::Vector2>::GetUuid()))
