...
void ViewportInteraction::UpdateCursor()
{
    ...
    else if (m_activeElementId.IsValid())
    {
                 ...
                 m_interactionType == InteractionType::DIRECT)
        {
            ...
            float topAngle = atan2f(rect.TopRight().GetY() - rect.TopLeft().GetY(), rect.TopRight().GetX() - rect.TopLeft().GetX()) * (180.0f / AZ::Constants::Pi);
            float leftAngle = atan2f(rect.TopLeft().GetY() - rect.BottomLeft().GetY(), rect.TopLeft().GetX() - rect.BottomLeft().GetX()) * (180.0f / AZ::Constants::Pi);
