...
namespace Box3D::Tests
{
    namespace
    {
        ...
        {
        public:
            explicit CharacterMoveRecorder(
            ...
            void OnCharacterMoved(
                const CharacterState& state) override
            {
                ++m_moveCount;
                m_state = state;
            }
        ...
        {
        public:
            explicit JointThresholdRecorder(
                AZ::EntityId entityId)
            {
                BusConnect(entityId);
            }
    ...
    TEST(
        Box3DComponentTests,
        RigidBodyOwnsItsAssociationAndSynchronizesTransforms)
    {
    ...
    TEST(
        Box3DComponentTests,
        HeightfieldUpdatesPreserveTheComponentHandles)
    {
        NameDictionaryScope nameDictionaryScope;
        AZ::ComponentDescriptor* transformDescriptor = AzFramework::TransformComponent::CreateDescriptor();
    ...
    TEST(
        Box3DComponentTests,
        UniformScaleChangesRecookColliderGeometry)
    {
        NameDictionaryScope nameDictionaryScope;
        AZ::ComponentDescriptor* transformDescriptor = AzFramework::TransformComponent::CreateDescriptor();
        AZ::ComponentDescriptor* colliderDescriptor = ColliderComponent::CreateDescriptor();
    ...
    TEST(
        Box3DComponentTests,
        StepDispatchesQueuedCharacterMovesByEntity)
    {
    ...
    TEST(
        Box3DComponentTests,
        StepDispatchesJointThresholdsByEntity)
    {
        SystemConfiguration systemConfiguration;
        systemConfiguration.m_workerCount = 1;
        System system(systemConfiguration);
