...
namespace Box3D::Tests
{
    ...
    TEST(
        Box3DCookingTests,
        RegistersProviderOwnedInterfaceAndCooksEveryGeometryFamily)
    {
    ...
    TEST(
        Box3DCookingTests,
        RejectsStaleHandlesAndInstanceOverridesOfBakedData)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        const BodyHandle bodyHandle = CreateStaticBody(system, worldHandle);
        ...
        ASSERT_TRUE(
            system.Raycast(
                firstHandle,
                -2.0f * AZ::Vector3::CreateAxisX() + AZ::Vector3(0.0f, 2.0f, 3.0f),
    ...
    TEST(
        Box3DCookingTests,
        KeepsMaterialsStableAcrossCookedAndLiveShapeLifetimes)
    {
        System system;
    ...
    TEST(
        Box3DCookingTests,
        UpdatesCompoundFrictionAndRestitutionAcrossEveryMaterialSlot)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        const BodyHandle bodyHandle = CreateStaticBody(system, worldHandle);
        ...
        CompoundShapeConfiguration compound;
        compound.m_children.push_back({SphereShapeConfiguration{0.5f}});
        compound.m_children.push_back({SphereShapeConfiguration{0.5f}, AZ::Transform::CreateTranslation(AZ::Vector3::CreateAxisX(2.0f))});
        configuration.m_geometry = AZStd::move(compound);
