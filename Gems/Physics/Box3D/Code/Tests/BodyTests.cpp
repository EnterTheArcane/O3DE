...
namespace Box3D::Tests
{
    namespace
    {
        BodyHandle CreateDynamicSphere(
            System& system,
            ...
            const AZ::Transform& transform)
        {
            ...
            if (system.CreateShape(worldHandle, bodyHandle, shapeConfiguration))
            {
                return bodyHandle;
            }
    ...
    TEST(
        Box3DBodyTests,
        ConvertsPointsVectorsAndPointVelocities)
    {
        System system;
    ...
    TEST(
        Box3DBodyTests,
        MutatesPropertiesMassAndWakePolicy)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(
        Box3DBodyTests,
        AppliesForcesImpulsesTorqueAndKinematicTargets)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(
        Box3DBodyTests,
        DefersMassUpdatesForBulkShapeChanges)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        RigidBodyConfiguration bodyConfiguration;
    ...
    TEST(
        Box3DBodyTests,
        EnumeratesShapesAndJointsWithoutAllocatingOutputContainers)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        RigidBodyConfiguration bodyConfiguration;
    ...
    TEST(
        Box3DBodyTests,
        ReturnsCurrentBodyAndShapeContactsWithExplicitBufferSizing)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(
        Box3DBodyTests,
        RejectsUnsupportedBodyTypeChangesWithoutLockingTheWorld)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        RigidBodyConfiguration bodyConfiguration;
    ...
    TEST(
        Box3DBodyTests,
        ReturnsCurrentSensorOverlapsForBodiesAndShapes)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        ASSERT_TRUE(system.SetWorldGravity(worldHandle, AZ::Vector3::CreateZero()));
    ...
    TEST(
        Box3DBodyTests,
        InspectsAndMutatesShapeStateWithoutRecookingGeometry)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        RigidBodyConfiguration bodyConfiguration;
    ...
    TEST(
        Box3DBodyTests,
        ReportsEntityNameAndCreationOnlyPolicy)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
