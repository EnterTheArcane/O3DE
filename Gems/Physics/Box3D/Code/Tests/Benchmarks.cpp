...
namespace Box3D::Benchmarks
{
        ...
        benchmark::State& state)
    {
        ...
        if constexpr (CaptureTailSamples)
        {
            ...
            for ([[maybe_unused]] auto iteration : state)
            {
                for (double& sample : tailSamples)
                {
                    ...
                    const bool stepSucceeded = system.StepWorld(worldHandle, worldConfiguration.m_fixedTimeStep);
        ...
        qualityValid = allStepsSucceeded
            && minimumHeight >= 0.45f
            && maximumDisplacement <= 0.02f
            && qualityValid;
    ...
    void StepSettledBoxes(
        benchmark::State& state)
    {
        StepSettledBoxesImpl<false>(state);
    }

    void StepSettledBoxesTail(
        benchmark::State& state)
    {
        StepSettledBoxesImpl<true>(state);
    }
        ...
        benchmark::State& state)
    {
        ...
        for ([[maybe_unused]] auto iteration : state)
        {
            ...
            for (AZ::u32 bodyIndex = 0; bodyIndex < bodyCount; ++bodyIndex)
            {
                ...
                const ShapeHandle shapeHandle = CreateSphere(system, worldHandle, bodyHandle);
                iterationValid = bodyHandle && shapeHandle && iterationValid;
                benchmark::DoNotOptimize(shapeHandle);
                bodies.push_back(bodyHandle);
            }
            for (BodyHandle bodyHandle : bodies)
            {
                const bool bodyDestroyed = system.DestroyBody(worldHandle, bodyHandle);
                iterationValid = bodyDestroyed && iterationValid;
                benchmark::DoNotOptimize(bodyDestroyed);
            }
            bodies.clear();
            if (!iterationValid)
            {
                state.SkipWithError("Failed to create or destroy a Box3D lifecycle benchmark body.");
                return;
            }
        }
        ...
        state.counters["BackgroundWorkers"] = 0;
        state.counters["CallerParticipates"] = 1;
        state.counters["Notifications"] = 0;
        ...
        benchmark::State& state)
    {
        ...
        AZStd::array<Hit, expectedHitCount> hits;
        QueryResult result;
        ...
        for ([[maybe_unused]] auto iteration : state)
        {
            for (AZ::u32 queryIndex = 0; queryIndex < queryCount; ++queryIndex)
            {
                ...
                if (result.m_requiredHitCount == expectedHitCount
                    && !result.HasOverflow())
                {
                    ++completeQueryCount;
                }
    ...
    BENCHMARK(StepSettledBoxesTail)
        ->Name("Box3D/Tail/Step/SettledBoxes")
        ->Args({128, 1})
        ->Args({128, 4})
        ->Args({128, 8})
        ->Args({1024, 1})
