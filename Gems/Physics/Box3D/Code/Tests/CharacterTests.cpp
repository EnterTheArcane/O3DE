...
namespace Box3D::Tests
{
    ...
    TEST(
        Box3DCharacterTests,
        GroundStickSnapsToWalkableSupport)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        CreateGround(system, worldHandle);
    ...
    TEST(
        Box3DCharacterTests,
        StepHeightControlsObstacleTraversal)
    {
    ...
    TEST(
        Box3DCharacterTests,
        SteepSurfaceReportsSlidingSupport)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(
        Box3DCharacterTests,
        InteractionScalePushesDynamicBodies)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(
        Box3DCharacterTests,
        ReturnsRetainedProviderConfiguration)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        CharacterConfiguration configuration;
    ...
    TEST(
        Box3DCharacterTests,
        RecycledSlotsRejectStaleHandlesAndResetColdState)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
