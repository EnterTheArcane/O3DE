...
namespace Box3D::Tests
{
    ...
    TEST(
        Box3DQueryTests,
        RaycastClosestAndFiltersReturnExpectedBodies)
    {
    ...
    TEST(
        Box3DQueryTests,
        ClosestRaycastBatchUsesCallerStorageAndReportsOverflow)
    {
    ...
    TEST(
        Box3DQueryTests,
        ClosestRaycastBatchPreservesPerRequestFiltering)
    {
        QueryFixture fixture;
    ...
    TEST(
        Box3DQueryTests,
        RaycastClosestIgnoresInitialOverlapOnEveryFilteringPath)
    {
        System system;
    ...
    TEST(
        Box3DQueryTests,
        AxisAlignedBoxRaycastsPreserveClosestQuerySemantics)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        ...
        ASSERT_TRUE(
            system.SetBodyTransform(
                worldHandle,
                boxBody,
                AZ::Transform::CreateFromQuaternionAndTranslation(rotation, 3.0f * AZ::Vector3::CreateAxisX())));
    ...
    TEST(
        Box3DQueryTests,
        AxisAlignedClosestRaycastsMatchAllHitsAcrossDirectionsAndFilters)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        constexpr AZ::u64 TargetCategory = AZ::u64{1} << 1;
        const AZStd::array directions{AZ::Vector3::CreateAxisX(),
    ...
    TEST(
        Box3DQueryTests,
        ClosestRaycastMapsReusedNativeShapeIdsToCurrentHandles)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        RigidBodyConfiguration bodyConfiguration;
    ...
    TEST(
        Box3DQueryTests,
        EveryConvexQueryGeometryCanSweep)
    {
        QueryFixture fixture;
    ...
    TEST(
        Box3DQueryTests,
        QueryUniformScaleIsPartOfTransform)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        RigidBodyConfiguration bodyConfiguration;
    ...
    TEST(
        Box3DQueryTests,
        OverlapAndAabbOverlapReportCapacityAndStableOrder)
    {
        QueryFixture fixture;
    ...
    TEST(
        Box3DQueryTests,
        CompactOverlapMatchesGeneralResultsAndFilters)
    {
        QueryFixture fixture;
        OverlapRequest request;
    ...
    TEST(
        Box3DQueryTests,
        OverlapSortsLargeResultSetsWithoutChangingDefaultHitData)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(
        Box3DQueryTests,
        SphereOverlapHandlesRotatedBoxFacesEdgesAndCorners)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        const AZ::Quaternion rotation = AZ::Quaternion::CreateRotationZ(AZ::Constants::QuarterPi);
        ...
        const auto expectOverlap = [&](const AZ::Vector3& center, BodyHandle expectedBody, bool expected)
        {
            ...
            const QueryResult result = system.Overlap(worldHandle, request, hits);
            AZ::u32 expectedHitCount = 0;
            if (expected)
            {
                expectedHitCount = 1;
            }
            EXPECT_EQ(result.m_hitCount, expectedHitCount);
    ...
    TEST(
        Box3DQueryTests,
        BodyQueriesOnlyTraverseTheRequestedBody)
    {
        QueryFixture fixture;
    ...
    TEST(
        Box3DQueryTests,
        CompoundMeshFacesPreserveProviderMaterialIndices)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(
        Box3DQueryTests,
        InvalidRequestsReturnEmptyResults)
    {
        QueryFixture fixture;
