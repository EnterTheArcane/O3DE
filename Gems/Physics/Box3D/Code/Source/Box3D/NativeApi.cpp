...
namespace Box3D
{
    ...
    AZ::u8 MeshGeometry::GetMaterialIndex(
        const size_t faceIndex) const
    {
        const auto* mesh = static_cast<const b3MeshData*>(m_data);
        if (mesh && faceIndex < aznumeric_cast<size_t>(mesh->triangleCount))
        {
            return b3GetMeshMaterialIndices(mesh)[faceIndex];
        }
    ...
    AZ::u8 HeightFieldGeometry::GetMaterialIndex(
        const size_t faceIndex) const
    {
        ...
        const AZ::u8* materialIndices = b3GetHeightFieldMaterialIndices(heightField);
        if (faceIndex < faceCount && materialIndices)
        {
            return materialIndices[faceIndex >> 1];
        }
    ...
    namespace
    {
        ...
        AZ::Vector3 TransformGeometryPoint(
        ...
        AZ::Vector3 TransformGeometryVector(
            const GeometryTransform& transform,
            ...
            float& scale)
        {
            ...
            if (!AZ::IsFiniteFloat(lengthSq)
                || lengthSq < MinimumGeometryScale * MinimumGeometryScale
                || !AZ::IsClose(basisY.GetLengthSq(), lengthSq, tolerance)
                || !AZ::IsClose(basisZ.GetLengthSq(), lengthSq, tolerance)
                || !AZ::IsClose(basisX.Dot(basisY), 0.0f, tolerance)
                || !AZ::IsClose(basisX.Dot(basisZ), 0.0f, tolerance)
        ...
        float MixFriction(
            const float valueA,
            const uint64_t materialIdA,
            const float valueB,
            const uint64_t materialIdB)
        {
            ...
            const char* taskName)
        {
            ...
            if (!nativeTaskContext
                || !nativeTaskContext->m_jobContext
        ...
        void FinishNativeTask(
            void* task,
            [[maybe_unused]] void* userContext)
        {
        ...
        void* CreateNativeDebugShape(
            const b3DebugShape* shape,
            void* context)
        {
            return CreateReplayDebugShape(shape, context);
        }

        void DestroyNativeDebugShape(
            void* shape,
            void* context)
        {
            ...
            [[maybe_unused]] const int lineNumber)
        {
            ...
            AZ_Error("Box3D", false, "Native assertion failed: %s (%s:%d)", reportedCondition, reportedFileName, lineNumber);
            return 1;
        }

        void ReportNativeWarning(
            [[maybe_unused]] const char* message)
        {
            if (message)
            {
                AZ_Warning("Box3D", false, "%s", message);
                return;
            }

            AZ_Warning("Box3D", false, "%s", "<empty native warning>");
        }
        ...
        bool ConvertCastOutput(
            const b3CastOutput& output,
            GeometryCastHit& hit)
        {
            if (!output.hit)
            {
        ...
        NativeBodyType FromNative(
            const b3BodyType type)
        {
            switch (type)
            {
        ...
        NativeShapeDefinition ToNative(
            ...
            void* context)
        {
            ...
            const OverlapCandidate candidate{{b3StoreShapeId(shapeId)}, b3Shape_GetUserData(shapeId)};
            if (!overlapContext->m_predicate || overlapContext->m_predicate(candidate, overlapContext->m_context))
            {
        ...
        {
            explicit MoverScratchData(
                const size_t maximumPlaneCount)
        ...
        bool CollectMoverPlanes(
            const b3ShapeId shapeId,
            const b3PlaneResult* planeResults,
            ...
            void* context)
        {
            ...
            for (int planeIndex = 0; planeIndex < planeCount; ++planeIndex)
            {
                ...
                if (!std::isfinite(plane.plane.normal.x)
                    || !std::isfinite(plane.plane.normal.y)
                    || !std::isfinite(plane.plane.normal.z)
                    || !std::isfinite(plane.plane.offset)
                    || !std::isfinite(plane.point.x)
                    || !std::isfinite(plane.point.y)
                    || !std::isfinite(plane.point.z)
                    || b3LengthSquared(plane.plane.normal) == 0.0f)
                {
                    data.m_invalidPlane = true;
                    continue;
                }
                if (plane.plane.normal.x == 0.0f)
                {
                    plane.plane.normal.x = 0.0f;
                }
                if (plane.plane.normal.y == 0.0f)
                {
                    plane.plane.normal.y = 0.0f;
                }
                if (plane.plane.normal.z == 0.0f)
                {
                    plane.plane.normal.z = 0.0f;
                }
                if (plane.plane.offset == 0.0f)
                {
                    plane.plane.offset = 0.0f;
                }
                if (plane.point.x == 0.0f)
                {
                    plane.point.x = 0.0f;
                }
                if (plane.point.y == 0.0f)
                {
                    plane.point.y = 0.0f;
                }
                if (plane.point.z == 0.0f)
                {
                    plane.point.z = 0.0f;
                }
            ...
            void* context)
        {
            ...
            const BodyId storedBodyId{b3StoreBodyId(bodyId)};
            return storedBodyId != data.m_ignoredBodyIdA
                && storedBodyId != data.m_ignoredBodyIdB
                && !b3Shape_IsSensor(shapeId)
                && b3Body_IsEnabled(bodyId);
        }

        bool LessMoverEntry(
            const MoverEntry& left,
            const MoverEntry& right)
        {
        ...
        constexpr AZ::u32 ToUnsigned(
            const int value)
        {
            if (value > 0)
            {
                return static_cast<AZ::u32>(value);
            }
        ...
        AZ::Color GetDebugColor(
            const b3HexColor color)
        {
        ...
        void* CreateReplayDebugShape(
            const b3DebugShape* source,
            [[maybe_unused]] void* context)
        {
            ...
            {
                if (!hull || hull->vertexCount < 4 || hull->vertexOffset <= 0 || hull->edgeOffset <= 0 || hull->faceOffset <= 0)
                {
                    return false;
                }
                ...
                for (int faceIndex = 0; faceIndex < hull->faceCount; ++faceIndex)
                {
                    ...
                    while (nextEdge != firstEdge)
                    {
                        primitive.m_indices.insert(
                            ...
                            {edges[firstEdge].origin, edges[edge].origin, edges[nextEdge].origin});
                        edge = nextEdge;
            ...
            {
                if (!mesh || mesh->vertexCount < 3 || mesh->triangleCount < 1 || mesh->vertexOffset <= 0 || mesh->triangleOffset <= 0)
                {
                    return false;
                }
            ...
            case b3_heightShape:
                {
                    ...
                    const AZ::u8* materials = nullptr;
                    if (heightField.materialOffset > 0)
                    {
                        materials =
                    ...
                    for (int row = 0; row < heightField.rowCount; ++row)
                    {
                        for (int column = 0; column < heightField.columnCount; ++column)
                        {
                            ...
                            primitive.m_vertices.push_back(
                                FromNative(b3Vec3{column * heightField.scale.x, height * heightField.scale.y, row * heightField.scale.z}));
                        }
                    }
                    ...
                    for (int row = 0; row + 1 < heightField.rowCount; ++row)
                    {
                        for (int column = 0; column + 1 < heightField.columnCount; ++column)
                        {
                            ...
                            if (materials && materials[materialIndex] == B3_HEIGHT_FIELD_HOLE)
                            {
                                continue;
                            }
                            ...
                            if (heightField.clockwise)
                            {
                                primitive.m_indices.insert(
                                    primitive.m_indices.end(),
                                    {upperLeft, lowerRight, upperRight, upperLeft, lowerLeft, lowerRight});
                            }
                            else
                            {
                                primitive.m_indices.insert(
                                    primitive.m_indices.end(),
                                    {upperLeft, upperRight, lowerRight, upperLeft, lowerRight, lowerLeft});
                            }
                        }
                    }
                    break;
            ...
            void* context)
        {
            if (value)
            {
                static_cast<DebugContext*>(context)->m_renderer.DrawText(FromNativePosition(position), value, GetDebugColor(color));
                return;
            }

            static_cast<DebugContext*>(context)->m_renderer.DrawText(FromNativePosition(position), "", GetDebugColor(color));
        }

        b3DebugDraw MakeDebugDraw(
            const DebugDrawSettings& settings,
            DebugContext& context,
            bool (*drawShape)(void*, b3WorldTransform, b3HexColor, void*))
        {
    ...
    AZStd::shared_ptr<const CompoundGeometry> CompoundGeometry::CloneWithMaterials(
        AZStd::span<const SurfaceMaterial> materials) const
    {
        const auto* compound = static_cast<const b3CompoundData*>(m_data);
        if (!compound || materials.empty() || m_materialSources.size() != aznumeric_cast<size_t>(compound->materialCount))
        {
            return {};
        }
        ...
        for (int meshIndex = 0; meshIndex < compound->meshCount; ++meshIndex)
        {
            ...
            if (!mesh.meshData || mesh.meshData->materialCount < 1 || mesh.meshData->materialCount > B3_MAX_COMPOUND_MESH_MATERIALS)
            {
                return {};
            }
    ...
    public:
        ...
        ReplayInfo GetInfo() const override
        {
            ...
            return {ToUnsigned(info.frameCount),
                    ToUnsigned(info.workerCount),
                    info.timeStep,
                    ToUnsigned(info.subStepCount),
                    info.lengthScale,
        ...
        void Seek(
            const AZ::u32 frame) override
        {
            const DeterministicFloatScope floatScope;
            b3RecPlayer_SeekFrame(
                m_player,
                aznumeric_cast<int>(AZStd::min(frame, aznumeric_cast<AZ::u32>((AZStd::numeric_limits<int>::max)()))));
        }
        ...
        void SetWorkerCount(
            const AZ::u32 workerCount) override
        {
        ...
        void SetKeyframePolicy(
            const size_t budgetBytes,
            const AZ::u32 minimumInterval) override
        {
        ...
        bool GetBody(
            const AZ::u32 index,
            ReplayBody& body) const override
        {
            body = {};
        ...
        bool GetFrameQuery(
            const AZ::u32 index,
            ReplayQuery& query) const override
        {
            query = {};
            ...
            query.m_id = info.id;
            query.m_name = AZ::Name{};
            if (info.name)
            {
                query.m_name = AZ::Name(info.name);
            }
            return true;
        }

        bool GetFrameQueryHit(
            const AZ::u32 queryIndex,
            const AZ::u32 hitIndex,
            ReplayQueryHit& hit) const override
        {
            hit = {};
            ReplayQuery query;
        ...
        bool Draw(
            ...
            const AZ::s32 queryIndex,
            const AZ::s32 selectedQueryIndex) override
        {
            if (!AZ::IsFiniteFloat(settings.m_forceScale)
                || settings.m_forceScale < 0.0f
                || !AZ::IsFiniteFloat(settings.m_jointScale)
                || settings.m_jointScale < 0.0f
                || queryIndex < -1
    ...
    AZStd::unique_ptr<Recording> CreateRecording(
        const size_t initialCapacityBytes)
    {
        ...
        if (recording)
        {
            return AZStd::unique_ptr<Recording>(aznew Recording(recording));
        }
        ...
        Recording& recording)
    {
        ...
        if (!IsValid(worldId) || !nativeRecording)
        {
            return false;
        }
        const DeterministicFloatScope floatScope;
        ...
        const AZ::u32 workerCount)
    {
        const DeterministicFloatScope floatScope;
        return !data.empty()
            && data.size() <= aznumeric_cast<size_t>((AZStd::numeric_limits<int>::max)())
            && workerCount > 0
        ...
        const AZ::u32 workerCount)
    {
        ...
        if (player)
        {
            return AZStd::unique_ptr<IReplay>(aznew ReplayInstance(player));
        }

        return nullptr;
    }

    void SetLengthUnitsPerMeter(
        const float lengthUnitsPerMeter)
    {
        InstallNativeCallbacks();
        b3SetLengthUnitsPerMeter(lengthUnitsPerMeter);
    }

    void SetStallWarningThreshold(
        const float seconds)
    {
        InstallNativeCallbacks();
    ...
    {
        if (!IsValid(worldId)
            || !AZ::IsFiniteFloat(settings.m_forceScale)
            || settings.m_forceScale < 0.0f
            || !AZ::IsFiniteFloat(settings.m_jointScale)
            || settings.m_jointScale < 0.0f)
        {
        ...
        const NativeWorldConfiguration& configuration)
    {
        ...
        definition.frictionCallback = nullptr;
        if (configuration.m_frictionCallback)
        {
            definition.frictionCallback = MixFriction;
        }
        definition.restitutionCallback = nullptr;
        if (configuration.m_restitutionCallback)
        {
            definition.restitutionCallback = MixRestitution;
        }
        definition.workerCount = configuration.m_workerCount;
        if (configuration.m_taskContext && configuration.m_taskContext->m_jobContext)
        {
        ...
        const NativeWorldConfiguration& configuration)
    {
        ...
        b3FrictionCallback* frictionCallback = nullptr;
        if (configuration.m_frictionCallback)
        {
            frictionCallback = MixFriction;
        }
        b3World_SetFrictionCallback(world, frictionCallback);
        b3RestitutionCallback* restitutionCallback = nullptr;
        if (configuration.m_restitutionCallback)
        {
            restitutionCallback = MixRestitution;
        }
        b3World_SetRestitutionCallback(world, restitutionCallback);
    ...
    void SetContactCallbacks(
        ...
        const bool enableCustomFilterCallback,
        const bool enablePreSolveCallback)
    {
        if (!IsValid(worldId))
        {
            return;
        ...
        const b3WorldId world = b3LoadWorldId(worldId.m_value);
        decltype(&FilterContact) customFilterCallback = nullptr;
        if (callbacks && enableCustomFilterCallback)
        {
            customFilterCallback = FilterContact;
        }
        b3World_SetCustomFilterCallback(world, customFilterCallback, callbacks);
        decltype(&FilterPreSolveContact) preSolveCallback = nullptr;
        if (callbacks && enablePreSolveCallback)
        {
            preSolveCallback = FilterPreSolveContact;
        }
        b3World_SetPreSolveCallback(world, preSolveCallback, callbacks);
    }
    ...
    AZ::Vector3 GetGravity(
        const WorldId worldId)
    {
        return FromNative(b3World_GetGravity(b3LoadWorldId(worldId.m_value)));
    }

    AZ::Aabb GetWorldAabb(
        const WorldId worldId)
    {
        if (IsValid(worldId))
        {
            return FromNative(b3World_GetBounds(b3LoadWorldId(worldId.m_value)));
        }
    ...
    bool Explode(
        ...
        const float radius,
        const float falloff,
        const float impulsePerArea)
    {
        if (!IsValid(worldId)
            || !position.IsFinite()
            || !AZ::IsFiniteFloat(radius)
            || radius < 0.0f
            || !AZ::IsFiniteFloat(falloff)
            || falloff < 0.0f
            || !AZ::IsFiniteFloat(impulsePerArea))
        {
            return false;
        }
        ...
        WorldStatistics& statistics)
    {
        ...
        if (HasAnyFlag(flags, StatisticsFlags::Capacity))
        {
            ...
            statistics.m_capacityHighWaterMarks = {
                .m_staticShapes = ToUnsigned(capacity.staticShapeCount),
                .m_dynamicShapes = ToUnsigned(capacity.dynamicShapeCount),
                .m_staticBodies = ToUnsigned(capacity.staticBodyCount),
                .m_dynamicBodies = ToUnsigned(capacity.dynamicBodyCount),
                .m_contacts = ToUnsigned(capacity.contactCount),
            };
        }
        ...
        AZStd::vector<NativeJointThresholdEvent>& jointEvents)
    {
        ...
        if ((flags & StepEventFlags::Joints) != StepEventFlags::None)
        {
            ...
            for (int eventIndex = 0; eventIndex < nativeJointEvents.count; ++eventIndex)
            {
                ...
                jointEvents.push_back({{b3StoreJointId(event.jointId)}, event.userData, JointHandle::Invalid});
            }
        }
    }
    ...
    namespace
    {
        bool ConvertContactData(
            const b3ContactData& nativeData,
            ...
            AZStd::vector<NativeContactPoint>& points)
        {
            ...
            AZStd::sort(
                ...
                [](const ContactData& left, const ContactData& right)
                {
                    ...
                    return AZStd::max(left.m_shapeIdA.m_value, left.m_shapeIdB.m_value)
        ...
        AZStd::vector<NativeContactPoint>& points)
    {
        ...
        points.clear();
        auto* scratchData = static_cast<ContactScratchData*>(scratch.m_data);
        if (!IsValid(shapeId) || !scratchData)
        {
            return false;
        }
        ...
        AZStd::vector<SensorOverlapData>& overlaps)
    {
        ...
        AZStd::sort(
            ...
            {
                if (left.m_sensor.m_shapeId.m_value != right.m_sensor.m_shapeId.m_value)
                {
        ...
        MoverResult& result)
    {
        ...
        if (!IsValid(worldId)
            ...
            || deltaTime < 0.0f
            || maximumIterations == 0
            || !AZ::IsFiniteFloat(minimumTranslation)
            || minimumTranslation < 0.0f)
        {
            return false;
        }
        ...
        if (deltaTime > 0.0f)
        {
            ...
            result.m_clippedVelocity = FromNative(
                b3ClipVector(requestedVelocity, data->m_solverPlanes.data(), aznumeric_cast<int>(data->m_solverPlanes.size())));
        }
        return true;
    }
        ...
        CastHit& hit)
    {
        ...
        int normalizedChildIndex = -1;
        if (b3Shape_GetType(result.shapeId) == b3_compoundShape)
        {
            normalizedChildIndex = result.childIndex;
        }
        ...
        GeometryCastHit& hit)
    {
        ...
        if (!hull || !start.IsFinite() || !translation.IsFinite())
        {
            return false;
        }
        ...
        GeometryCastHit& hit)
    {
        ...
        if (!meshData || !start.IsFinite() || !translation.IsFinite())
        {
            return false;
        }
        ...
        GeometryCastHit& hit)
    {
        ...
        if (!heightField || !start.IsFinite() || !translation.IsFinite())
        {
            return false;
        }
        ...
        GeometryCastHit& hit)
    {
        ...
        if (!compound || !start.IsFinite() || !translation.IsFinite())
        {
            return false;
        }
    ...
    {
        const b3CompoundData* compound = GeometryCastAccess::Get(geometry);
        if (!compound || !transform.GetTranslation().IsFinite() || !transform.GetRotation().IsFinite())
        {
            return AZ::Aabb::CreateNull();
        }
    ...
    void CastShape(
        ...
        void* context)
    {
        if (!IsValid(worldId) || points.empty() || points.size() > B3_MAX_SHAPE_CAST_POINTS || !callback)
        {
            return;
        }
    ...
    TreeVisitStatistics OverlapAabb(
        ...
        void* context)
    {
        if (!IsValid(worldId) || !bounds.IsValid() || !callback)
        {
            return {};
        }
        ...
        const b3AABB nativeBounds = {{minimum.GetX(), -maximum.GetZ(), minimum.GetY()}, {maximum.GetX(), -minimum.GetZ(), maximum.GetY()}};
        b3QueryFilter filter = b3DefaultQueryFilter();
        filter.categoryBits = categoryBits;
        filter.maskBits = maskBits;
        OverlapContext overlapContext{callback, context};
        ...
        const BodyConfiguration& configuration)
    {
        ...
        definition.motionLocks = {configuration.m_motionLocks.m_linearX,
                                  configuration.m_motionLocks.m_linearZ,
                                  configuration.m_motionLocks.m_linearY,
                                  configuration.m_motionLocks.m_angularX,
                                  configuration.m_motionLocks.m_angularZ,
                                  configuration.m_motionLocks.m_angularY};
        definition.enableSleep = configuration.m_enableSleep;
        ...
        AZ::Name name)
    {
        ...
        if (!name.IsEmpty())
        {
            nativeName = name.GetCStr();
        }
        b3Body_SetName(b3LoadBodyId(bodyId.m_value), nativeName);
    }

    AZ::Transform GetBodyTransform(
        const BodyId bodyId)
    {
    ...
    NativeMassProperties GetMassProperties(
        const BodyId bodyId)
    {
    ...
    NativeMassProperties GetShapeMassProperties(
        const ShapeId shapeId)
    {
        if (!IsValid(shapeId))
        {
    ...
    NativeMotionLocks GetMotionLocks(
        const BodyId bodyId)
    {
        if (!IsValid(bodyId))
        {
    ...
    bool IsBodyContactRecyclingEnabled(
        const BodyId bodyId)
    {
        ...
        JointScratch& scratch)
    {
        ...
        {
            data->m_joints[aznumeric_cast<size_t>(jointIndex)] = {b3StoreJointId(data->m_nativeJoints[aznumeric_cast<size_t>(jointIndex)])};
        }
    ...
    void* GetJointUserData(
        const JointId jointId)
    {
        if (IsValid(jointId))
        {
            return b3Joint_GetUserData(b3LoadJointId(jointId.m_value));
        }

        return nullptr;
    }

    AZ::Vector3 GetJointConstraintForce(
        const JointId jointId)
    {
        if (IsValid(jointId))
        {
            return FromNative(b3Joint_GetConstraintForce(b3LoadJointId(jointId.m_value)));
        }

        return AZ::Vector3::CreateZero();
    }

    AZ::Vector3 GetJointConstraintTorque(
        const JointId jointId)
    {
        if (IsValid(jointId))
        {
            return FromNative(b3Joint_GetConstraintTorque(b3LoadJointId(jointId.m_value)));
        }

        return AZ::Vector3::CreateZero();
    }

    float GetJointLinearSeparation(
        const JointId jointId)
    {
        if (IsValid(jointId))
        {
            return b3Joint_GetLinearSeparation(b3LoadJointId(jointId.m_value));
        }

        return 0.0f;
    }

    float GetJointAngularSeparation(
        const JointId jointId)
    {
        if (IsValid(jointId))
        {
            return b3Joint_GetAngularSeparation(b3LoadJointId(jointId.m_value));
        }
        ...
        const AZ::Transform& localTransform)
    {
        ...
        if (geometry)
        {
            return CreateHullShape(bodyId, configuration, *geometry);
        }
        ...
        const GeometryTransform& transform)
    {
        ...
        if (!halfExtents.IsFinite()
            || halfExtents.GetMinElement() <= 0.0f
            || !IsRigidTransform(transform.m_localTransform)
            || !IsValidGeometryScale(transform.m_scale)
        ...
        if (IsIdentityScale(transform.m_scale) && IsIdentityScale(transform.m_postScale))
        {
            ...
            if (hull)
            {
                return AZStd::unique_ptr<HullGeometry>(aznew HullGeometry(hull));
            }

            return nullptr;
        }
        b3HullData* localHull = nullptr;
        if (IsIdentityPose(transform.m_localTransform) && IsIdentityScale(transform.m_scale))
        {
            localHull = b3CloneHull(&box.base);
        }
        else
        {
            localHull = b3CloneAndTransformHull(&box.base, localTransform, ToNativeScale(transform.m_scale));
        }
        if (!localHull)
        {
            return {};
        ...
        const GeometryTransform& transform)
    {
        ...
        if (transformedHull)
        {
            return AZStd::unique_ptr<HullGeometry>(aznew HullGeometry(transformedHull));
        }
        ...
        const bool identifyEdges)
    {
        ...
        definition.materialIndices = nullptr;
        if (!nativeMaterialIndices.empty())
        {
            definition.materialIndices = nativeMaterialIndices.data();
        }
        ...
        if (mesh)
        {
            return AZStd::unique_ptr<MeshGeometry>(aznew MeshGeometry(mesh));
        }
    ...
    ShapeId CreateMeshShape(
        ...
        const MeshGeometry& geometry)
    {
        if (!IsValid(bodyId) || !geometry.m_data || GetBodyType(bodyId) != NativeBodyType::Static)
        {
            return {};
        }

        NativeShapeDefinition definition = ToNative(configuration);
        return {b3StoreShapeId(
            b3CreateMeshShape(
                b3LoadBodyId(bodyId.m_value),
                &definition.m_value,
        ...
        const bool clockwise)
    {
        ...
        if (columnCount < 2
            || rowCount < 2
            || columnCount > maximumByteCount
            || rowCount > maximumByteCount
            || !gridResolution.IsFinite()
            || gridResolution.GetX() <= 0.0f
            || gridResolution.GetY() <= 0.0f
            || !scale.IsFinite()
            || scale.GetMinElement() <= 0.0f
            || columnCount > maximumByteCount / rowCount
        ...
        if (heightCount != heights.size()
            || cellCount != materialIndices.size()
            || heightCount > (maximumByteCount - 7) / sizeof(AZ::u16)
            || cellCount > maximumByteCount - 7
        ...
        if (heightField)
        {
            return AZStd::unique_ptr<HeightFieldGeometry>(aznew HeightFieldGeometry(heightField));
        }
        ...
        const HeightFieldGeometry& geometry)
    {
        ...
        return {b3StoreShapeId(
            b3CreateHeightFieldShape(
                b3LoadBodyId(bodyId.m_value),
                &definition.m_value,
                static_cast<const b3HeightFieldData*>(geometry.m_data)))};
    }

    AZStd::unique_ptr<CompoundGeometry> CreateCompoundGeometry(
        const CompoundGeometry::Definition& definition)
    {
        InstallNativeCallbacks();
        ...
        {
            if (!capsule.m_center1.IsFinite()
                || !capsule.m_center2.IsFinite()
                || !AZ::IsFiniteFloat(capsule.m_radius)
        ...
        for (const CompoundGeometry::Hull& hull : definition.m_hulls)
        {
            if (!hull.m_geometry
                ...
                || !hull.m_transform.GetTranslation().IsFinite()
                || !hull.m_transform.GetRotation().IsFinite())
            {
                return {};
            }
        ...
        {
            const b3MeshData* geometry = nullptr;
            if (mesh.m_geometry)
            {
                geometry = GeometryCastAccess::Get(*mesh.m_geometry);
            }
            if (!geometry
                || mesh.m_materials.empty()
                || mesh.m_materials.size() > B3_MAX_COMPOUND_MESH_MATERIALS
                || mesh.m_materials.size() != aznumeric_cast<size_t>(geometry->materialCount)
                || !mesh.m_transform.GetTranslation().IsFinite()
                || !mesh.m_transform.GetRotation().IsFinite()
                || !mesh.m_scale.IsFinite()
                || mesh.m_scale.GetX() == 0.0f
                || mesh.m_scale.GetY() == 0.0f
                || mesh.m_scale.GetZ() == 0.0f)
            {
                return {};
            }
    ...
    ShapeId CreateCompoundShape(
        ...
        const CompoundGeometry& geometry)
    {
        if (!IsValid(bodyId) || !geometry.m_data || GetBodyType(bodyId) != NativeBodyType::Static || configuration.m_isSensor)
        {
            return {};
        }
        ...
        NativeShapeDefinition definition = ToNative(configuration);
        return {b3StoreShapeId(
            b3CreateCompoundShape(b3LoadBodyId(bodyId.m_value), &definition.m_value, static_cast<const b3CompoundData*>(geometry.m_data)))};
    }
    ...
    {
        if (!IsRigidTransform(transform.m_localTransform)
            || !IsValidGeometryScale(transform.m_scale)
            || !IsValidGeometryScale(transform.m_postScale))
        {
            return AZStd::monostate{};
        ...
        return AZStd::visit(
            [&transform, materials](const auto& shapeGeometry) -> NativeGeometry
            {
                ...
                {
                    if (!AZ::IsFiniteFloat(shapeGeometry.m_height)
                        || shapeGeometry.m_height <= 0.0f
                        || !AZ::IsFiniteFloat(shapeGeometry.m_radius)
                        || shapeGeometry.m_radius <= 0.0f)
                    {
                        return AZStd::monostate{};
                    }
                    float scale = 0.0f;
                ...
                else if constexpr (AZStd::is_same_v<Geometry, CylinderShapeConfiguration>)
                {
                    ...
                    return AZStd::shared_ptr<const HullGeometry>(
                        CreateCylinderGeometry(
                            shapeGeometry.m_height,
                            shapeGeometry.m_radius,
                            aznumeric_cast<AZ::u8>(shapeGeometry.m_sideCount),
                            transform));
                }
                ...
                else if constexpr (AZStd::is_same_v<Geometry, TriangleMeshShapeConfiguration>)
                {
                    ...
                    return AZStd::shared_ptr<const MeshGeometry>(
                        CreateMeshGeometry(
                            transformedVertices,
                            transformedIndices,
                ...
                {
                    if (!transform.m_localTransform.GetTranslation().IsZero()
                        || !transform.m_localTransform.GetRotation().IsIdentity()
                        || !IsValidGeometryScale(transform.m_scale, false)
                        || !IsValidGeometryScale(transform.m_postScale, false))
                    {
                        return AZStd::monostate{};
                    }
                    ...
                    {
                        if (shapeGeometry.m_columnCount < 2
                            || shapeGeometry.m_rowCount < 2
                    ...
                    float quantizationMinimum = *minimumHeight;
                    float quantizationMaximum = *maximumHeight;
                    if (shapeGeometry.m_useSharedHeightRange)
                    {
                        quantizationMinimum = shapeGeometry.m_minimumHeight;
                        quantizationMaximum = shapeGeometry.m_maximumHeight;
                    }
                    if (!AZ::IsFiniteFloat(quantizationMinimum)
                        || !AZ::IsFiniteFloat(quantizationMaximum)
                        || quantizationMinimum > *minimumHeight
                        || quantizationMaximum < *maximumHeight
                        || quantizationMinimum > quantizationMaximum)
                    {
                        return AZStd::monostate{};
                    }

                    return AZStd::shared_ptr<const HeightFieldGeometry>(
                ...
                else if constexpr (AZStd::is_same_v<Geometry, CompoundShapeConfiguration>)
                {
                    ...
                    for (const CompoundChildShapeConfiguration& child : shapeGeometry.m_children)
                    {
                        ...
                        if (!materials.empty())
                        {
                            material = materials[child.m_materialIndex];
                        }
                        ...
                        const bool created = AZStd::visit(
                            [&](const auto& childGeometry)
                            {
                                ...
                                {
                                    if (!isSimilarity
                                        || !AZ::IsFiniteFloat(childGeometry.m_height)
                                        || childGeometry.m_height <= 0.0f
                                        || !AZ::IsFiniteFloat(childGeometry.m_radius)
                                        || childGeometry.m_radius <= 0.0f)
                                    {
                                        return false;
                                    }
                                ...
                                {
                                    if (childGeometry.m_sideCount < 3
                                        || childGeometry.m_sideCount > 32
                                        || !AZ::IsFiniteFloat(childGeometry.m_height)
                                        || childGeometry.m_height <= 0.0f
                                        || !AZ::IsFiniteFloat(childGeometry.m_radius)
                                        || childGeometry.m_radius <= 0.0f)
                    ...
                    return AZStd::shared_ptr<const CompoundGeometry>(CreateCompoundGeometry({spheres, capsules, hulls, meshes, materials}));
                }
            },
            geometry);
    }
        ...
        return AZStd::visit(
            [bodyId, &configuration](const auto& nativeGeometry) -> ShapeId
            {
                ...
                else if constexpr (AZStd::is_same_v<Geometry, AZStd::shared_ptr<const HullGeometry>>)
                {
                    if (nativeGeometry)
                    {
                        return CreateHullShape(bodyId, configuration, *nativeGeometry);
                    }
                ...
                else if constexpr (AZStd::is_same_v<Geometry, AZStd::shared_ptr<const MeshGeometry>>)
                {
                    if (nativeGeometry)
                    {
                        return CreateMeshShape(bodyId, configuration, *nativeGeometry);
                    }
                ...
                else if constexpr (AZStd::is_same_v<Geometry, AZStd::shared_ptr<const HeightFieldGeometry>>)
                {
                    if (nativeGeometry)
                    {
                        return CreateHeightFieldShape(bodyId, configuration, *nativeGeometry);
                    }
                ...
                else if constexpr (AZStd::is_same_v<Geometry, AZStd::shared_ptr<const CompoundGeometry>>)
                {
                    if (nativeGeometry)
                    {
                        return CreateCompoundShape(bodyId, configuration, *nativeGeometry);
                    }
        ...
        const NativeGeometry& geometry)
    {
        ...
        return AZStd::visit(
            [shapeId, shapeType = state.m_type](const auto& nativeGeometry)
            {
                ...
                {
                    return shapeType == NativeShapeType::Hull && nativeGeometry && SetHullShape(shapeId, *nativeGeometry);
                }
                else if constexpr (AZStd::is_same_v<Geometry, AZStd::shared_ptr<const MeshGeometry>>)
                {
                    return shapeType == NativeShapeType::Mesh && nativeGeometry && SetMeshShape(shapeId, *nativeGeometry);
                }
                else
                {
                    return false;
        ...
        return AZStd::visit(
            [](const auto& nativeGeometry)
            {
                ...
                else if constexpr (AZStd::is_same_v<Geometry, AZStd::shared_ptr<const HullGeometry>>)
                {
                    if (nativeGeometry)
                    {
                        return FromNative(b3ComputeHullAABB(GeometryCastAccess::Get(*nativeGeometry), b3Transform_identity));
                    }
                ...
                else if constexpr (AZStd::is_same_v<Geometry, AZStd::shared_ptr<const HeightFieldGeometry>>)
                {
                    if (nativeGeometry)
                    {
                        return FromNative(b3ComputeHeightFieldAABB(GeometryCastAccess::Get(*nativeGeometry), b3Transform_identity));
                    }
        ...
        return AZStd::visit(
            [&start, &translation, &hit](const auto& nativeGeometry)
            {
                ...
                else
                {
                    if (nativeGeometry)
                    {
                        return CastRay(*nativeGeometry, start, translation, hit);
                    }
    ...
    size_t GetShapeMaterialCount(
        const ShapeId shapeId)
    {
        if (IsValid(shapeId))
        {
            return aznumeric_cast<size_t>(b3Shape_GetMeshMaterialCount(b3LoadShapeId(shapeId.m_value)));
        }
        ...
        const bool wake)
    {
        if (!IsValid(shapeId)
            || !velocity.IsFinite()
            || !AZ::IsFiniteFloat(drag)
            || drag < 0.0f
            || !AZ::IsFiniteFloat(lift)
            || !AZ::IsFiniteFloat(maximumSpeed)
            || maximumSpeed < 0.0f)
        {
            return false;
        }
