...
namespace Box3D
{
    ...
    ColliderComponent::ColliderComponent(
        AZStd::vector<ShapeConfiguration> shapeConfigurations)
        ...
        AZ::ReflectContext* context)
    {
        ...
        {
            serializeContext->Class<ColliderComponent, AZ::Component>()->Field("Shapes", &ColliderComponent::m_shapeConfigurations);
    ...
    bool ColliderComponent::UpdateShape(
        const size_t index,
        const ShapeConfiguration& configuration)
    {
    ...
    bool ColliderComponent::SetCollisionFilter(
        const size_t index,
        const CollisionFilter& collisionFilter)
    {
        ...
        if (m_system && !m_system->SetShapeCollisionFilter(m_worldHandle, m_shapeHandles[index], collisionFilter))
        {
            return false;
        }
    ...
    bool ColliderComponent::SetMaterials(
        const size_t index,
        AZStd::span<const MaterialHandle> materials)
    {
        ...
        if (m_system && !m_system->SetShapeMaterials(m_worldHandle, m_shapeHandles[index], materials))
        {
            return false;
        }
        if (m_system)
        ...
        const float uniformScale)
    {
        ...
        if (AZ::IsClose(uniformScale, m_uniformScale, AZ::Constants::Tolerance) || !m_system)
        {
            m_uniformScale = uniformScale;
            return true;
        }
