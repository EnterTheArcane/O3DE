...
namespace Box3D
{
    ...
    namespace
    {
        ...
        NativeBodyType ToNativeBodyType(
        ...
        BodyType FromNativeBodyType(
            const NativeBodyType bodyType)
        {
            switch (bodyType)
            {
        ...
        QueryBodyTypes GetQueryBodyType(
            const NativeBodyType bodyType)
        {
            switch (bodyType)
            {
        ...
        bool IncludesBodyType(
            const QueryBodyTypes bodyTypes,
            const NativeBodyType bodyType)
        {
            ...
            const AZ::Transform& transform)
        {
            ...
            return transform.GetTranslation().IsFinite()
                && IsNormalized(transform.GetRotation())
        ...
        bool IsValidRigidBodyConfiguration(
            const RigidBodyConfiguration& configuration)
        {
            return IsValidTransform(configuration.m_transform)
                && HasUnitScale(configuration.m_transform)
                && configuration.m_linearVelocity.IsFinite()
                && configuration.m_angularVelocity.IsFinite()
                && IsNonNegative(configuration.m_linearDamping)
                && IsNonNegative(configuration.m_angularDamping)
                && AZ::IsFiniteFloat(configuration.m_gravityScale)
                && IsNonNegative(configuration.m_sleepThreshold)
                && IsValidBodyType(configuration.m_bodyType);
        }

        bool IsValidCharacterConfiguration(
            const CharacterConfiguration& configuration)
        {
            return configuration.m_basePosition.IsFinite()
                && IsNormalized(configuration.m_rotation)
                && configuration.m_upDirection.IsFinite()
                && !configuration.m_upDirection.IsZero()
                && AZ::IsFiniteFloat(configuration.m_height)
                && AZ::IsFiniteFloat(configuration.m_radius)
                && configuration.m_radius > 0.0f
                && configuration.m_height >= 2.0f * configuration.m_radius
                && IsNonNegative(configuration.m_maximumSlopeAngle)
                && configuration.m_maximumSlopeAngle <= 90.0f
                && IsNonNegative(configuration.m_stepHeight)
                && IsNonNegative(configuration.m_minimumMovementDistance)
                && IsNonNegative(configuration.m_maximumSpeed)
                && IsNonNegative(configuration.m_groundStickDistance)
                && IsNonNegative(configuration.m_interactionScale)
        ...
        bool IsValidJointConfiguration(
            const JointConfiguration& configuration)
        {
            const JointCommonConfiguration& common = GetCommonJointConfiguration(configuration);
            if (!common.m_parentBody
                || !common.m_childBody
                || common.m_parentBody == common.m_childBody
                || !IsValidTransform(common.m_parentLocalFrame)
                || !HasUnitScale(common.m_parentLocalFrame)
                || !IsValidTransform(common.m_childLocalFrame)
                || !HasUnitScale(common.m_childLocalFrame)
                || !IsNonNegative(common.m_forceThreshold)
                || !IsNonNegative(common.m_torqueThreshold)
                || !IsNonNegative(common.m_constraintHertz)
                || !IsNonNegative(common.m_constraintDampingRatio)
            ...
            return AZStd::visit(
                [](const auto& typedConfiguration)
                {
                    ...
                    {
                        return IsNonNegative(typedConfiguration.m_hertz)
                            && IsNonNegative(typedConfiguration.m_dampingRatio)
                            && IsNonNegative(typedConfiguration.m_maxTorque);
                    }
                    else if constexpr (AZStd::is_same_v<Configuration, DistanceJointConfiguration>)
                    {
                        return AZ::IsFiniteFloat(typedConfiguration.m_length)
                            && typedConfiguration.m_length > 0.0f
                            ...
                            && IsNonNegative(typedConfiguration.m_hertz)
                            && IsNonNegative(typedConfiguration.m_dampingRatio)
                            && IsNonNegative(typedConfiguration.m_minLength)
                            && IsNonNegative(typedConfiguration.m_maxLength)
                            && typedConfiguration.m_minLength <= typedConfiguration.m_maxLength
                            && IsNonNegative(typedConfiguration.m_maxMotorForce)
                            && AZ::IsFiniteFloat(typedConfiguration.m_motorSpeed);
                    }
                    ...
                    {
                        return typedConfiguration.m_linearVelocity.IsFinite()
                            && typedConfiguration.m_angularVelocity.IsFinite()
                            && IsNonNegative(typedConfiguration.m_maxVelocityForce)
                            && IsNonNegative(typedConfiguration.m_maxVelocityTorque)
                            && IsNonNegative(typedConfiguration.m_linearHertz)
                            && IsNonNegative(typedConfiguration.m_linearDampingRatio)
                            && IsNonNegative(typedConfiguration.m_maxSpringForce)
                            && IsNonNegative(typedConfiguration.m_angularHertz)
                            && IsNonNegative(typedConfiguration.m_angularDampingRatio)
                            && IsNonNegative(typedConfiguration.m_maxSpringTorque);
                    }
                    ...
                    {
                        return AZ::IsFiniteFloat(typedConfiguration.m_targetAngle)
                            && IsNonNegative(typedConfiguration.m_hertz)
                            && IsNonNegative(typedConfiguration.m_dampingRatio)
                            && AZ::IsFiniteFloat(typedConfiguration.m_lowerAngle)
                    ...
                    {
                        return IsNormalized(typedConfiguration.m_targetRotation)
                            && typedConfiguration.m_motorVelocity.IsFinite()
                            && IsNonNegative(typedConfiguration.m_hertz)
                            && IsNonNegative(typedConfiguration.m_dampingRatio)
                    ...
                    {
                        return IsNonNegative(typedConfiguration.m_linearHertz)
                            && IsNonNegative(typedConfiguration.m_angularHertz)
                            && IsNonNegative(typedConfiguration.m_linearDampingRatio)
                            && IsNonNegative(typedConfiguration.m_angularDampingRatio);
                    }
                    else if constexpr (AZStd::is_same_v<Configuration, WheelJointConfiguration>)
                    {
                        return IsNonNegative(typedConfiguration.m_suspensionHertz)
                            ...
                            && IsNonNegative(typedConfiguration.m_maxSpinTorque)
                            && AZ::IsFiniteFloat(typedConfiguration.m_spinSpeed)
                            && IsNonNegative(typedConfiguration.m_steeringHertz)
                            && IsNonNegative(typedConfiguration.m_steeringDampingRatio)
        ...
        struct QueryProxy final
        {
            ...
            [[nodiscard]]
            AZStd::span<const AZ::Vector3> GetPoints() const
            {
                if (m_overflowPoints.empty())
                {
            ...
            return AZStd::visit(
                [&transform, &proxy](const auto& configuration)
                {
                    ...
                    else if constexpr (AZStd::is_same_v<Configuration, CapsuleShapeConfiguration>)
                    {
                        ...
                        proxy.AddPoint(-axis);
                        proxy.AddPoint(axis);
                        return AZ::IsFiniteFloat(configuration.m_height)
                            && configuration.m_height > 0.0f
                            && AZ::IsFiniteFloat(proxy.m_radius)
                            && proxy.m_radius > 0.0f;
                    }
                    ...
                    {
                        if (configuration.m_sideCount < 3
                            || configuration.m_sideCount > 32
                            || !AZ::IsFiniteFloat(configuration.m_height)
                            || configuration.m_height <= 0.0f
                            || !AZ::IsFiniteFloat(configuration.m_radius)
                            || configuration.m_radius <= 0.0f)
        ...
        const JointCommonConfiguration& GetCommonJointConfiguration(
            const JointConfiguration& configuration)
        {
            return AZStd::visit(
        ...
        , m_worldIndex(worldIndex)
    {
        ...
        nativeConfiguration.m_taskContext = nullptr;
        if (configuration.m_jobContext)
        {
            nativeConfiguration.m_taskContext = &m_nativeTaskContext;
        }
    ...
    void World::StepAutomatically(
        const float deltaTime,
        const AZ::u32 subStepCount)
    {
        AZStd::lock_guard lock(m_mutex);
        if (!m_configuration.m_autoSimulate
            || !m_configuration.m_enabled
            || !AZ::IsFiniteFloat(deltaTime)
            || deltaTime <= 0.0f
            || !AZ::IsFiniteFloat(m_configuration.m_fixedTimeStep)
            || m_configuration.m_fixedTimeStep <= 0.0f)
        {
            return;
        }
        ...
        const RigidBodyConfiguration& configuration)
    {
        ...
        nativeConfiguration.m_motionLocks = {
            .m_linearX = configuration.m_motionLocks.m_linearX,
            .m_linearY = configuration.m_motionLocks.m_linearY,
            .m_linearZ = configuration.m_motionLocks.m_linearZ,
            .m_angularX = configuration.m_motionLocks.m_angularX,
            .m_angularY = configuration.m_motionLocks.m_angularY,
            .m_angularZ = configuration.m_motionLocks.m_angularZ,
        };
        ...
        if (configuration.m_entityId.IsValid())
        {
            ++m_entityBodyCount;
        }
        ...
        const BodyHandle bodyHandle)
    {
        ...
        for (AZ::u32 jointIndex = 0; jointIndex < m_jointSlots.size(); ++jointIndex)
        {
            ...
            {
                if (jointSlot.m_entityId.IsValid())
                {
                    --m_entityJointCount;
                }
                jointSlot = JointSlot{};
        ...
        if (bodySlot->m_entityId.IsValid())
        {
            --m_entityBodyCount;
        }
        *bodySlot = BodySlot{};
        ...
        BodyState& state) const
    {
        ...
        Internal::WorldMemberHandleParts parts;
        if (!slot || !Internal::DecodeWorldMemberHandle(bodyHandle, parts))
        {
            return false;
        }
        ...
        const BodyHandle bodyHandle) const
    {
        ...
        if (slot && Internal::DecodeWorldMemberHandle(bodyHandle, parts))
        {
            return m_bodyNames[parts.m_index];
        }

        return {};
    }

    bool World::SetBodyName(
        const BodyHandle bodyHandle,
        AZ::Name name)
    {
        AZStd::lock_guard lock(m_mutex);
        ...
        BodyProperties& properties) const
    {
        ...
        properties.m_motionLocks = {
            .m_linearX = nativeLocks.m_linearX,
            .m_linearY = nativeLocks.m_linearY,
            .m_linearZ = nativeLocks.m_linearZ,
            .m_angularX = nativeLocks.m_angularX,
            .m_angularY = nativeLocks.m_angularY,
            .m_angularZ = nativeLocks.m_angularZ,
        };
        ...
        const BodyProperties& properties)
    {
        ...
        BodySlot* slot = FindBodySlot(bodyHandle);
        if (!slot
            || !IsValidBodyType(properties.m_bodyType)
            || !AZ::IsFiniteFloat(properties.m_linearDamping)
            || properties.m_linearDamping < 0.0f
            || !AZ::IsFiniteFloat(properties.m_angularDamping)
            || properties.m_angularDamping < 0.0f
            || !AZ::IsFiniteFloat(properties.m_gravityScale)
            || !AZ::IsFiniteFloat(properties.m_sleepThreshold)
        ...
        if (properties.m_bodyType != BodyType::Static)
        {
                 ...
                 shapeIndex = m_shapeSlots[shapeIndex].m_nextShapeIndex)
            {
                ...
                if (!Box3D::GetShapeState(m_shapeSlots[shapeIndex].m_nativeId, shapeState)
                    || shapeState.m_type == NativeShapeType::Compound
                    || shapeState.m_type == NativeShapeType::Heightfield)
                {
                    return false;
                }
            }
    ...
    bool World::SetBodyHitEventsEnabled(
        const BodyHandle bodyHandle,
        const bool enabled)
    {
        AZStd::lock_guard lock(m_mutex);
        ...
        const AZ::Transform& transform)
    {
        AZStd::lock_guard lock(m_mutex);
        BodySlot* slot = FindBodySlot(bodyHandle);
        if (!slot || !IsValidTransform(transform) || !HasUnitScale(transform))
        {
            return false;
        }
    ...
    AZ::Vector3 World::GetLinearVelocityAtLocalPoint(
        const BodyHandle bodyHandle,
        const AZ::Vector3& localPoint) const
    {
        ...
        if (slot && localPoint.IsFinite())
        {
            return Box3D::GetLinearVelocityAtLocalPoint(slot->m_nativeId, localPoint);
        }

        return AZ::Vector3::CreateZero();
    }

    AZ::Vector3 World::GetLinearVelocityAtWorldPoint(
        const BodyHandle bodyHandle,
        const AZ::Vector3& worldPoint) const
    {
        ...
        if (slot && worldPoint.IsFinite())
        {
            return Box3D::GetLinearVelocityAtWorldPoint(slot->m_nativeId, worldPoint);
        }

        return AZ::Vector3::CreateZero();
    }

    bool World::SetKinematicTarget(
        const BodyHandle bodyHandle,
        const AZ::Transform& transform,
        const float fixedTimeStep,
        const bool wake)
    {
        AZStd::lock_guard lock(m_mutex);
        BodySlot* slot = FindBodySlot(bodyHandle);
        if (!slot || !IsValidTransform(transform) || !HasUnitScale(transform) || !AZ::IsFiniteFloat(fixedTimeStep) || fixedTimeStep <= 0.0f)
        {
            return false;
        }
    ...
    bool World::ApplyLinearImpulseAtWorldPoint(
        const BodyHandle bodyHandle,
        const AZ::Vector3& impulse,
        const AZ::Vector3& worldPoint,
        const bool wake)
    {
        AZStd::lock_guard lock(m_mutex);
        BodySlot* slot = FindBodySlot(bodyHandle);
        if (!slot || !impulse.IsFinite() || !worldPoint.IsFinite())
        {
            return false;
        }
    ...
    bool World::ApplyAngularImpulse(
        const BodyHandle bodyHandle,
        const AZ::Vector3& impulse,
        const bool wake)
    {
    ...
    bool World::ApplyForceAtWorldPoint(
        const BodyHandle bodyHandle,
        const AZ::Vector3& force,
        const AZ::Vector3& worldPoint,
        const bool wake)
    {
        AZStd::lock_guard lock(m_mutex);
        BodySlot* slot = FindBodySlot(bodyHandle);
        if (!slot || !force.IsFinite() || !worldPoint.IsFinite())
        {
            return false;
        }
        ...
        const MassProperties& properties)
    {
        ...
        BodySlot* slot = FindBodySlot(bodyHandle);
        if (!slot
            || !properties.m_inertia.IsFinite()
            || !properties.m_center.IsFinite()
            || !AZ::IsFiniteFloat(properties.m_mass)
            || properties.m_mass < 0.0f)
        {
            return false;
        }
    ...
    bool World::RecomputeMassFromShapes(
        const BodyHandle bodyHandle)
    {
        AZStd::lock_guard lock(m_mutex);
        BodySlot* slot = FindBodySlot(bodyHandle);
    ...
    AZ::Matrix3x3 World::GetWorldInverseInertia(
        const BodyHandle bodyHandle) const
    {
        AZStd::lock_guard lock(m_mutex);
        const BodySlot* slot = FindBodySlot(bodyHandle);
        if (slot)
        {
            return Box3D::GetWorldInverseInertia(slot->m_nativeId);
        }

        return AZ::Matrix3x3::CreateZero();
    }

    AZ::Vector3 World::GetWorldCenterOfMass(
        const BodyHandle bodyHandle) const
    {
        AZStd::lock_guard lock(m_mutex);
        const BodySlot* slot = FindBodySlot(bodyHandle);
        if (slot)
        {
            return Box3D::GetWorldCenterOfMass(slot->m_nativeId);
        }

        return AZ::Vector3::CreateZero();
    }

    bool World::GetBodyClosestPoint(
        const BodyHandle bodyHandle,
        ...
        float& distance) const
    {
        AZStd::lock_guard lock(m_mutex);
        const BodySlot* slot = FindBodySlot(bodyHandle);
        return slot && target.IsFinite() && Box3D::GetBodyClosestPoint(slot->m_nativeId, target, position, distance);
    }

    AZ::Aabb World::GetBodyAabb(
        const BodyHandle bodyHandle) const
    {
        AZStd::lock_guard lock(m_mutex);
        const BodySlot* slot = FindBodySlot(bodyHandle);
        if (slot)
        {
            return Box3D::GetBodyAabb(slot->m_nativeId);
        }

        return AZ::Aabb::CreateNull();
    }

    BufferResult World::GetBodyShapes(
        const BodyHandle bodyHandle,
    ...
    BufferResult World::GetBodyJoints(
        const BodyHandle bodyHandle,
        AZStd::span<JointHandle> jointHandles) const
    {
        ...
        BufferResult result;
        const BodySlot* bodySlot = FindBodySlot(bodyHandle);
        if (!bodySlot || !Box3D::GetBodyJoints(bodySlot->m_nativeId, m_nativeBodyJoints))
        {
            return result;
        }
    ...
    BufferResult World::GetBodySensorOverlaps(
        const BodyHandle bodyHandle,
        const AZStd::span<SensorOverlap> overlaps) const
    {
        AZStd::lock_guard lock(m_mutex);
        const BodySlot* bodySlot = FindBodySlot(bodyHandle);
        if (!bodySlot || !Box3D::GetSensorOverlaps(bodySlot->m_nativeId, m_nativeSensorScratch, m_nativeSensorOverlaps))
        {
            return {};
        }
        ...
        QueryHit& hit) const
    {
        ...
        if (!bodySlot
            ...
            || !request.m_direction.IsFinite()
            || request.m_direction.IsZero()
            || !AZ::IsFiniteFloat(request.m_distance)
        ...
        QueryHit& hit) const
    {
        ...
        QueryProxy proxy;
        if (!bodySlot
            || !request.m_translation.IsFinite()
            || request.m_translation.IsZero()
        ...
        const BodyOverlapRequest& request) const
    {
        ...
        QueryProxy proxy;
        return bodySlot
            && BuildQueryProxy(request.m_geometry, request.m_transform, proxy)
            && Box3D::OverlapShape(
                bodySlot->m_nativeId,
    ...
    ShapeHandle World::CreateShapeFromCooked(
        const BodyHandle bodyHandle,
        ...
        const float uniformScale)
    {
        ...
        BodySlot* bodySlot = FindBodySlot(bodyHandle);
        if (!bodySlot
            || !Internal::DecodeWorldMemberHandle(bodyHandle, bodyParts)
        ...
        float uniformScale)
    {
        ...
        if (!shapeSlot
            || !Internal::DecodeWorldMemberHandle(shapeHandle, shapeParts)
            || !AZ::IsFiniteFloat(uniformScale)
            || uniformScale <= 0.0f)
        {
            return false;
        ...
        const bool updateBodyMass)
    {
        ...
        Internal::WorldMemberHandleParts shapeParts;
        ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        if (!shapeSlot || !Internal::DecodeWorldMemberHandle(shapeHandle, shapeParts))
        {
            return false;
        }
    ...
    bool World::SetShapeCollisionFilter(
        const ShapeHandle shapeHandle,
        ...
        AZStd::span<const MaterialHandle> materials)
    {
        ...
        Internal::WorldMemberHandleParts parts;
        ShapeSlot* slot = FindShapeSlot(shapeHandle);
        if (!slot || materials.empty() || !Internal::DecodeWorldMemberHandle(shapeHandle, parts))
        {
            return false;
        }
        ...
        if (state.m_type == NativeShapeType::Compound || m_recording)
        {
        ...
        const ShapeHandle shapeHandle) const
    {
        ...
        if (slot)
        {
            return Box3D::GetShapeAabb(slot->m_nativeId);
        }

        return AZ::Aabb::CreateNull();
    }

    bool World::GetShapeState(
        const ShapeHandle shapeHandle,
        ShapeState& state) const
    {
        ...
        if (!shapeSlot || !Box3D::GetShapeState(shapeSlot->m_nativeId, nativeState))
        {
            return false;
        }
    ...
    BufferResult World::GetShapeMaterials(
        const ShapeHandle shapeHandle,
        AZStd::span<MaterialHandle> materialHandles) const
    {
        ...
        Internal::WorldMemberHandleParts parts;
        const ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        if (!shapeSlot || !Internal::DecodeWorldMemberHandle(shapeHandle, parts))
        {
            return {};
        }
        ...
        const bool updateBodyMass)
    {
        AZStd::lock_guard lock(m_mutex);
        ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        return shapeSlot && Box3D::SetShapeDensity(shapeSlot->m_nativeId, density, updateBodyMass);
    }

    bool World::SetShapeFriction(
        const ShapeHandle shapeHandle,
        const float friction)
    {
        ...
        Internal::WorldMemberHandleParts parts;
        ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        if (!shapeSlot || !Internal::DecodeWorldMemberHandle(shapeHandle, parts) || !AZ::IsFiniteFloat(friction) || friction < 0.0f)
        {
            return false;
        }
        NativeShapeState state;
        ...
        const float restitution)
    {
        ...
        Internal::WorldMemberHandleParts parts;
        ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        if (!shapeSlot || !Internal::DecodeWorldMemberHandle(shapeHandle, parts) || !AZ::IsFiniteFloat(restitution) || restitution < 0.0f)
        {
            return false;
        }
        NativeShapeState state;
    ...
    bool World::GetShapeMassProperties(
        const ShapeHandle shapeHandle,
        MassProperties& properties) const
    {
        AZStd::lock_guard lock(m_mutex);
    ...
    bool World::GetShapeClosestPoint(
        const ShapeHandle shapeHandle,
        const AZ::Vector3& target,
        AZ::Vector3& position,
        float& distance) const
    {
        AZStd::lock_guard lock(m_mutex);
        const ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        if (!shapeSlot || !Box3D::GetShapeClosestPoint(shapeSlot->m_nativeId, target, position))
        {
            return false;
        }
        ...
        QueryHit& hit) const
    {
        ...
        const ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        if (!shapeSlot
            || !Internal::DecodeWorldMemberHandle(shapeHandle, parts)
            || !start.IsFinite()
            || !direction.IsFinite()
            || direction.IsZero()
        ...
        hit.m_materialHandle = Internal::HandleAccess::Create<MaterialHandle>(nativeHit.m_userMaterialId);
        const AZStd::vector<MaterialHandle>& materials = m_shapeResources[parts.m_index].m_materials;
        if (!hit.m_materialHandle.IsValid()
            && nativeHit.m_materialIndex >= 0
    ...
    BufferResult World::GetShapeSensorOverlaps(
        const ShapeHandle shapeHandle,
        const AZStd::span<SensorOverlap> overlaps) const
    {
        AZStd::lock_guard lock(m_mutex);
        const ShapeSlot* shapeSlot = FindShapeSlot(shapeHandle);
        if (!shapeSlot || !Box3D::GetSensorOverlaps(shapeSlot->m_nativeId, m_nativeSensorScratch, m_nativeSensorOverlaps))
        {
            return {};
        }
        return CopySensorOverlaps(m_nativeSensorOverlaps, overlaps);
        ...
        NativeJointConfiguration& nativeConfiguration) const
    {
        ...
        nativeConfiguration = {};
        nativeConfiguration.m_bodyA = {};
        if (parentBody)
        {
            nativeConfiguration.m_bodyA = parentBody->m_nativeId;
        }
        ...
        AZStd::visit(
            [&nativeConfiguration](const auto& typedConfiguration)
            {
                ...
                if constexpr (AZStd::is_same_v<Configuration, ParallelJointConfiguration>)
                {
                    ...
                    nativeConfiguration.m_parallel = {
                        .m_hertz = typedConfiguration.m_hertz,
                        .m_dampingRatio = typedConfiguration.m_dampingRatio,
                        .m_maxTorque = typedConfiguration.m_maxTorque,
                    };
                }
                else if constexpr (AZStd::is_same_v<Configuration, DistanceJointConfiguration>)
                {
                    nativeConfiguration.m_kind = JointKind::Distance;
                    nativeConfiguration.m_distance = {
                        .m_length = typedConfiguration.m_length,
                        .m_lowerSpringForce = typedConfiguration.m_lowerSpringForce,
                        .m_upperSpringForce = typedConfiguration.m_upperSpringForce,
                        .m_hertz = typedConfiguration.m_hertz,
                        .m_dampingRatio = typedConfiguration.m_dampingRatio,
                        .m_minLength = typedConfiguration.m_minLength,
                        .m_maxLength = typedConfiguration.m_maxLength,
                        .m_maxMotorForce = typedConfiguration.m_maxMotorForce,
                        .m_motorSpeed = typedConfiguration.m_motorSpeed,
                        .m_enableSpring = typedConfiguration.m_enableSpring,
                        .m_enableLimit = typedConfiguration.m_enableLimit,
                        .m_enableMotor = typedConfiguration.m_enableMotor,
                    };
                }
                else if constexpr (AZStd::is_same_v<Configuration, FilterJointConfiguration>)
                {
                ...
                else if constexpr (AZStd::is_same_v<Configuration, MotorJointConfiguration>)
                {
                    ...
                    nativeConfiguration.m_motor = {
                        .m_linearVelocity = typedConfiguration.m_linearVelocity,
                        .m_angularVelocity = typedConfiguration.m_angularVelocity,
                        .m_maxVelocityForce = typedConfiguration.m_maxVelocityForce,
                        .m_maxVelocityTorque = typedConfiguration.m_maxVelocityTorque,
                        .m_linearHertz = typedConfiguration.m_linearHertz,
                        .m_linearDampingRatio = typedConfiguration.m_linearDampingRatio,
                        .m_maxSpringForce = typedConfiguration.m_maxSpringForce,
                        .m_angularHertz = typedConfiguration.m_angularHertz,
                        .m_angularDampingRatio = typedConfiguration.m_angularDampingRatio,
                        .m_maxSpringTorque = typedConfiguration.m_maxSpringTorque,
                    };
                }
                else if constexpr (AZStd::is_same_v<Configuration, PrismaticJointConfiguration>)
                {
                    nativeConfiguration.m_kind = JointKind::Prismatic;
                    nativeConfiguration.m_prismatic = {
                        .m_hertz = typedConfiguration.m_hertz,
                        .m_dampingRatio = typedConfiguration.m_dampingRatio,
                        .m_targetTranslation = typedConfiguration.m_targetTranslation,
                        .m_lowerTranslation = typedConfiguration.m_lowerTranslation,
                        .m_upperTranslation = typedConfiguration.m_upperTranslation,
                        .m_maxMotorForce = typedConfiguration.m_maxMotorForce,
                        .m_motorSpeed = typedConfiguration.m_motorSpeed,
                        .m_enableSpring = typedConfiguration.m_enableSpring,
                        .m_enableLimit = typedConfiguration.m_enableLimit,
                ...
                else if constexpr (AZStd::is_same_v<Configuration, RevoluteJointConfiguration>)
                {
                    ...
                    nativeConfiguration.m_revolute = {
                        .m_targetAngle = typedConfiguration.m_targetAngle,
                        .m_hertz = typedConfiguration.m_hertz,
                        .m_dampingRatio = typedConfiguration.m_dampingRatio,
                        .m_lowerAngle = typedConfiguration.m_lowerAngle,
                        .m_upperAngle = typedConfiguration.m_upperAngle,
                        .m_maxMotorTorque = typedConfiguration.m_maxMotorTorque,
                        .m_motorSpeed = typedConfiguration.m_motorSpeed,
                        .m_enableSpring = typedConfiguration.m_enableSpring,
                        .m_enableLimit = typedConfiguration.m_enableLimit,
                        .m_enableMotor = typedConfiguration.m_enableMotor,
                ...
                else if constexpr (AZStd::is_same_v<Configuration, SphericalJointConfiguration>)
                {
                    ...
                    nativeConfiguration.m_spherical = {
                        .m_targetRotation = typedConfiguration.m_targetRotation,
                        .m_motorVelocity = typedConfiguration.m_motorVelocity,
                        .m_hertz = typedConfiguration.m_hertz,
                        .m_dampingRatio = typedConfiguration.m_dampingRatio,
                        .m_coneAngle = typedConfiguration.m_coneAngle,
                        .m_lowerTwistAngle = typedConfiguration.m_lowerTwistAngle,
                        .m_upperTwistAngle = typedConfiguration.m_upperTwistAngle,
                        .m_maxMotorTorque = typedConfiguration.m_maxMotorTorque,
                        .m_enableSpring = typedConfiguration.m_enableSpring,
                        .m_enableConeLimit = typedConfiguration.m_enableConeLimit,
                        .m_enableTwistLimit = typedConfiguration.m_enableTwistLimit,
                        .m_enableMotor = typedConfiguration.m_enableMotor,
                    };
                }
                else if constexpr (AZStd::is_same_v<Configuration, WeldJointConfiguration>)
                ...
                else if constexpr (AZStd::is_same_v<Configuration, WheelJointConfiguration>)
                {
                    ...
                    nativeConfiguration.m_wheel = {
                        .m_suspensionHertz = typedConfiguration.m_suspensionHertz,
                        .m_suspensionDampingRatio = typedConfiguration.m_suspensionDampingRatio,
                        .m_lowerSuspensionLimit = typedConfiguration.m_lowerSuspensionLimit,
                        .m_upperSuspensionLimit = typedConfiguration.m_upperSuspensionLimit,
                        .m_maxSpinTorque = typedConfiguration.m_maxSpinTorque,
                        .m_spinSpeed = typedConfiguration.m_spinSpeed,
                        .m_steeringHertz = typedConfiguration.m_steeringHertz,
                        .m_steeringDampingRatio = typedConfiguration.m_steeringDampingRatio,
                        .m_targetSteeringAngle = typedConfiguration.m_targetSteeringAngle,
                        .m_maxSteeringTorque = typedConfiguration.m_maxSteeringTorque,
                        .m_lowerSteeringLimit = typedConfiguration.m_lowerSteeringLimit,
                        .m_upperSteeringLimit = typedConfiguration.m_upperSteeringLimit,
                        .m_enableSuspensionSpring = typedConfiguration.m_enableSuspensionSpring,
                        .m_enableSuspensionLimit = typedConfiguration.m_enableSuspensionLimit,
                        .m_enableSpinMotor = typedConfiguration.m_enableSpinMotor,
                        .m_enableSteering = typedConfiguration.m_enableSteering,
                        .m_enableSteeringLimit = typedConfiguration.m_enableSteeringLimit,
                    };
                }
            },
            configuration);
        ...
        const bool wakeAttachedBodies)
    {
        ...
        Internal::WorldMemberHandleParts parts;
        JointSlot* slot = FindJointSlot(jointHandle);
        if (!slot || !Internal::DecodeWorldMemberHandle(jointHandle, parts))
        {
            return false;
        }
    ...
    bool World::GetJointConfiguration(
        const JointHandle jointHandle,
    ...
    bool World::GetJointMeasurements(
        const JointHandle jointHandle,
    ...
    void World::EnsureCharacterScratchCapacity(
        size_t maximumPlaneCount)
    {
        ...
        const CharacterConfiguration& configuration)
    {
        ...
        if (!slot || !IsValidCharacterConfiguration(configuration))
        {
            return false;
        }
        EnsureCharacterScratchCapacity(configuration.m_maximumContactPlanes);
        ...
        const float fixedTimeStep)
    {
        ...
        AZ::u32 characterIndex = 0;
        CharacterSlot* slot = FindCharacterSlot(characterHandle, &characterIndex);
        if (!slot || !velocity.IsFinite() || !AZ::IsFiniteFloat(fixedTimeStep) || fixedTimeStep <= 0.0f)
        {
            return false;
        }
    ...
    bool World::MoveCharacterImmediately(
        ...
        float fixedTimeStep)
    {
        ...
        AZ::Vector3 clampedVelocity = velocity;
        if (velocity.GetLength() > configuration.m_maximumSpeed)
        {
            clampedVelocity = velocity.GetNormalized() * configuration.m_maximumSpeed;
        }
        ...
        for (const MoverPlane& plane : m_characterScratch->GetPlanes())
        {
            ...
            AZ::u32 bodyIndex = 0;
            ResolvedShape resolvedShape;
            if (!DecodeSlotIndex(plane.m_bodyUserData, bodyIndex)
            ...
            support.m_state = CharacterSupportState::Sliding;
            if (upDot >= walkableDot)
            {
                support.m_state = CharacterSupportState::Supported;
            }
    ...
    bool World::ApplyPendingCharacterMoves(
        const float fixedTimeStep,
        const bool consume)
    {
    ...
    bool World::GetCharacterConfiguration(
        const CharacterHandle characterHandle,
...
#endif
        AZStd::lock_guard lock(m_mutex);
        if (CanUseClosestRaycast(request.m_filter))
        {
            return RaycastClosestDefaultUnlocked(request, hit);
        }

        return RaycastClosestFilteredUnlocked(request, hit);
    }

    BufferResult World::RaycastClosestBatch(
        AZStd::span<const RaycastRequest> requests,
        AZStd::span<ClosestQueryResult> results) const
    {
        ...
        const bool canRunConcurrently = !m_recording
        ...
        if (!canRunConcurrently)
        {
            for (size_t requestIndex = 0; requestIndex < requestCount; ++requestIndex)
            {
                ...
                if (CanUseClosestRaycast(requests[requestIndex].m_filter))
                {
                    results[requestIndex].m_found = RaycastClosestDefaultUnlocked(requests[requestIndex], results[requestIndex].m_hit);
                }
                else
                {
                    results[requestIndex].m_found = RaycastClosestFilteredUnlocked(requests[requestIndex], results[requestIndex].m_hit);
                }
            }
    ...
    bool World::CanUseClosestRaycast(
        const QueryFilter& filter) const
    {
        return !filter.m_callback && filter.m_bodyTypes == QueryBodyTypes::All && (filter.m_includeSensors || m_sensorShapeCount == 0);
    }

    bool World::RaycastClosestDefaultUnlocked(
        const RaycastRequest& request,
        QueryHit& hit) const
    {
        ...
        AZ::Vector3 direction = request.m_direction;
    ...
    bool World::RaycastClosestFilteredUnlocked(
        const RaycastRequest& request,
        QueryHit& hit) const
    {
        ...
        filteredRequest.m_filter.m_callback = [](const QueryHit& candidate, void* userData)
        {
            ...
            return candidate.m_fraction > 0.0f && (!context.m_callback || context.m_callback(candidate, context.m_userData));
        };
    ...
    QueryResult World::Raycast(
        const RaycastRequest& request,
        AZStd::span<QueryHit> hits) const
    {
        ...
        const auto callback = [](const CastHit& nativeHit, void* opaqueContext)
        {
            ...
            queryHit.m_childIndex = -1;
            if (shapeSlot.m_isCompound)
            {
                queryHit.m_childIndex = nativeHit.m_childIndex;
            }
            if (queryContext.m_filter.m_callback && !queryContext.m_filter.m_callback(queryHit, queryContext.m_filter.m_userData))
            {
                return 1.0f;
            }
        ...
        AZStd::span<QueryHit> hits) const
    {
        ...
        QueryProxy proxy;
        if (!request.m_translation.IsFinite()
            || request.m_translation.IsZero()
            || !BuildQueryProxy(request.m_geometry, request.m_start, proxy))
        {
            return {};
        ...
        const auto callback = [](const CastHit& nativeHit, void* opaqueContext)
        {
            ...
            if (!bodySlot || (!filter.m_includeSensors && shapeSlot.m_isSensor) || !IncludesBodyType(filter.m_bodyTypes, bodySlot->m_type))
            {
                return 1.0f;
            }
            QueryHit queryHit;
            ...
            if (filter.m_callback && !filter.m_callback(queryHit, filter.m_userData))
            {
                return 1.0f;
            }
            ++queryContext.m_requiredHitCount;
        ...
        AZStd::span<Hit> hits) const
    {
        ...
        const auto callback = [](const OverlapCandidate& candidate, void* opaqueContext)
        {
            ...
            if (queryContext.m_filterBodyTypes)
            {
                ...
                if (!bodySlot || !IncludesBodyType(queryContext.m_filter.m_bodyTypes, bodySlot->m_type))
                {
                    return true;
                }
            }
            ...
            if (queryContext.m_filter.m_callback)
            {
                QueryHit filterHit;
    ...
    template<typename Hit>
    QueryResult World::OverlapAabbImpl(
    ...
    {
        AZStd::lock_guard lock(m_mutex);
        if (m_recording && (collisionFilterCallback || preSolveCallback))
        {
            return false;
        }
        Box3D::SetContactCallbacks(m_nativeId, this, collisionFilterCallback, preSolveCallback);
    ...
    bool World::StartRecording(
        const size_t initialCapacityBytes)
    {
        AZStd::lock_guard lock(m_mutex);
        AZ_PROFILE_SCOPE(Physics, "Box3D::World::StartRecording");
        if (m_recording
            || m_collisionFilterCallback
            || m_preSolveCallback
            || m_systemConfiguration.m_frictionCallback
            || m_systemConfiguration.m_restitutionCallback)
        {
            return false;
        }
        m_recording = Box3D::CreateRecording(initialCapacityBytes);
        if (m_recording && Box3D::StartRecording(m_nativeId, *m_recording))
        {
            return true;
        }
        m_recording.reset();
        ...
        const BodyHandle bodyHandle) const
    {
        ...
        if (slot.m_generation == parts.m_generation && Box3D::IsValid(slot.m_nativeId))
        {
            return &slot;
        }
    ...
    bool World::ResolveShape(
        void* userData,
        const ShapeId nativeId,
    ...
    void World::RegisterShape(
        const ShapeId nativeId,
        const AZ::u32 shapeIndex)
    {
        const AZ::u32 nativeIndex = nativeId.GetIndex();
    ...
    World::JointSlot* World::FindJointSlot(
        const JointHandle jointHandle)
    {
        return const_cast<JointSlot*>(static_cast<const World&>(*this).FindJointSlot(jointHandle));
    }

    const World::JointSlot* World::FindJointSlot(
        const JointHandle jointHandle) const
    {
        Internal::WorldMemberHandleParts parts;
        if (!Internal::DecodeWorldMemberHandle(jointHandle, parts)
            || parts.m_worldIndex != m_worldIndex
    ...
    World::CharacterSlot* World::FindCharacterSlot(
        const CharacterHandle characterHandle,
        AZ::u32* characterIndex)
    {
        return const_cast<CharacterSlot*>(static_cast<const World&>(*this).FindCharacterSlot(characterHandle, characterIndex));
    }

    const World::CharacterSlot* World::FindCharacterSlot(
        const CharacterHandle characterHandle,
        AZ::u32* characterIndex) const
    {
        Internal::WorldMemberHandleParts parts;
    ...
    ShapeId World::CreateNativeShape(
        ...
        float uniformScale)
    {
        if (!IsValidTransform(properties.m_localTransform)
            || !AZ::IsFiniteFloat(uniformScale)
            || uniformScale <= 0.0f
            || !IsNonNegative(properties.m_density)
            || !IsNonNegative(properties.m_explosionScale)
            || (!geometry) == (!cookedGeometry))
        {
            return {};
        }
        ...
        if (cookedGeometry)
        {
            resources.m_geometry = *cookedGeometry;
        }
        else
        {
            resources.m_geometry = CookGeometry(*geometry, GeometryTransform{properties.m_localTransform, uniformScale}, nativeMaterials);
        }
        shapeSlot.m_isCompound = AZStd::holds_alternative<AZStd::shared_ptr<const CompoundGeometry>>(resources.m_geometry);
    ...
    BodyHandle World::GetBodyHandle(
        const BodyMove& bodyMove) const
    {
        AZ::u32 bodyIndex = 0;
        ...
        const BodySlot& slot = m_bodySlots[bodyIndex];
        if (Box3D::IsValid(slot.m_nativeId) && slot.m_nativeId == bodyMove.m_bodyId)
        {
            return Internal::MakeWorldMemberHandle<BodyHandle>(m_worldIndex, bodyIndex, slot.m_generation);
        }
        ...
        const ShapeData& shapeData) const
    {
        ...
        const BodySlot& slot = m_bodySlots[bodyIndex];
        if (Box3D::IsValid(slot.m_nativeId) && slot.m_nativeId == shapeData.m_bodyId)
        {
            return Internal::MakeWorldMemberHandle<BodyHandle>(m_worldIndex, bodyIndex, slot.m_generation);
        }
    ...
    ShapeHandle World::GetShapeHandle(
        const ShapeData& shapeData) const
    {
        ResolvedShape resolvedShape;
        if (ResolveShape(shapeData.m_shapeUserData, shapeData.m_shapeId, resolvedShape))
        {
            return resolvedShape.m_shapeHandle;
        }

        return {};
    }

    bool World::ResolveShapeHandles(
        const ShapeId nativeId,
    ...
    ContactSnapshotResult World::CopyContactSnapshots(
        const AZStd::span<const ContactData> nativeContacts,
    ...
    BufferResult World::CopySensorOverlaps(
        const AZStd::span<const SensorOverlapData> nativeOverlaps,
        ...
        bool append)
    {
        ...
        if ((m_configuration.m_collectedEventTypes & StepEventTypes::BodyMoves) != StepEventTypes::None || m_entityBodyCount > 0)
        {
            nativeFlags |= StepEventFlags::BodyMoves;
        }
        if ((m_configuration.m_collectedEventTypes & StepEventTypes::Sensors) != StepEventTypes::None)
        {
            nativeFlags |= StepEventFlags::Sensors;
        }
        if ((m_configuration.m_collectedEventTypes & StepEventTypes::Contacts) != StepEventTypes::None)
        {
            nativeFlags |= StepEventFlags::Contacts;
        }
        if ((m_configuration.m_collectedEventTypes & StepEventTypes::ContactHits) != StepEventTypes::None)
        {
            nativeFlags |= StepEventFlags::ContactHits;
        }
        if ((m_configuration.m_collectedEventTypes & StepEventTypes::JointThresholds) != StepEventTypes::None || m_entityJointCount > 0)
        {
            nativeFlags |= StepEventFlags::Joints;
        }
        Box3D::GetStepEvents(
            m_nativeId,
            nativeFlags,
        ...
        for (const BodyMove& nativeEvent : m_nativeBodyMoves)
        {
            ...
            if (bodyHandle)
            {
                ...
                const BodySlot* bodySlot = FindBodySlot(bodyHandle);
                if (bodySlot && bodySlot->m_entityId.IsValid())
                {
        ...
        for (const SensorTransition& nativeEvent : m_nativeSensorEvents)
        {
            ...
            EventPhase phase = EventPhase::End;
            if (nativeEvent.m_type == SensorTransitionType::Begin)
            {
                phase = EventPhase::Begin;
            }
            m_sensorEvents.push_back({sensorBody, visitorBody, sensorShape, visitorShape, phase});
        }
        AZStd::sort(
        ...
        for (const NativeJointThresholdEvent& nativeEvent : m_nativeJointEvents)
        {
            ...
            if (Box3D::IsValid(slot.m_nativeId))
            {
                const JointThresholdEvent event{Internal::MakeWorldMemberHandle<JointHandle>(m_worldIndex, jointIndex, slot.m_generation)};
