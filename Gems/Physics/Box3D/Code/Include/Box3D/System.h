...
namespace Box3D
{
    ...
    using CollisionFilterCallback = bool (*)(
        ShapeHandle shapeA,
        ShapeHandle shapeB,
        void* userData);
    using PreSolveCallback = bool (*)(
        ShapeHandle shapeA,
        ShapeHandle shapeB,
    ...
    public:
        ...
        [[nodiscard]]
        virtual WorldHandle FindWorld(AZ::Name name) const = 0;

        [[nodiscard]]
        virtual bool GetWorldConfiguration(
            WorldHandle worldHandle,
            WorldConfiguration& configuration) const = 0;

        [[nodiscard]]
        virtual AZ::Aabb GetWorldAabb(WorldHandle worldHandle) const = 0;

        [[nodiscard]]
        virtual bool IsValid(WorldHandle worldHandle) const = 0;

        virtual bool SetWorldEnabled(
            WorldHandle worldHandle,
            bool enabled) = 0;

        [[nodiscard]]
        virtual bool IsWorldEnabled(WorldHandle worldHandle) const = 0;
        ...
        [[nodiscard]]
        virtual SimulationTick GetLastCompletedTick(WorldHandle worldHandle) const = 0;

        [[nodiscard]]
        virtual AZ::u64 GetStateDigest(WorldHandle worldHandle) const = 0;

        [[nodiscard]]
        virtual AZStd::string_view GetCompatibilityFingerprint() const = 0;
        ...
        virtual bool GetMaterial(
            MaterialHandle materialHandle,
            MaterialConfiguration& configuration) const = 0;
        ...
        virtual BodyHandle CreateBody(
            WorldHandle worldHandle,
            const RigidBodyConfiguration& configuration) = 0;
        ...
        [[nodiscard]]
        virtual AZ::Name GetBodyName(
            WorldHandle worldHandle,
            BodyHandle bodyHandle) const = 0;

        virtual bool SetBodyName(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            AZ::Name name) = 0;

        [[nodiscard]]
        virtual bool GetBodyProperties(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            BodyProperties& properties) const = 0;

        virtual bool SetBodyProperties(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const BodyProperties& properties) = 0;

        virtual bool SetBodyAwake(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            bool awake) = 0;

        virtual bool SetBodyEnabled(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            bool enabled) = 0;

        virtual bool SetBodyHitEventsEnabled(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            bool enabled) = 0;
        ...
        [[nodiscard]]
        virtual bool GetBodyLocalPoint(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& worldPoint,
            AZ::Vector3& localPoint) const = 0;

        [[nodiscard]]
        virtual bool GetBodyWorldPoint(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& localPoint,
            AZ::Vector3& worldPoint) const = 0;

        [[nodiscard]]
        virtual bool GetBodyLocalVector(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& worldVector,
            AZ::Vector3& localVector) const = 0;

        [[nodiscard]]
        virtual bool GetBodyWorldVector(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& localVector,
            AZ::Vector3& worldVector) const = 0;

        virtual bool SetLinearVelocity(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& velocity) = 0;

        virtual bool SetAngularVelocity(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& velocity) = 0;

        [[nodiscard]]
        virtual AZ::Vector3 GetLinearVelocityAtLocalPoint(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& localPoint) const = 0;

        [[nodiscard]]
        virtual AZ::Vector3 GetLinearVelocityAtWorldPoint(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& worldPoint) const = 0;
        ...
        virtual bool ApplyLinearImpulse(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
        ...
        virtual bool ApplyAngularImpulse(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const AZ::Vector3& impulse,
            bool wake = true) = 0;
        ...
        [[nodiscard]]
        virtual bool GetMassProperties(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            MassProperties& properties) const = 0;

        virtual bool SetMassProperties(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const MassProperties& properties) = 0;

        virtual bool RecomputeMassFromShapes(
            WorldHandle worldHandle,
            BodyHandle bodyHandle) = 0;

        [[nodiscard]]
        virtual AZ::Matrix3x3 GetWorldInverseInertia(
            WorldHandle worldHandle,
            BodyHandle bodyHandle) const = 0;

        [[nodiscard]]
        virtual AZ::Vector3 GetWorldCenterOfMass(
            WorldHandle worldHandle,
            BodyHandle bodyHandle) const = 0;

        [[nodiscard]]
        virtual bool GetBodyClosestPoint(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
        ...
        [[nodiscard]]
        virtual AZ::Aabb GetBodyAabb(
            WorldHandle worldHandle,
            BodyHandle bodyHandle) const = 0;

        [[nodiscard]]
        virtual BufferResult GetBodyShapes(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            AZStd::span<ShapeHandle> shapeHandles) const = 0;

        [[nodiscard]]
        virtual BufferResult GetBodyJoints(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            AZStd::span<JointHandle> jointHandles) const = 0;

        [[nodiscard]]
        virtual ContactSnapshotResult GetBodyContacts(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
        ...
        [[nodiscard]]
        virtual BufferResult GetBodySensorOverlaps(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            AZStd::span<SensorOverlap> overlaps) const = 0;

        [[nodiscard]]
        virtual bool RaycastBody(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
        ...
        [[nodiscard]]
        virtual bool OverlapBody(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            const BodyOverlapRequest& request) const = 0;
        ...
        [[nodiscard]]
        virtual ShapeHandle CreateShapeFromCooked(
            WorldHandle worldHandle,
            BodyHandle bodyHandle,
            CookedShapeHandle cookedShapeHandle,
            const ShapeProperties& properties) = 0;
        ...
        virtual bool DestroyShape(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            bool updateBodyMass = true) = 0;

        virtual bool SetShapeCollisionFilter(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            const CollisionFilter& collisionFilter) = 0;

        virtual bool SetShapeMaterials(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            AZStd::span<const MaterialHandle> materials) = 0;

        [[nodiscard]]
        virtual AZ::Aabb GetShapeAabb(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle) const = 0;

        [[nodiscard]]
        virtual bool GetShapeState(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            ShapeState& state) const = 0;

        [[nodiscard]]
        virtual BufferResult GetShapeMaterials(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            AZStd::span<MaterialHandle> materialHandles) const = 0;

        virtual bool SetShapeDensity(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            float density,
            bool updateBodyMass = true) = 0;

        virtual bool SetShapeFriction(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            float friction) = 0;

        virtual bool SetShapeRestitution(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            float restitution) = 0;
        ...
        [[nodiscard]]
        virtual bool GetShapeMassProperties(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            MassProperties& properties) const = 0;

        [[nodiscard]]
        virtual bool GetShapeClosestPoint(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            const AZ::Vector3& target,
            AZ::Vector3& position,
        ...
        [[nodiscard]]
        virtual ContactSnapshotResult GetShapeContacts(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            AZStd::span<ContactSnapshot> contacts,
            AZStd::span<ContactPoint> points) const = 0;
        ...
        [[nodiscard]]
        virtual BufferResult GetShapeSensorOverlaps(
            WorldHandle worldHandle,
            ShapeHandle shapeHandle,
            AZStd::span<SensorOverlap> overlaps) const = 0;

        [[nodiscard]]
        virtual JointHandle CreateJoint(
            WorldHandle worldHandle,
            const JointConfiguration& configuration) = 0;
        ...
        virtual bool DestroyJoint(
            WorldHandle worldHandle,
            JointHandle jointHandle,
            bool wakeAttachedBodies = true) = 0;

        [[nodiscard]]
        virtual bool GetJointConfiguration(
            WorldHandle worldHandle,
            JointHandle jointHandle,
            JointConfiguration& configuration) const = 0;

        [[nodiscard]]
        virtual bool GetJointMeasurements(
            WorldHandle worldHandle,
            JointHandle jointHandle,
            JointMeasurements& measurements) const = 0;

        virtual bool WakeJointBodies(
            WorldHandle worldHandle,
            JointHandle jointHandle) = 0;
        ...
        [[nodiscard]]
        virtual bool GetCharacterConfiguration(
            WorldHandle worldHandle,
            CharacterHandle characterHandle,
            CharacterConfiguration& configuration) const = 0;
        ...
        virtual BufferResult RaycastClosestBatch(
            ...
            AZStd::span<ClosestQueryResult> results) const = 0;

        [[nodiscard]]
        virtual QueryResult Raycast(
            WorldHandle worldHandle,
            const RaycastRequest& request,
            AZStd::span<QueryHit> hits) const = 0;

        [[nodiscard]]
        virtual QueryResult ShapeCast(
            WorldHandle worldHandle,
            const ShapeCastRequest& request,
            AZStd::span<QueryHit> hits) const = 0;

        [[nodiscard]]
        virtual QueryResult Overlap(
            WorldHandle worldHandle,
        ...
        [[nodiscard]]
        virtual QueryResult OverlapAabb(
            WorldHandle worldHandle,
        ...
        [[nodiscard]]
        virtual StepEvents GetStepEvents(WorldHandle worldHandle) const = 0;
