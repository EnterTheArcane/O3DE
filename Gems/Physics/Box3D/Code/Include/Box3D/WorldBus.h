...
namespace Box3D
{
    ...
    struct RaycastRequestCollection final
    {
        ...
        [[nodiscard]]
        AZStd::span<const RaycastRequest> GetRequests() const noexcept
        {
            return m_requests;
        }
    ...
    struct ClosestQueryResultCollection final
    {
        ...
        [[nodiscard]]
        size_t GetResultCount() const noexcept
        {
            return m_results.size();
        }

        [[nodiscard]]
        ClosestQueryResult GetResult(
            size_t index) const
        {
    ...
    public:
        ...
        [[nodiscard]]
        virtual ClosestQueryResult RaycastClosest(
            WorldHandle worldHandle,
            const RaycastRequest& request) const = 0;

        [[nodiscard]]
        virtual ClosestQueryResultCollection RaycastClosestBatch(
            WorldHandle worldHandle,
            const RaycastRequestCollection& requests) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection Raycast(
            WorldHandle worldHandle,
            const RaycastRequest& request,
            AZ::u32 maxHitCount) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection OverlapAabb(
            WorldHandle worldHandle,
            const AabbOverlapRequest& request,
            AZ::u32 maxHitCount) const = 0;

        [[nodiscard]]
        virtual size_t GetContactPointCount(WorldHandle worldHandle) const = 0;

        [[nodiscard]]
        virtual ContactPoint GetContactPointAt(
            WorldHandle worldHandle,
            size_t index) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection ShapeCastSphere(
            WorldHandle worldHandle,
            const SphereShapeConfiguration& geometry,
            const ConvexCastParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection OverlapSphere(
            WorldHandle worldHandle,
            const SphereShapeConfiguration& geometry,
            const ConvexOverlapParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection ShapeCastCapsule(
            WorldHandle worldHandle,
            const CapsuleShapeConfiguration& geometry,
            const ConvexCastParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection OverlapCapsule(
            WorldHandle worldHandle,
            const CapsuleShapeConfiguration& geometry,
            const ConvexOverlapParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection ShapeCastBox(
            WorldHandle worldHandle,
            const BoxShapeConfiguration& geometry,
            const ConvexCastParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection OverlapBox(
            WorldHandle worldHandle,
            const BoxShapeConfiguration& geometry,
            const ConvexOverlapParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection ShapeCastCylinder(
            WorldHandle worldHandle,
            const CylinderShapeConfiguration& geometry,
            const ConvexCastParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection OverlapCylinder(
            WorldHandle worldHandle,
            const CylinderShapeConfiguration& geometry,
            const ConvexOverlapParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection ShapeCastConvexHull(
            WorldHandle worldHandle,
            const ConvexHullShapeConfiguration& geometry,
            const ConvexCastParameters& parameters) const = 0;

        [[nodiscard]]
        virtual QueryHitCollection OverlapConvexHull(
            WorldHandle worldHandle,
            const ConvexHullShapeConfiguration& geometry,
            const ConvexOverlapParameters& parameters) const = 0;
    };
    ...
    public:
        ...
        virtual void OnContactHit(
            [[maybe_unused]] const ContactHitEvent& event)
        {
        }

        virtual void OnJointThresholdExceeded(
            [[maybe_unused]] const JointThresholdEvent& event)
        {
        }

        virtual void OnSensor(
            [[maybe_unused]] const SensorEvent& event)
        {
        }
    };
