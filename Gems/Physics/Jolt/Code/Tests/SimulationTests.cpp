...
namespace Jolt
{
    namespace
    {
        ...
        class TestFrameGraphBuilder final
            : public AZ::RHI::FrameGraphBuilder
        {
        public:
            AZ::RHI::FrameGraphAttachmentInterface GetAttachmentDatabase() override
            {
                return AZ::RHI::FrameGraphAttachmentInterface(m_attachmentDatabase);
            }

            AZ::RHI::ResultCode ImportScopeProducer(
                [[maybe_unused]] AZ::RHI::ScopeProducer& scopeProducer) override
            {
                return AZ::RHI::ResultCode::Success;
            }

        private:
            AZ::RHI::FrameGraphAttachmentDatabase m_attachmentDatabase;
        };
        ...
        class CountingComputeQueue final
            : public JPH::ComputeQueue
        {
        public:
            CountingComputeQueue(
                JPH::ComputeQueue* queue,
                AZStd::atomic_uint32_t& executeCount,
                AZStd::atomic_uint32_t& waitCount)
                : m_queue(queue)
            ...
            void SetShader(
                const JPH::ComputeShader* shader) override
            {
                m_queue->SetShader(shader);
            }
            ...
            {
                m_queue->SetConstantBuffer(name, buffer);
            }
            ...
            void SetBuffer(
                ...
                const JPH::ComputeBuffer* buffer) override
            {
                m_queue->SetBuffer(name, buffer);
            }

            void SetRWBuffer(
                const char* name,
                JPH::ComputeBuffer* buffer,
                const EBarrier barrier) override
            {
                m_queue->SetRWBuffer(name, buffer, barrier);
            }
            ...
            {
                m_queue->ScheduleReadback(destination, source);
            }

            void Dispatch(
                const JPH::uint threadGroupCountX,
                const JPH::uint threadGroupCountY,
                const JPH::uint threadGroupCountZ) override
            {
                m_queue->Dispatch(
                    threadGroupCountX,
                    threadGroupCountY,
            ...
            {
                m_executeCount.fetch_add(1, AZStd::memory_order_relaxed);
                m_queue->Execute();
            }

            void Wait() override
            {
                m_waitCount.fetch_add(1, AZStd::memory_order_relaxed);
                m_queue->Wait();
            }

        private:
            JPH::Ref<JPH::ComputeQueue> m_queue;
            AZStd::atomic_uint32_t& m_executeCount;
            AZStd::atomic_uint32_t& m_waitCount;
        };

        class CountingComputeSystem final
            : public JPH::ComputeSystemCPU
        {
        public:
            CountingComputeSystem(
                AZStd::atomic_uint32_t& executeCount,
                AZStd::atomic_uint32_t& waitCount,
                AZStd::atomic_uint32_t& bufferCreationCount,
                AZStd::atomic_uint32_t& failBufferCreationAt)
                : m_executeCount(executeCount)
                , m_waitCount(waitCount)
                , m_bufferCreationCount(bufferCreationCount)
                , m_failBufferCreationAt(failBufferCreationAt)
            {
            }

            [[nodiscard]]
            JPH::ComputeBufferResult CreateComputeBuffer(
                const JPH::ComputeBuffer::EType type,
                const JPH::uint64 elementCount,
                const JPH::uint stride,
                const void* data) override
            {
                const AZ::u32 creationIndex = m_bufferCreationCount.fetch_add(
                    1,
                    AZStd::memory_order_relaxed);
                if (creationIndex == m_failBufferCreationAt.load(AZStd::memory_order_relaxed))
                {
                    JPH::ComputeBufferResult result;
                    result.SetError("Injected compute-buffer allocation failure.");
                    return result;
                }

                return JPH::ComputeSystemCPU::CreateComputeBuffer(
                    type,
                    elementCount,
                    stride,
                    data);
            ...
            {
                JPH::ComputeQueueResult queueResult = JPH::ComputeSystemCPU::CreateComputeQueue();
                ...
                JPH::Ref<CountingComputeQueue> queue = new CountingComputeQueue(
                    queueResult.Get(),
                    m_executeCount,
                    m_waitCount);
        ...
        private:
            AZStd::atomic_uint32_t& m_executeCount;
            AZStd::atomic_uint32_t& m_waitCount;
            AZStd::atomic_uint32_t& m_bufferCreationCount;
            AZStd::atomic_uint32_t& m_failBufferCreationAt;
        };

        class TestHairComputeProvider final
        ...
        public:
            ...
            {
            public:
                bool Initialize([[maybe_unused]] JPH::Hair& hair) override
                {
                    ++m_initializeCount;
                    return true;
                }

                bool BeginUpdate([[maybe_unused]] JPH::Hair& hair) override
                {
                    ++m_beginUpdateCount;
                    return true;
                }
                ...
                bool Acquire(HairRenderBuffer& buffer) override
                {
                    ...
                    buffer = {
                        .m_attachmentId = AZ::Name::FromStringLiteral(
                            "Test Hair Render Buffer",
                            nullptr),
                        .m_buffer = reinterpret_cast<AZ::RHI::Buffer*>(this),
                        .m_token = m_token,
                        .m_vertexCount = 3,
                        .m_stride = 12,
                    };
                ...
                bool ImportHandoff(
                    [[maybe_unused]] AZ::RHI::FrameGraphBuilder& frameGraphBuilder,
                    const AZ::u64 token) override
                {
                    if (!m_acquired || token != m_token)
                    {
                        return false;
                    }
                ...
                {
                    return !m_acquired || m_allowUnsafeDestruction;
                }

                AZ::u64 m_token = 0;
                AZ::u32 m_initializeCount = 0;
                AZ::u32 m_beginUpdateCount = 0;
                AZ::u32 m_completeUpdateCount = 0;
                bool m_ready = false;
                bool m_acquired = false;
                bool m_allowUnsafeDestruction = false;
            };
            ...
            JPH::ComputeSystemResult CreateComputeSystem() override
            {
                ...
                JPH::Ref<CountingComputeSystem> computeSystem = new CountingComputeSystem(
                    m_executeCount,
                    m_waitCount,
                    m_bufferCreationCount,
                    m_failBufferCreationAt);
                JPH::HairRegisterShaders(computeSystem.GetPtr());
            ...
            AZStd::unique_ptr<IHairRenderBuffers> CreateRenderBuffers(
                ...
                [[maybe_unused]] const AZ::u32 vertexCount,
                [[maybe_unused]] const AZ::u32 stride) override
            {
                auto renderBuffers = AZStd::make_unique<RenderBuffers>();
                m_renderBuffers = renderBuffers.get();
                return renderBuffers;
            }

            RenderBuffers* m_renderBuffers = nullptr;
            AZ::u32 m_createCount = 0;
            AZStd::atomic_uint32_t m_executeCount = 0;
            AZStd::atomic_uint32_t m_waitCount = 0;
            AZStd::atomic_uint32_t m_bufferCreationCount = 0;
            AZStd::atomic_uint32_t m_failBufferCreationAt = AZStd::numeric_limits<AZ::u32>::max();
        };
    ...
    TEST(SimulationTests, RuntimeInfoReportsConfiguredHairDeterminismPolicy)
    {
        SystemConfiguration configuration = CreateSerialSystemConfiguration();
        configuration.m_createDefaultWorld = false;
        configuration.m_hairComputeBackend = HairComputeBackend::None;
        {
            System system(configuration, nullptr);
            ASSERT_TRUE(system);

            const RuntimeInfo runtimeInfo = system.GetRuntimeInfo();
            EXPECT_EQ(runtimeInfo.m_hairComputeBackend, HairComputeBackend::None);
            EXPECT_EQ(runtimeInfo.m_hairDeterminism, DeterminismCertification::None);
        }

        configuration.m_hairComputeBackend = HairComputeBackend::DeterministicCpu;
        {
            System system(configuration, nullptr);
            ASSERT_TRUE(system);
        ...
        configuration.m_hairComputeBackend = HairComputeBackend::PlatformGpu;
        {
            ...
            const RuntimeInfo runtimeInfo = system.GetRuntimeInfo();
            EXPECT_EQ(runtimeInfo.m_hairComputeBackend, HairComputeBackend::PlatformGpu);
            EXPECT_EQ(runtimeInfo.m_hairDeterminism, DeterminismCertification::None);
        }
    }

    TEST(SimulationTests, DeterministicCpuHairIgnoresRegisteredPlatformProvider)
    {
        TestHairComputeProvider provider;
        ...
        EXPECT_EQ(provider.m_createCount, 0);
        EXPECT_FALSE(provider.m_renderBuffers);

        const AZStd::array jointModelTransforms{
            AZ::Transform::CreateIdentity(),
        };
        EXPECT_TRUE(system.EnableHairAutoUpdate(
            worldHandle,
            hairHandle,
            AZ::Transform::CreateIdentity(),
            jointModelTransforms));
    ...
    TEST(SimulationTests, PlatformHairRequiresOptInAndUsesRegisteredProvider)
    {
        TestHairComputeProvider provider;
        HairComputeProviderScope providerScope(provider);
        ...
        configuration.m_hairComputeBackend = HairComputeBackend::PlatformGpu;
        {
            ...
            EXPECT_FALSE(CreateMinimalHairDefinition(system));
        ...
        configuration.m_allowNondeterministicHair = true;
        {
            ...
            hairConfiguration.m_definitionHandle = definitionHandle;
            hairConfiguration.m_objectLayer = DefaultLayers::Moving;
            const HairHandle hairHandle = system.CreateHair(
                system.GetDefaultWorldHandle(),
                hairConfiguration);

            EXPECT_TRUE(hairHandle);
            EXPECT_EQ(provider.m_createCount, 1);
            EXPECT_TRUE(system.DestroyHair(system.GetDefaultWorldHandle(), hairHandle));
            EXPECT_TRUE(system.DestroyHairDefinition(definitionHandle));
        }
    }

    TEST(SimulationTests, PlatformHairFlushesManualUpdatesAndBatchesAutomaticUpdates)
    {
        TestHairComputeProvider provider;
        HairComputeProviderScope providerScope(provider);
        ...
        EXPECT_EQ(provider.m_executeCount.load(AZStd::memory_order_relaxed), 0);
        EXPECT_EQ(provider.m_waitCount.load(AZStd::memory_order_relaxed), 0);
        ASSERT_TRUE(system.UpdateHair(
            ...
            AZ::Transform::CreateIdentity(),
            jointModelTransforms));
        EXPECT_EQ(provider.m_executeCount.load(AZStd::memory_order_relaxed), 1);
        EXPECT_EQ(provider.m_waitCount.load(AZStd::memory_order_relaxed), 1);
        ...
        EXPECT_EQ(provider.m_executeCount.load(AZStd::memory_order_relaxed), 2);
        EXPECT_EQ(provider.m_waitCount.load(AZStd::memory_order_relaxed), 2);

        EXPECT_TRUE(system.DestroyHair(worldHandle, firstHairHandle));
        EXPECT_TRUE(system.DestroyHair(worldHandle, secondHairHandle));
        EXPECT_TRUE(system.DestroyHairDefinition(definitionHandle));
    }

    TEST(SimulationTests, PlatformHairRenderBufferUsesOpaqueTokenAndProtectsRendererOwnership)
    {
        TestHairComputeProvider provider;
        HairComputeProviderScope providerScope(provider);
        ...
        const HairHandle hairHandle = system.CreateHair(worldHandle, hairConfiguration);
        ASSERT_TRUE(hairHandle);
        ASSERT_TRUE(provider.m_renderBuffers);
        EXPECT_EQ(provider.m_renderBuffers->m_initializeCount, 1);

        HairRenderBuffer renderBuffer;
        EXPECT_FALSE(system.AcquireHairRenderBuffer(
            worldHandle,
            hairHandle,
            renderBuffer));
        ...
        ASSERT_TRUE(system.UpdateHair(
            ...
            AZ::Transform::CreateIdentity(),
            jointModelTransforms));
        EXPECT_EQ(provider.m_renderBuffers->m_beginUpdateCount, 1);
        EXPECT_EQ(provider.m_renderBuffers->m_completeUpdateCount, 1);

        ASSERT_TRUE(system.AcquireHairRenderBuffer(
            worldHandle,
            hairHandle,
            renderBuffer));
        EXPECT_TRUE(renderBuffer);
        EXPECT_EQ(renderBuffer.m_token, 1);
        EXPECT_EQ(renderBuffer.m_vertexCount, 3);
        EXPECT_EQ(renderBuffer.m_stride, 12);
        const AZ::u64 acquiredToken = renderBuffer.m_token;
        EXPECT_FALSE(system.AcquireHairRenderBuffer(
            worldHandle,
            hairHandle,
            renderBuffer));
        EXPECT_FALSE(renderBuffer);

        TestFrameGraphBuilder frameGraphBuilder;
        EXPECT_FALSE(system.ImportHairRenderBufferHandoff(
            frameGraphBuilder,
            worldHandle,
            hairHandle,
            acquiredToken + 1));
        EXPECT_FALSE(system.DestroyHair(worldHandle, hairHandle));
        EXPECT_TRUE(system.ImportHairRenderBufferHandoff(
            frameGraphBuilder,
            worldHandle,
            hairHandle,
            acquiredToken));
    ...
    TEST(SimulationTests, PlatformHairCreationFailsTransactionallyWhenComputeBufferAllocationFails)
    {
        TestHairComputeProvider provider;
        HairComputeProviderScope providerScope(provider);
        ...
        provider.m_failBufferCreationAt.store(0, AZStd::memory_order_relaxed);
        AZ_TEST_START_TRACE_SUPPRESSION;
        EXPECT_FALSE(CreateMinimalHairDefinition(system));
        AZ_TEST_STOP_TRACE_SUPPRESSION(1);

        provider.m_failBufferCreationAt.store(
        ...
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        provider.m_failBufferCreationAt.store(
            provider.m_bufferCreationCount.load(AZStd::memory_order_relaxed),
            AZStd::memory_order_relaxed);
        AZ_TEST_START_TRACE_SUPPRESSION;
        EXPECT_FALSE(system.CreateHair(worldHandle, hairConfiguration));
        AZ_TEST_STOP_TRACE_SUPPRESSION(1);
    ...
    TEST(SimulationTests, PlatformHairRejectsDeterministicStateOperations)
    {
        TestHairComputeProvider provider;
        HairComputeProviderScope providerScope(provider);
        ...
        EXPECT_FALSE(system.CaptureWorldState(worldHandle));
        WorldStateDigest digest;
        EXPECT_FALSE(system.GetWorldStateDigest(worldHandle, digest));

        const StateSnapshotHandle bodySnapshotHandle = system.CaptureWorldState(
            worldHandle,
            {.m_flags = StateSnapshotFlags::Bodies},
            {});
        ASSERT_TRUE(bodySnapshotHandle);
        EXPECT_TRUE(system.RestoreWorldState(worldHandle, bodySnapshotHandle));
