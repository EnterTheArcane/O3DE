...
option(LY_JOLT_ENABLE_IPO "Build private Jolt targets with interprocedural optimization" ON)
option(
...
if(NOT jolt_native_target_architectures AND CMAKE_VS_PLATFORM_NAME)
    list(APPEND jolt_native_target_architectures "${CMAKE_VS_PLATFORM_NAME}")
endif()
if(NOT jolt_native_target_architectures AND CMAKE_CXX_COMPILER_TARGET)
    list(APPEND jolt_native_target_architectures "${CMAKE_CXX_COMPILER_TARGET}")
endif()
if(NOT jolt_native_target_architectures)
    list(APPEND jolt_native_target_architectures "${CMAKE_SYSTEM_PROCESSOR}")
endif()
list(JOIN jolt_native_target_architectures "," jolt_native_target_architecture_identity)

set(JOLT_NATIVE_TARGET_HAS_ARM OFF)
set(JOLT_NATIVE_TARGET_HAS_ARM32 OFF)
set(JOLT_NATIVE_TARGET_HAS_X86 OFF)
foreach(jolt_native_target_architecture IN LISTS jolt_native_target_architectures)
    string(TOLOWER "${jolt_native_target_architecture}" jolt_native_target_architecture_lower)
    if(jolt_native_target_architecture_lower MATCHES "^(arm64|arm64ec|aarch64)(-|$)")
        set(JOLT_NATIVE_TARGET_HAS_ARM ON)
    elseif(jolt_native_target_architecture_lower MATCHES "^(arm|armv[4-9])(-|$)")
        set(JOLT_NATIVE_TARGET_HAS_ARM ON)
        set(JOLT_NATIVE_TARGET_HAS_ARM32 ON)
    elseif(jolt_native_target_architecture_lower MATCHES "^(amd64|x64|x86_64|x86|i[3-6]86)(-|$)")
        set(JOLT_NATIVE_TARGET_HAS_X86 ON)
    endif()
endforeach()

if(JOLT_NATIVE_TARGET_HAS_ARM AND JOLT_NATIVE_TARGET_HAS_X86 AND NOT XCODE)
    message(FATAL_ERROR "A multi-architecture Jolt target is supported only by the Xcode generator.")
endif()

function(jolt_apply_native_abi target_name)
    if(XCODE AND JOLT_NATIVE_TARGET_HAS_X86)
        set_property(
            TARGET ${target_name} PROPERTY
            "XCODE_ATTRIBUTE_GCC_PREPROCESSOR_DEFINITIONS[arch=x86_64]"
            "$(inherited) JPH_USE_SSE4_1")
        set_property(
            TARGET ${target_name} PROPERTY
            "XCODE_ATTRIBUTE_OTHER_CPLUSPLUSFLAGS[arch=x86_64]"
            "$(inherited) -mno-avx -mno-avx2 -mno-avx512f -mno-f16c -mno-fma -mno-bmi -mno-bmi2 -mno-lzcnt -mno-popcnt -mno-sse4.2 -msse4.1")
    elseif(JOLT_NATIVE_TARGET_HAS_X86)
        target_compile_definitions(${target_name} PRIVATE JPH_USE_SSE4_1)
...
jolt_apply_native_abi(Jolt)
if(XCODE)
    if(JOLT_NATIVE_TARGET_HAS_X86)
        set_property(
            TARGET Jolt PROPERTY
            "XCODE_ATTRIBUTE_GCC_PREPROCESSOR_DEFINITIONS[arch=x86_64]"
            "$(inherited) JPH_USE_SSE4_1 JOLT_ARCH_FAMILY_ARM=0 JOLT_ARCH_FAMILY_X86=1")
    endif()
    if(JOLT_NATIVE_TARGET_HAS_ARM)
        set_property(
            TARGET Jolt PROPERTY
            "XCODE_ATTRIBUTE_GCC_PREPROCESSOR_DEFINITIONS[arch=arm64]"
            "$(inherited) JOLT_ARCH_FAMILY_ARM=1 JOLT_ARCH_FAMILY_X86=0")
    endif()
elseif(JOLT_NATIVE_TARGET_HAS_X86)
    target_compile_definitions(Jolt PRIVATE JOLT_ARCH_FAMILY_ARM=0 JOLT_ARCH_FAMILY_X86=1)
elseif(JOLT_NATIVE_TARGET_HAS_ARM)
    target_compile_definitions(Jolt PRIVATE JOLT_ARCH_FAMILY_ARM=1 JOLT_ARCH_FAMILY_X86=0)
else()
    target_compile_definitions(Jolt PRIVATE JOLT_ARCH_FAMILY_ARM=0 JOLT_ARCH_FAMILY_X86=0)
endif()

set(JOLT_NATIVE_IPO_ENABLED OFF)
set(jolt_native_ipo_excluded OFF)
if(LY_MONOLITHIC_GAME)
    set(jolt_native_ipo_excluded ON)
elseif(WIN32 AND JOLT_NATIVE_TARGET_HAS_ARM)
    set(jolt_native_ipo_excluded ON)
elseif(JOLT_NATIVE_TARGET_HAS_ARM32)
    set(jolt_native_ipo_excluded ON)
endif()

if(LY_JOLT_ENABLE_IPO AND NOT jolt_native_ipo_excluded)
    include(CheckIPOSupported)
    if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC" AND CMAKE_GENERATOR STREQUAL "Ninja Multi-Config")
        # CMake 4.4's MSVC IPO Debug try-compile does not return under Ninja
        # Multi-Config. Check the same CMake/toolchain capability predicates;
        # the Release Jolt build remains the definitive /GL and /LTCG probe.
        if(_CMAKE_CXX_IPO_SUPPORTED_BY_CMAKE AND _CMAKE_CXX_IPO_MAY_BE_SUPPORTED_BY_COMPILER)
            set(jolt_native_ipo_supported ON)
        else()
            set(jolt_native_ipo_supported OFF)
            set(jolt_native_ipo_error "CMake or the MSVC toolchain does not advertise IPO support")
        endif()
    else()
        check_ipo_supported(RESULT jolt_native_ipo_supported OUTPUT jolt_native_ipo_error LANGUAGES CXX)
    endif()
    if(jolt_native_ipo_supported)
        set(JOLT_NATIVE_IPO_ENABLED ON)
        set_property(TARGET Jolt PROPERTY INTERPROCEDURAL_OPTIMIZATION_RELEASE TRUE)
    else()
        message(STATUS "Private Jolt IPO is unavailable: ${jolt_native_ipo_error}")
    endif()
elseif(LY_JOLT_ENABLE_IPO)
    message(STATUS "Private Jolt IPO is disabled for this target topology or architecture.")
endif()
...
string(CONCAT jolt_native_build_identity
    ...
    "target_architectures=${jolt_native_target_architecture_identity};"
