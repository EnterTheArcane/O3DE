...
namespace Box3D
{
    ...
    struct MaterialHandleCollection final
    {
        ...
        explicit MaterialHandleCollection(AZStd::span<const MaterialHandle> handles)
            : m_handles(handles.begin(), handles.end())
        {
        }

        void Add(MaterialHandle handle)
        {
        ...
        [[nodiscard]] MaterialHandle GetAt(size_t index) const
        {
            return index < m_handles.size() ? m_handles[index] : MaterialHandle{};
        }
        [[nodiscard]] AZStd::span<const MaterialHandle> GetHandles() const
        {
            return m_handles;
        }
    ...
    struct MaterialConfigurationCollection final
    {
        ...
        explicit MaterialConfigurationCollection(AZStd::span<const MaterialConfiguration> configurations)
            : m_configurations(configurations.begin(), configurations.end())
        {
        }

        void Add(const MaterialConfiguration& configuration)
        {
        ...
        [[nodiscard]] MaterialConfiguration GetAt(size_t index) const
        {
            return index < m_configurations.size() ? m_configurations[index] : MaterialConfiguration{};
        }
        [[nodiscard]] AZStd::span<const MaterialConfiguration> GetConfigurations() const
        {
