...
namespace Box3D
{
    ...
    public:
        ...
        explicit constexpr CollisionLayer(AZ::u8 index) noexcept
            : m_index(index)
        {
        }
        ...
        [[nodiscard]] constexpr AZ::u8 GetIndex() const noexcept
        {
            return m_index;
        }

        [[nodiscard]] constexpr AZ::u64 GetMask() const noexcept
        {
            return IsValid() ? AZ::u64{ 1 } << m_index : 0;
        }
    ...
    struct CollisionFilter final
    {
        ...
        [[nodiscard]] constexpr bool Allows(const CollisionFilter& other) const noexcept
        {
