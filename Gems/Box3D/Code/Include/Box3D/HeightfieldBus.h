...
namespace Box3D
{
    ...
    public:
        ...
        [[nodiscard]] virtual ShapeHandle GetShapeHandle() const = 0;
        [[nodiscard]] virtual AZ::u32 GetColumnCount() const = 0;
        [[nodiscard]] virtual AZ::u32 GetRowCount() const = 0;
        [[nodiscard]] virtual AZStd::span<const float> GetHeights() const = 0;
        [[nodiscard]] virtual AZStd::span<const AZ::u8> GetMaterialIndices() const = 0;
        [[nodiscard]] virtual AZStd::vector<float> GetHeightsCopy() const
        {
            const AZStd::span<const float> heights = GetHeights();
            return { heights.begin(), heights.end() };
        }
        [[nodiscard]] virtual AZStd::vector<AZ::u8> GetMaterialIndicesCopy() const
        {
        ...
        virtual bool UpdateHeights(
            AZ::u32 startColumn, AZ::u32 startRow, AZ::u32 columnCount, AZ::u32 rowCount, AZStd::span<const float> heights) = 0;
        virtual bool UpdateHeightsFromList(
            AZ::u32 startColumn, AZ::u32 startRow, AZ::u32 columnCount, AZ::u32 rowCount, const AZStd::vector<float>& heights)
        {
            return UpdateHeights(startColumn, startRow, columnCount, rowCount, heights);
        }
        virtual bool UpdateMaterials(
            AZ::u32 startColumn, AZ::u32 startRow, AZ::u32 columnCount, AZ::u32 rowCount, AZStd::span<const AZ::u8> materialIndices) = 0;
        virtual bool UpdateMaterialsFromList(
            AZ::u32 startColumn, AZ::u32 startRow, AZ::u32 columnCount, AZ::u32 rowCount, const AZStd::vector<AZ::u8>& materialIndices)
        {
