...
namespace Box3D::Tests
{
    ...
    namespace
    {
        ...
        [[nodiscard]] AZ::u64 ReadArm64FloatControl()
        {
#if defined(_M_ARM64) || defined(_M_ARM64EC)
        ...
        void WriteArm64FloatControl(AZ::u64 control)
        {
#if defined(_M_ARM64) || defined(_M_ARM64EC)
        ...
        BodyHandle CreateBody(System& system, WorldHandle worldHandle, BodyType bodyType, const AZ::Vector3& position)
        {
        ...
        ShapeHandle CreateSphere(System& system, WorldHandle worldHandle, BodyHandle bodyHandle, float radius = 0.5f)
        {
        ...
        ReferenceSimulation SimulateReferenceStack(AZ::u32 workerCount, AZ::JobContext* jobContext = nullptr)
        {
        ...
        void SetJointBodies(JointConfiguration& configuration, BodyHandle parentBody, BodyHandle childBody)
        {
            AZStd::visit(
        ...
        bool FilterContact(ShapeHandle, ShapeHandle, void* userData)
        {
        ...
        bool FilterPreSolve(ShapeHandle, ShapeHandle, const AZ::Vector3&, const AZ::Vector3&, void* userData)
        {
            auto& state = *static_cast<ContactCallbackState*>(userData);
        ...
        float MixMaterials(float valueA, SurfaceTypeId, float valueB, SurfaceTypeId)
        {
        ...
        float MixMaterial(float valueA, SurfaceTypeId surfaceTypeA, float valueB, SurfaceTypeId surfaceTypeB)
        {
    ...
    TEST(Box3DSystemTests, RegistersOnlyProviderOwnedInterfaces)
    {
    ...
    TEST(Box3DSystemTests, ScriptBusesUseProviderQualifiedNames)
    {
    ...
    TEST(Box3DSystemTests, RecycledSlotsInvalidateOldHandles)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, WorldQueryViewsUseValidatedWorldLifetime)
    {
        System system;
    ...
    TEST(Box3DSystemTests, JointScriptAdaptersPreserveConcreteConfigurationTypes)
    {
    ...
    TEST(Box3DSystemTests, WorldsOwnIndependentGravityEnablementAndNames)
    {
        System system;
        WorldConfiguration configuration;
    ...
    TEST(Box3DSystemTests, InvalidWorldAndBodyConfigurationsFailBeforeCreatingObjects)
    {
        System system;
    ...
    TEST(Box3DSystemTests, InvalidBodyUpdatesAreTransactional)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, MaterialLifetimeTracksShapeReferences)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, InvalidMaterialAndShapeUpdatesPreserveLiveState)
    {
        System system;
        MaterialConfiguration materialConfiguration;
    ...
    TEST(Box3DSystemTests, InvalidJointAndCharacterUpdatesPreserveLiveObjects)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, SimulationAndRecordingAreDeterministicAcrossWorkerCounts)
    {
    ...
    TEST(Box3DSystemTests, SimulationRestoresCallingThreadFloatEnvironment)
    {
...
#if defined(_M_ARM64) || defined(_M_ARM64EC) || defined(__aarch64__)
    TEST(Box3DSystemTests, DeterministicScopeCanonicalizesAndRestoresArm64FloatControl)
    {
    ...
    TEST(Box3DSystemTests, RecordedMutationsAreIndependentOfCallerFloatEnvironment)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        const int originalRoundingMode = std::fegetround();
    ...
    TEST(Box3DSystemTests, RecordingRejectsCallbacksThatCannotBeReplayed)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, ConfigurationChangesApplyToExistingWorlds)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, MaximumLinearSpeedChangesApplyToExistingWorlds)
    {
    ...
    TEST(Box3DSystemTests, DisablingSleepWakesExistingBodies)
    {
        SystemConfiguration configuration;
    ...
    TEST(Box3DSystemTests, ContinuousCollisionPreventsFastBodyTunneling)
    {
    ...
    TEST(Box3DSystemTests, HitEventThresholdChangesApplyToExistingWorlds)
    {
        SystemConfiguration configuration;
    ...
    TEST(Box3DSystemTests, CreatesEveryShapeFamily)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        const BodyHandle bodyHandle = CreateBody(system, worldHandle, BodyType::Static, AZ::Vector3::CreateZero());
    ...
    TEST(Box3DSystemTests, CreatesEveryJointFamily)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        const BodyHandle parent = CreateBody(system, worldHandle, BodyType::Dynamic, AZ::Vector3::CreateZero());
        ...
        AZStd::array<JointConfiguration, 9> configurations{ ParallelJointConfiguration{},  DistanceJointConfiguration{},
                                                            FilterJointConfiguration{},    MotorJointConfiguration{},
                                                            PrismaticJointConfiguration{}, RevoluteJointConfiguration{},
                                                            SphericalJointConfiguration{}, WeldJointConfiguration{},
    ...
    TEST(Box3DSystemTests, QueryReportsRequiredCapacityAndKeepsClosestHits)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, ReportsContactPhasesHitsAndBodyMoves)
    {
        SystemConfiguration systemConfiguration;
    ...
    TEST(Box3DSystemTests, ReportsSensorTransitions)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, InvokesOptInCollisionAndPreSolveCallbacks)
    {
        System system;
        ContactCallbackState callbackState;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DSystemTests, HonorsCollisionGroupOverrides)
    {
        for (const AZ::s32 groupIndex : { -7, 7 })
        {
            ...
            filterA.m_maskBits = groupIndex > 0 ? 0 : AZStd::numeric_limits<AZ::u64>::max();
    ...
    TEST(Box3DSystemTests, CharacterMovesAndReportsState)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
