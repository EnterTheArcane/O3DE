...
namespace Box3D::Tests
{
    namespace
    {
        BodyHandle CreateEventBody(System& system, WorldHandle worldHandle, BodyType bodyType, const AZ::Vector3& position)
        {
        ...
        {
        public:
            void OnBodyMoved(const BodyMoveEvent&) override
            {
                ++m_bodyMoveCount;
            }
    ...
    TEST(Box3DEventTests, DestroyedContactShapeRetainsEndEventIdentity)
    {
        System system;
        ...
        const auto ended = AZStd::find_if(
            ...
            {
                return event.m_phase == EventPhase::End && event.m_bodyA == bodyA && event.m_bodyB == bodyB && event.m_shapeA == shapeA &&
                    event.m_shapeB == shapeB;
            });
        EXPECT_NE(ended, events.m_contactEvents.end());
    }

    TEST(Box3DEventTests, WorldNotificationBusDispatchesCollectedEventsOncePerStep)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DEventTests, WorldNotificationBusDispatchesEveryEventFamily)
    {
        ...
        const BodyHandle contactVisitor =
            CreateEventBody(system, worldHandle, BodyType::Dynamic, 10.0f * AZ::Vector3::CreateAxisX() + 5.0f * AZ::Vector3::CreateAxisZ());
        ...
        for (AZ::u32 step = 0; step < 120 &&
             (handler.m_bodyMoveCount == 0 || handler.m_sensorCount == 0 || handler.m_contactCount == 0 || handler.m_contactHitCount == 0 ||
        ...
        EXPECT_TRUE(
            (handler.m_lastContactHit.m_bodyA == contactBody && handler.m_lastContactHit.m_bodyB == contactVisitor) ||
            (handler.m_lastContactHit.m_bodyA == contactVisitor && handler.m_lastContactHit.m_bodyB == contactBody));
        EXPECT_EQ(handler.m_lastJointThreshold.m_jointHandle, jointHandle);
    }

    TEST(Box3DEventTests, RuntimeSensorMutationReportsTheNextOverlap)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        const BodyHandle sensorBody = CreateEventBody(system, worldHandle, BodyType::Static, 10.0f * AZ::Vector3::CreateAxisX());
        ...
        const auto sensorEvent = AZStd::find_if(
            ...
            {
                return event.m_phase == EventPhase::Begin && event.m_sensorBody == sensorBody && event.m_visitorBody == visitorBody &&
                    event.m_sensorShape == sensorShape && event.m_visitorShape == visitorShape;
            });
        EXPECT_NE(sensorEvent, events.m_sensorEvents.end());
    }

    TEST(Box3DEventTests, DestroyedSensorBodyCannotBeAttributedToARecycledSlot)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
        ...
        const auto ended = AZStd::find_if(
            ...
            [sensorBody, visitorBody, sensorShape, visitorShape](const SensorEvent& event)
            {
                return event.m_phase == EventPhase::End && event.m_sensorBody == sensorBody && event.m_visitorBody == visitorBody &&
                    event.m_sensorShape == sensorShape && event.m_visitorShape == visitorShape;
            });
        EXPECT_NE(ended, events.m_sensorEvents.end());
    }

    TEST(Box3DEventTests, AutomaticCatchUpRetainsEventsFromEveryFixedStep)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DEventTests, DisabledCollectionAvoidsEveryEventStream)
    {
        System system;
