...
namespace Box3D::Tests
{
    namespace
    {
        ...
        public:
            ...
            void DrawLine(const AZ::Vector3&, const AZ::Vector3&, const AZ::Color&) override
            {
                ++m_drawCallCount;
            }

            void DrawTransform(const AZ::Transform&) override
            {
                ++m_drawCallCount;
            }

            void DrawPoint(const AZ::Vector3&, float, const AZ::Color&) override
            {
                ++m_drawCallCount;
            }

            void DrawSphere(const AZ::Vector3&, float, const AZ::Color&, float) override
            {
                ++m_drawCallCount;
            }

            void DrawCapsule(const AZ::Vector3&, const AZ::Vector3&, float, const AZ::Color&, float) override
            {
                ++m_drawCallCount;
            }

            void DrawBounds(const AZ::Aabb&, const AZ::Color&) override
            {
                ++m_drawCallCount;
            }

            void DrawBox(const AZ::Vector3&, const AZ::Transform&, const AZ::Color&) override
            {
                ++m_drawCallCount;
            }

            void DrawText(const AZ::Vector3&, AZStd::string_view, const AZ::Color&) override
            {
                ++m_drawCallCount;
            }

            [[nodiscard]] AZ::u32 GetDrawCallCount() const
            {
    ...
    TEST(Box3DDiagnosticsTests, ReportsStatisticsDrawsAndRebuildsStaticTree)
    {
        System system;
    ...
    TEST(Box3DDiagnosticsTests, RecordingReplaySupportsSeekingQueriesBodiesAndDrawing)
    {
        System system;
        const WorldHandle worldHandle = system.GetDefaultWorldHandle();
    ...
    TEST(Box3DDiagnosticsTests, RecordingCapturesConfiguredLengthScale)
    {
