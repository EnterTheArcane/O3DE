...
namespace Box3D
{
    namespace
    {
        ...
        Geometry GetCompoundChildGeometry(const CompoundChildGeometry& geometry)
        {
    ...
    void CompoundChildShapeConfiguration::SetSphere(const SphereShapeConfiguration& geometry)
    {
        m_geometry = geometry;
    }

    void CompoundChildShapeConfiguration::SetCapsule(const CapsuleShapeConfiguration& geometry)
    {
        m_geometry = geometry;
    }

    void CompoundChildShapeConfiguration::SetBox(const BoxShapeConfiguration& geometry)
    {
        m_geometry = geometry;
    }

    void CompoundChildShapeConfiguration::SetCylinder(const CylinderShapeConfiguration& geometry)
    {
        m_geometry = geometry;
    }

    void CompoundChildShapeConfiguration::SetConvexHull(const ConvexHullShapeConfiguration& geometry)
    {
        m_geometry = geometry;
    }

    void CompoundChildShapeConfiguration::SetTriangleMesh(const TriangleMeshShapeConfiguration& geometry)
    {
        m_geometry = geometry;
    }
    ...
    void ShapeConfiguration::Reflect(AZ::ReflectContext* context)
    {
        ...
        {
            serializeContext->Class<SphereShapeConfiguration>()->Version(0)->Field("Radius", &SphereShapeConfiguration::m_radius);
            ...
            serializeContext->Class<BoxShapeConfiguration>()->Version(0)->Field("HalfExtents", &BoxShapeConfiguration::m_halfExtents);
            ...
            serializeContext->Class<ConvexHullShapeConfiguration>()->Version(0)->Field(
                "Vertices", &ConvexHullShapeConfiguration::m_vertices);
            serializeContext->Class<TriangleMeshShapeConfiguration>()
                ->Version(1)
            ...
            if (AZ::EditContext* editContext = serializeContext->GetEditContext())
            {
                ...
                editContext->Class<CapsuleShapeConfiguration>("Capsule", "")
                    ...
                    ->Attribute(AZ::Edit::Attributes::Min, 0.001f);
                editContext->Class<BoxShapeConfiguration>("Box", "")->DataElement(
                    AZ::Edit::UIHandlers::Default, &BoxShapeConfiguration::m_halfExtents, "Half extents", "");
                ...
                editContext->Class<TriangleMeshShapeConfiguration>("Triangle mesh", "")
                    ...
                    ->DataElement(
                        ...
                        &TriangleMeshShapeConfiguration::m_weldTolerance,
                        "Weld tolerance",
                        "Maximum distance for optional vertex welding.")
                    ->Attribute(AZ::Edit::Attributes::Min, 0.0f)
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &TriangleMeshShapeConfiguration::m_weldVertices,
                        "Weld vertices",
                        "Merge vertices within the weld tolerance while cooking.")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &TriangleMeshShapeConfiguration::m_useMedianSplit,
                        "Median split",
                        "Prefer faster median partitioning for structured meshes such as grids.")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &TriangleMeshShapeConfiguration::m_identifyEdges,
                        "Identify edges",
                        "Compute triangle adjacency from shared edges.");
                editContext->Class<HeightfieldShapeConfiguration>("Heightfield", "")
                    ...
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &HeightfieldShapeConfiguration::m_materialIndices,
                        "Material indices",
                    ...
                    ->DataElement(
                        ...
                        "Quantization minimum shared by adjacent heightfields when enabled.")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &HeightfieldShapeConfiguration::m_maximumHeight,
                        "Shared maximum height",
                        "Quantization maximum shared by adjacent heightfields when enabled.")
                    ->DataElement(AZ::Edit::UIHandlers::Default, &HeightfieldShapeConfiguration::m_clockwise, "Clockwise", "")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &HeightfieldShapeConfiguration::m_useSharedHeightRange,
                        "Use shared height range",
                        "Use the explicit range so adjacent fields quantize shared heights identically.");
                ...
                editContext->Class<ShapeProperties>("Shape properties", "Collision and instance behavior")
                    ...
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &ShapeProperties::m_enableSensorEvents,
                        "Sensor events",
                        "Report when this dynamic or kinematic shape enters and leaves sensors")
                    ...
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &ShapeProperties::m_enableCustomFiltering,
                        "Custom filtering",
                        "Invoke the world collision filter for this shape")
                    ->DataElement(AZ::Edit::UIHandlers::Default, &ShapeProperties::m_enablePreSolveEvents, "Pre-solve events", "")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &ShapeProperties::m_createContactsImmediately,
                        "Create contacts immediately",
                        "Disable while attaching shapes in bulk; contacts are created by a later simulation step.")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &ShapeProperties::m_updateBodyMass,
                        "Update body mass",
