...
namespace Box3D
{
    ...
    RigidBodyComponent::RigidBodyComponent(RigidBodyConfiguration configuration, AZ::Name worldName)
    ...
    BodyProperties RigidBodyComponent::GetProperties() const
    {
        ...
        if (m_system != nullptr && m_system->GetBodyProperties(m_worldHandle, m_bodyHandle, properties))
        {
            return properties;
        }
    ...
    bool RigidBodyComponent::SetProperties(const BodyProperties& properties)
    {
        if (m_system != nullptr && m_bodyHandle.IsValid() && !m_system->SetBodyProperties(m_worldHandle, m_bodyHandle, properties))
        {
            return false;
        }
    ...
    ClosestPoint RigidBodyComponent::GetClosestPoint(const AZ::Vector3& target) const
    {
        ClosestPoint result;
        if (m_system != nullptr)
        {
    ...
    bool RigidBodyComponent::SetMassProperties(const MassProperties& properties)
    {
    ...
    bool RigidBodyComponent::SetTransform(const AZ::Transform& transform)
    {
        if (m_system == nullptr || m_collider == nullptr)
        {
            return false;
    ...
    bool RigidBodyComponent::SetLinearVelocity(const AZ::Vector3& velocity)
    {
        return m_system != nullptr && m_system->SetLinearVelocity(m_worldHandle, m_bodyHandle, velocity);
    }

    bool RigidBodyComponent::SetAngularVelocity(const AZ::Vector3& velocity)
    {
    ...
    bool RigidBodyComponent::ApplyLinearImpulse(const AZ::Vector3& impulse)
    {
        return m_system != nullptr && m_system->ApplyLinearImpulse(m_worldHandle, m_bodyHandle, impulse);
    }

    bool RigidBodyComponent::ApplyLinearImpulseAtWorldPoint(const AZ::Vector3& impulse, const AZ::Vector3& worldPoint)
    {
        return m_system != nullptr && m_system->ApplyLinearImpulseAtWorldPoint(m_worldHandle, m_bodyHandle, impulse, worldPoint);
    }

    bool RigidBodyComponent::ApplyAngularImpulse(const AZ::Vector3& impulse)
    {
    ...
    {
        if (!m_syncingTransform && m_system != nullptr && m_bodyHandle.IsValid())
        {
