...
namespace Box3D
{
    ...
    struct ShapeId final
    {
        ...
        [[nodiscard]] constexpr AZ::u32 GetIndex() const noexcept
        {
            const AZ::u64 index = m_value >> 32;
            return index != 0 ? static_cast<AZ::u32>(index - 1) : AZStd::numeric_limits<AZ::u32>::max();
        }
    ...
    public:
        ...
        HullGeometry(const HullGeometry&) = delete;
        HullGeometry& operator=(const HullGeometry&) = delete;
        HullGeometry(HullGeometry&&) = delete;
        HullGeometry& operator=(HullGeometry&&) = delete;
    ...
    private:
        ...
        friend AZStd::unique_ptr<HullGeometry> CreateBoxGeometry(const AZ::Vector3& halfExtents, const GeometryTransform& transform);
        ...
        friend AZStd::unique_ptr<HullGeometry> CreateHullGeometry(
            AZStd::span<const AZ::Vector3> points, const GeometryTransform& transform);
        friend struct GeometryCastAccess;
        friend ShapeId CreateHullShape(BodyId bodyId, const NativeShapeConfiguration& configuration, const HullGeometry& geometry);
    };
    ...
    public:
        ...
        [[nodiscard]] AZ::u8 GetMaterialIndex(size_t faceIndex) const;
    ...
    private:
        ...
        friend AZStd::unique_ptr<MeshGeometry> CreateMeshGeometry(
            ...
            bool useMedianSplit,
            bool identifyEdges);
        friend ShapeId CreateMeshShape(BodyId bodyId, const NativeShapeConfiguration& configuration, const MeshGeometry& geometry);
        friend struct GeometryCastAccess;
    };
    ...
    public:
        ...
        HeightFieldGeometry(const HeightFieldGeometry&) = delete;
    ...
    private:
        ...
        friend ShapeId CreateHeightFieldShape(
            BodyId bodyId, const NativeShapeConfiguration& configuration, const HeightFieldGeometry& geometry);
        friend struct GeometryCastAccess;
    };
    ...
    public:
        ...
        [[nodiscard]] AZStd::span<const AZ::u8> GetMaterialSources() const;
        [[nodiscard]] AZStd::shared_ptr<const CompoundGeometry> CloneWithMaterials(AZStd::span<const SurfaceMaterial> materials) const;

        CompoundGeometry(const CompoundGeometry&) = delete;
        CompoundGeometry& operator=(const CompoundGeometry&) = delete;
        CompoundGeometry(CompoundGeometry&&) = delete;
        CompoundGeometry& operator=(CompoundGeometry&&) = delete;

    private:
        explicit CompoundGeometry(void* data, AZStd::vector<AZ::u8>&& materialSources);
        ...
        friend ShapeId CreateCompoundShape(BodyId bodyId, const NativeShapeConfiguration& configuration, const CompoundGeometry& geometry);
        friend struct GeometryCastAccess;
    };
    ...
    [[nodiscard]] NativeGeometry CookGeometry(
        const ShapeGeometry& geometry, const GeometryTransform& transform, AZStd::span<const SurfaceMaterial> materials);
    [[nodiscard]] ShapeId CreateShape(BodyId bodyId, const NativeShapeConfiguration& configuration, const NativeGeometry& geometry);
    [[nodiscard]] bool SetShapeGeometry(ShapeId shapeId, const NativeGeometry& geometry);
    [[nodiscard]] AZ::Aabb GetAabb(const NativeGeometry& geometry);
    [[nodiscard]] bool CastRay(
        const NativeGeometry& geometry, const AZ::Vector3& start, const AZ::Vector3& translation, GeometryCastHit& hit);
    ...
    public:
        ...
        [[nodiscard]] AZStd::span<const MoverPlane> GetPlanes() const;
    ...
    public:
        ...
        [[nodiscard]] AZStd::span<const JointId> GetJoints() const;
    ...
    public:
        ...
        virtual bool ShouldCollide(ShapeId shapeA, ShapeId shapeB) = 0;
        virtual bool BeforeSolve(ShapeId shapeA, ShapeId shapeB, const AZ::Vector3& position, const AZ::Vector3& normal) = 0;
    };
    ...
    public:
        ...
        [[nodiscard]] virtual float MixFriction(float valueA, AZ::u64 materialIdA, float valueB, AZ::u64 materialIdB) const = 0;
        [[nodiscard]] virtual float MixRestitution(float valueA, AZ::u64 materialIdA, float valueB, AZ::u64 materialIdB) const = 0;
    };
    ...
    private:
        ...
        friend bool StartRecording(WorldId worldId, Recording& recording);
        friend bool StopRecording(WorldId worldId, Recording& recording, AZStd::vector<AZ::u8>& data);
    };
    ...
    [[nodiscard]] AZStd::unique_ptr<Recording> CreateRecording(size_t initialCapacityBytes);
    [[nodiscard]] bool StartRecording(WorldId worldId, Recording& recording);
    [[nodiscard]] bool StopRecording(WorldId worldId, Recording& recording, AZStd::vector<AZ::u8>& data);
    [[nodiscard]] bool ValidateRecording(AZStd::span<const AZ::u8> data, AZ::u32 workerCount);
    [[nodiscard]] AZStd::unique_ptr<IReplay> CreateReplay(AZStd::span<const AZ::u8> data, AZ::u32 workerCount);
    void SetLengthUnitsPerMeter(float lengthUnitsPerMeter);
    void SetStallWarningThreshold(float seconds);
    [[nodiscard]] bool DrawWorld(WorldId worldId, const DebugDrawSettings& settings, IDebugRenderer& renderer);
    [[nodiscard]] WorldId CreateWorld(const NativeWorldConfiguration& configuration);
    void DestroyWorld(WorldId worldId);
    [[nodiscard]] bool IsValid(WorldId worldId);
    ...
    [[nodiscard]] AZ::Vector3 GetGravity(WorldId worldId);
    [[nodiscard]] AZ::Aabb GetWorldAabb(WorldId worldId);
    [[nodiscard]] bool RebuildStaticTree(WorldId worldId);
    [[nodiscard]] bool Explode(
        WorldId worldId, const AZ::Vector3& position, AZ::u64 maskBits, float radius, float falloff, float impulsePerArea);
    [[nodiscard]] bool GetStatistics(WorldId worldId, StatisticsFlags flags, WorldStatistics& statistics);
    ...
    [[nodiscard]] bool CastRayCapsule(
        ...
        GeometryCastHit& hit);
    [[nodiscard]] bool CastRay(
        const HullGeometry& geometry, const AZ::Vector3& start, const AZ::Vector3& translation, GeometryCastHit& hit);
    [[nodiscard]] bool CastRay(
        const MeshGeometry& geometry, const AZ::Vector3& start, const AZ::Vector3& translation, GeometryCastHit& hit);
    [[nodiscard]] bool CastRay(
        const HeightFieldGeometry& geometry, const AZ::Vector3& start, const AZ::Vector3& translation, GeometryCastHit& hit);
    [[nodiscard]] bool CastRay(
        const CompoundGeometry& geometry, const AZ::Vector3& start, const AZ::Vector3& translation, GeometryCastHit& hit);
    [[nodiscard]] AZ::Aabb GetAabb(const CompoundGeometry& geometry, const AZ::Transform& transform = AZ::Transform::CreateIdentity());
    void CastShape(
        WorldId worldId,
    ...
    [[nodiscard]] BodyId CreateBody(WorldId worldId, const BodyConfiguration& configuration);
    void DestroyBody(BodyId bodyId);
    [[nodiscard]] bool IsValid(BodyId bodyId);
    [[nodiscard]] NativeBodyType GetBodyType(BodyId bodyId);
    [[nodiscard]] bool SetBodyType(BodyId bodyId, NativeBodyType type);
    void SetBodyName(BodyId bodyId, AZ::Name name);
    [[nodiscard]] AZ::Transform GetBodyTransform(BodyId bodyId);
    void SetBodyTransform(BodyId bodyId, const AZ::Transform& transform);
    [[nodiscard]] AZ::Vector3 GetBodyLocalPoint(BodyId bodyId, const AZ::Vector3& worldPoint);
    [[nodiscard]] AZ::Vector3 GetBodyWorldPoint(BodyId bodyId, const AZ::Vector3& localPoint);
    [[nodiscard]] AZ::Vector3 GetBodyLocalVector(BodyId bodyId, const AZ::Vector3& worldVector);
    [[nodiscard]] AZ::Vector3 GetBodyWorldVector(BodyId bodyId, const AZ::Vector3& localVector);
    [[nodiscard]] AZ::Vector3 GetLinearVelocity(BodyId bodyId);
    void SetLinearVelocity(BodyId bodyId, const AZ::Vector3& velocity);
    [[nodiscard]] AZ::Vector3 GetAngularVelocity(BodyId bodyId);
    void SetAngularVelocity(BodyId bodyId, const AZ::Vector3& velocity);
    [[nodiscard]] AZ::Vector3 GetLinearVelocityAtLocalPoint(BodyId bodyId, const AZ::Vector3& localPoint);
    [[nodiscard]] AZ::Vector3 GetLinearVelocityAtWorldPoint(BodyId bodyId, const AZ::Vector3& worldPoint);
    ...
    [[nodiscard]] NativeMotionLocks GetMotionLocks(BodyId bodyId);
    void SetMotionLocks(BodyId bodyId, const NativeMotionLocks& locks);
    [[nodiscard]] NativeMassProperties GetMassProperties(BodyId bodyId);
    [[nodiscard]] NativeMassProperties GetShapeMassProperties(ShapeId shapeId);
    void SetMassProperties(BodyId bodyId, const NativeMassProperties& properties);
    void ApplyMassFromShapes(BodyId bodyId);
    [[nodiscard]] AZ::Matrix3x3 GetWorldInverseInertia(BodyId bodyId);
    [[nodiscard]] AZ::Vector3 GetWorldCenterOfMass(BodyId bodyId);
    [[nodiscard]] float GetLinearDamping(BodyId bodyId);
    void SetLinearDamping(BodyId bodyId, float damping);
    [[nodiscard]] float GetAngularDamping(BodyId bodyId);
    void SetAngularDamping(BodyId bodyId, float damping);
    [[nodiscard]] float GetGravityScale(BodyId bodyId);
    void SetGravityScale(BodyId bodyId, float scale);
    [[nodiscard]] bool IsAwake(BodyId bodyId);
    void SetAwake(BodyId bodyId, bool awake);
    [[nodiscard]] float GetSleepThreshold(BodyId bodyId);
    void SetSleepThreshold(BodyId bodyId, float threshold);
    [[nodiscard]] bool IsBodyEnabled(BodyId bodyId);
    ...
    [[nodiscard]] bool IsBullet(BodyId bodyId);
    void SetBodySleepEnabled(BodyId bodyId, bool enabled);
    [[nodiscard]] bool IsBodySleepEnabled(BodyId bodyId);
    void SetBodyContactRecyclingEnabled(BodyId bodyId, bool enabled);
    [[nodiscard]] bool IsBodyContactRecyclingEnabled(BodyId bodyId);
    [[nodiscard]] AZ::Aabb GetBodyAabb(BodyId bodyId);
    [[nodiscard]] bool GetBodyClosestPoint(BodyId bodyId, const AZ::Vector3& target, AZ::Vector3& position, float& distance);
    [[nodiscard]] bool GetBodyJoints(BodyId bodyId, JointScratch& scratch);

    [[nodiscard]] JointId CreateJoint(WorldId worldId, const NativeJointConfiguration& configuration);
    [[nodiscard]] bool UpdateJoint(JointId jointId, const NativeJointConfiguration& configuration);
    void DestroyJoint(JointId jointId, bool wakeAttachedBodies = true);
    void WakeJointBodies(JointId jointId);
    [[nodiscard]] bool IsValid(JointId jointId);
    [[nodiscard]] void* GetJointUserData(JointId jointId);
    [[nodiscard]] AZ::Vector3 GetJointConstraintForce(JointId jointId);
    [[nodiscard]] AZ::Vector3 GetJointConstraintTorque(JointId jointId);
    [[nodiscard]] float GetJointLinearSeparation(JointId jointId);
    [[nodiscard]] float GetJointAngularSeparation(JointId jointId);
    [[nodiscard]] JointState GetJointState(JointId jointId, JointKind kind);

    [[nodiscard]] ShapeId CreateSphereShape(
        BodyId bodyId, const NativeShapeConfiguration& configuration, const AZ::Vector3& center, float radius);
    [[nodiscard]] ShapeId CreateCapsuleShape(
    ...
    [[nodiscard]] bool SetSphereShape(ShapeId shapeId, const AZ::Vector3& center, float radius);
    [[nodiscard]] bool SetCapsuleShape(ShapeId shapeId, const AZ::Vector3& center1, const AZ::Vector3& center2, float radius);
    [[nodiscard]] ShapeId CreateBoxShape(
    ...
    [[nodiscard]] ShapeId CreateHullShape(
        BodyId bodyId,
        const NativeShapeConfiguration& configuration,
        AZStd::span<const AZ::Vector3> points,
        const AZ::Vector3& scale,
        const AZ::Transform& localTransform);
    [[nodiscard]] AZStd::unique_ptr<HullGeometry> CreateBoxGeometry(const AZ::Vector3& halfExtents, const GeometryTransform& transform);
    [[nodiscard]] AZStd::unique_ptr<HullGeometry> CreateCylinderGeometry(
        float height, float radius, AZ::u8 subdivisionCount, const GeometryTransform& transform);
    [[nodiscard]] AZStd::unique_ptr<HullGeometry> CreateHullGeometry(
        AZStd::span<const AZ::Vector3> points, const GeometryTransform& transform);
    [[nodiscard]] ShapeId CreateHullShape(BodyId bodyId, const NativeShapeConfiguration& configuration, const HullGeometry& geometry);
    [[nodiscard]] bool SetHullShape(ShapeId shapeId, const HullGeometry& geometry);
    [[nodiscard]] AZStd::unique_ptr<MeshGeometry> CreateMeshGeometry(
        AZStd::span<const AZ::Vector3> vertices,
        AZStd::span<const AZ::u32> indices,
        AZStd::span<const AZ::u8> materialIndices = {},
    ...
    [[nodiscard]] ShapeId CreateMeshShape(BodyId bodyId, const NativeShapeConfiguration& configuration, const MeshGeometry& geometry);
    [[nodiscard]] bool SetMeshShape(ShapeId shapeId, const MeshGeometry& geometry);
    [[nodiscard]] AZStd::unique_ptr<HeightFieldGeometry> CreateHeightFieldGeometry(
        AZStd::span<const float> heights,
        AZStd::span<const AZ::u8> materialIndices,
        size_t columnCount,
        size_t rowCount,
        ...
        float maximumHeight,
        bool clockwise);
    [[nodiscard]] ShapeId CreateHeightFieldShape(
        BodyId bodyId, const NativeShapeConfiguration& configuration, const HeightFieldGeometry& geometry);
    [[nodiscard]] AZStd::unique_ptr<CompoundGeometry> CreateCompoundGeometry(const CompoundGeometry::Definition& definition);
    [[nodiscard]] ShapeId CreateCompoundShape(
        BodyId bodyId, const NativeShapeConfiguration& configuration, const CompoundGeometry& geometry);
    void DestroyShape(ShapeId shapeId, bool updateBodyMass = true);
    [[nodiscard]] bool IsValid(ShapeId shapeId);
    [[nodiscard]] bool GetShapeState(ShapeId shapeId, NativeShapeState& state);
    void SetShapeFilter(ShapeId shapeId, AZ::u64 categoryBits, AZ::u64 maskBits, AZ::s32 groupIndex);
    [[nodiscard]] bool SetShapeDensity(ShapeId shapeId, float density, bool updateBodyMass);
    [[nodiscard]] bool SetShapeFriction(ShapeId shapeId, float friction);
    [[nodiscard]] bool SetShapeRestitution(ShapeId shapeId, float restitution);
    [[nodiscard]] size_t GetShapeMaterialCount(ShapeId shapeId);
    [[nodiscard]] bool GetShapeMaterials(ShapeId shapeId, AZStd::span<SurfaceMaterial> materials);
    void SetShapeSurfaceMaterial(ShapeId shapeId, const SurfaceMaterial& material);
    [[nodiscard]] bool SetShapeMaterials(ShapeId shapeId, AZStd::span<const SurfaceMaterial> materials);
    [[nodiscard]] bool SetShapeEventSubscriptions(
        ShapeId shapeId, bool sensorEvents, bool contactEvents, bool hitEvents, bool preSolveEvents);
    [[nodiscard]] AZ::Aabb GetShapeAabb(ShapeId shapeId);
    [[nodiscard]] bool GetShapeClosestPoint(ShapeId shapeId, const AZ::Vector3& target, AZ::Vector3& position);
    [[nodiscard]] bool CastRay(ShapeId shapeId, const AZ::Vector3& start, const AZ::Vector3& translation, GeometryCastHit& hit);
    [[nodiscard]] bool ApplyWind(ShapeId shapeId, const AZ::Vector3& velocity, float drag, float lift, float maximumSpeed, bool wake);
} // namespace Box3D
