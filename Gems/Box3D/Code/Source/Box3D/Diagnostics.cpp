...
namespace Box3D
{
    StatisticsSnapshot StatisticsSnapshot::Create(const WorldStatistics& statistics, bool found)
    {
        ...
        result.m_profile = { profile.m_total.count(),
                             profile.m_pairGeneration.count(),
                             profile.m_collision.count(),
                             profile.m_solver.count(),
                             profile.m_solverSetup.count(),
                             profile.m_constraints.count(),
                             profile.m_prepareConstraints.count(),
                             profile.m_integrateVelocities.count(),
                             profile.m_warmStart.count(),
                             profile.m_solveImpulses.count(),
                             profile.m_integratePositions.count(),
                             profile.m_relaxImpulses.count(),
                             profile.m_applyRestitution.count(),
                             profile.m_storeImpulses.count(),
                             profile.m_splitIslands.count(),
                             profile.m_updateTransforms.count(),
                             profile.m_sensorHits.count(),
                             profile.m_jointEvents.count(),
                             profile.m_hitEvents.count(),
                             profile.m_refitBroadPhase.count(),
                             profile.m_continuousCollision.count(),
                             profile.m_sleepIslands.count(),
                             profile.m_sensorOverlap.count() };
