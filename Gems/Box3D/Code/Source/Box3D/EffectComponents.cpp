...
namespace Box3D
{
    ...
    void ExplosionComponent::Reflect(AZ::ReflectContext* context)
    {
        ...
        {
            serializeContext->Class<ExplosionComponent, AZ::Component>()
                ->Version(2)
    ...
    {
        if (m_effects == nullptr || m_system == nullptr)
        {
            return false;
        }
        const WorldHandle worldHandle = m_worldName.IsEmpty() ? m_system->GetDefaultWorldHandle() : m_system->FindWorld(m_worldName);
    ...
    void ExplosionComponent::UpdateConfiguration(const ExplosionConfiguration& configuration)
    {
    ...
    void WindComponent::UpdateConfiguration(const WindConfiguration& configuration)
    {
        m_configuration = configuration;
    }
    ...
    void WindComponent::OnTick([[maybe_unused]] float deltaTime, [[maybe_unused]] AZ::ScriptTimePoint time)
    {
        if (m_enabled)
        {
            ApplyWind();
