...
namespace Box3D
{
    ...
    public:
        ...
        explicit RigidBodyComponent(RigidBodyConfiguration configuration, AZ::Name worldName = {});
        ...
        [[nodiscard]] BodyHandle GetBodyHandle() const override;
        [[nodiscard]] BodyState GetState() const override;
        [[nodiscard]] AZ::Name GetName() const override;
        bool SetName(AZ::Name name) override;
        [[nodiscard]] BodyProperties GetProperties() const override;
        bool SetProperties(const BodyProperties& properties) override;
        [[nodiscard]] AZ::Aabb GetAabb() const override;
        [[nodiscard]] ClosestPoint GetClosestPoint(const AZ::Vector3& target) const override;
        [[nodiscard]] MassProperties GetMassProperties() const override;
        ...
        [[nodiscard]] AZ::Vector3 GetLinearVelocityAtWorldPoint(const AZ::Vector3& worldPoint) const override;
        bool SetKinematicTarget(const AZ::Transform& transform, float fixedTimeStep) override;
        bool ApplyLinearImpulse(const AZ::Vector3& impulse) override;
        bool ApplyLinearImpulseAtWorldPoint(const AZ::Vector3& impulse, const AZ::Vector3& worldPoint) override;
        bool ApplyAngularImpulse(const AZ::Vector3& impulse) override;
        bool ApplyForce(const AZ::Vector3& force, bool wake) override;
        bool ApplyForceAtWorldPoint(const AZ::Vector3& force, const AZ::Vector3& worldPoint, bool wake) override;
        bool ApplyTorque(const AZ::Vector3& torque, bool wake) override;
