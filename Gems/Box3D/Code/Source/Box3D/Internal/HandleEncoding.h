...
namespace Box3D::Internal
{
    ...
    static_assert(MaximumWorldCount == AZ::u32{ 1 } << WorldIndexBits);

    class HandleAccess final
    {
    public:
        template<class HandleType>
        [[nodiscard]] static constexpr HandleType Create(typename HandleType::ValueType value) noexcept
        {
            return HandleType(value);
        }

        template<class Tag>
        [[nodiscard]] static constexpr typename Handle<Tag>::ValueType GetValue(Handle<Tag> handle) noexcept
        {
    ...
    public:
        ...
        [[nodiscard]] AZ::u32 Acquire() noexcept
        {
            AZ::u32 generation = m_nextGeneration.load(AZStd::memory_order_relaxed);
            while (generation != 0)
            {
                const AZ::u32 nextGeneration = generation == AZStd::numeric_limits<AZ::u32>::max() ? 0 : generation + 1;
    ...
    template<class HandleType>
    [[nodiscard]] constexpr bool DecodeWorldMemberHandle(HandleType handle, WorldMemberHandleParts& parts) noexcept
    {
        const AZ::u64 value = HandleAccess::GetValue(handle);
        const AZ::u64 payload = value & HandlePayloadMask;
    ...
    template<class HandleType>
    [[nodiscard]] constexpr bool DecodeRegistryHandle(HandleType handle, AZ::u32& index, AZ::u32& generation) noexcept
    {
        const AZ::u64 value = HandleAccess::GetValue(handle);
        const AZ::u64 payload = value & HandlePayloadMask;
    ...
    [[nodiscard]] constexpr bool AdvanceGeneration(AZ::u32& generation) noexcept
    {
