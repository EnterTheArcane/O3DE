...
namespace Box3D::Editor
{
    namespace
    {
        ...
        void DrawTriangleMesh(
            AzFramework::DebugDisplayRequests& debugDisplay, const TriangleMeshShapeConfiguration& mesh, const AZ::Matrix3x4& transform)
        {
        ...
        {
            if (heightfield.m_columnCount < 2 || heightfield.m_rowCount < 2 ||
        ...
        void AddBoxBounds(AZ::Aabb& bounds, const AZ::Vector3& minimum, const AZ::Vector3& maximum, const AZ::Matrix3x4& transform)
        {
            for (AZ::u32 corner = 0; corner < 8; ++corner)
            {
                bounds.AddPoint(transform.TransformPoint(
                    AZ::Vector3(
                        (corner & 1) != 0 ? maximum.GetX() : minimum.GetX(),
                        (corner & 2) != 0 ? maximum.GetY() : minimum.GetY(),
                        (corner & 4) != 0 ? maximum.GetZ() : minimum.GetZ())));
            }
        }

        template<class GeometryVariant>
        void AddGeometryBounds(AZ::Aabb& bounds, const GeometryVariant& geometry, const AZ::Matrix3x4& transform)
        {
            AZStd::visit(
                [&bounds, &transform](const auto& typedGeometry)
                {
                    ...
                    else if constexpr (AZStd::is_same_v<Type, HeightfieldShapeConfiguration>)
                    {
                        const size_t sampleCount = AZStd::min(
                            typedGeometry.m_samples.size(), static_cast<size_t>(typedGeometry.m_columnCount) * typedGeometry.m_rowCount);
    ...
    AZ::Aabb CalculateShapeBounds(const ShapeGeometry& geometry, const AZ::Matrix3x4& transform)
    {
    ...
    bool IntersectEditorBounds(const AZ::Aabb& bounds, const AZ::Vector3& rayStart, const AZ::Vector3& rayDirection, float& distance)
    {
