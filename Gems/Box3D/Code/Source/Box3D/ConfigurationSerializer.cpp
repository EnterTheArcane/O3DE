...
namespace Box3D
{
    ...
    AZ::JsonSerializationResult::ResultCode JsonTaggedVariantSerializerBase::LoadAlternative(
        Variant& output, const AZ::TypeId& alternativeTypeId, const rapidjson::Value& inputValue, AZ::JsonDeserializerContext& context)
    {
        ...
        AZ::JsonDeserializerContext& context)
    {
        ...
        return context.Report(
            result,
            result.GetProcessing() != JSR::Processing::Halted ? "Loaded the tagged configuration."
                                                              : "Failed to load the tagged configuration.");
    }
        ...
        AZ::JsonSerializerContext& context)
    {
        ...
        AZStd::visit(
            [this, &outputValue, defaults, &context, &result](const auto& alternative)
            {
                ...
                const Alternative* defaultAlternative = defaults ? AZStd::get_if<Alternative>(defaults) : nullptr;
                result.Combine(ContinueStoringToJsonObjectField(
                    outputValue, ValueField, &alternative, defaultAlternative, azrtti_typeid<Alternative>(), context));
            },
            input);

        return context.Report(
            result,
            result.GetProcessing() != JSR::Processing::Halted ? "Stored the tagged configuration."
                                                              : "Failed to store the tagged configuration.");
    }
