...
namespace Box3D
{
    StaticRigidBodyComponent::StaticRigidBodyComponent(AZ::Name worldName)
    ...
    void StaticRigidBodyComponent::Reflect(AZ::ReflectContext* context)
    {
        ...
        {
            serializeContext->Class<StaticRigidBodyComponent, AZ::Component>()->Version(1)->Field(
    ...
    {
        return m_system != nullptr && m_bodyHandle.IsValid();
    }
    ...
    bool StaticRigidBodyComponent::SetProperties(const BodyProperties& properties)
    {
        BodyProperties staticProperties = properties;
        staticProperties.m_bodyType = BodyType::Static;
        return m_system != nullptr && m_system->SetBodyProperties(m_worldHandle, m_bodyHandle, staticProperties);
    }

    AZ::Aabb StaticRigidBodyComponent::GetAabb() const
    {
        return m_system != nullptr ? m_system->GetBodyAabb(m_worldHandle, m_bodyHandle) : AZ::Aabb::CreateNull();
    }

    ClosestPoint StaticRigidBodyComponent::GetClosestPoint(const AZ::Vector3& target) const
    {
        ClosestPoint result;
        if (m_system != nullptr)
        {
    ...
    bool StaticRigidBodyComponent::SetMassProperties(const MassProperties& properties)
    {
        return m_system != nullptr && m_system->SetMassProperties(m_worldHandle, m_bodyHandle, properties);
    }

    bool StaticRigidBodyComponent::RecomputeMassFromShapes()
    {
        return m_system != nullptr && m_system->RecomputeMassFromShapes(m_worldHandle, m_bodyHandle);
    }

    bool StaticRigidBodyComponent::SetTransform(const AZ::Transform& transform)
    {
        if (m_system == nullptr || m_collider == nullptr)
        {
            return false;
    ...
    bool StaticRigidBodyComponent::SetLinearVelocity(const AZ::Vector3& velocity)
    {
        return m_system != nullptr && m_system->SetLinearVelocity(m_worldHandle, m_bodyHandle, velocity);
    }

    bool StaticRigidBodyComponent::SetAngularVelocity(const AZ::Vector3& velocity)
    {
        return m_system != nullptr && m_system->SetAngularVelocity(m_worldHandle, m_bodyHandle, velocity);
    }

    AZ::Vector3 StaticRigidBodyComponent::GetLinearVelocityAtWorldPoint(const AZ::Vector3& worldPoint) const
    {
        return m_system != nullptr ? m_system->GetLinearVelocityAtWorldPoint(m_worldHandle, m_bodyHandle, worldPoint)
                                   : AZ::Vector3::CreateZero();
    }

    bool StaticRigidBodyComponent::SetKinematicTarget(const AZ::Transform&, float)
    {
        return false;
    }

    bool StaticRigidBodyComponent::ApplyLinearImpulse(const AZ::Vector3& impulse)
    {
        return m_system != nullptr && m_system->ApplyLinearImpulse(m_worldHandle, m_bodyHandle, impulse);
    }

    bool StaticRigidBodyComponent::ApplyLinearImpulseAtWorldPoint(const AZ::Vector3& impulse, const AZ::Vector3& worldPoint)
    {
        return m_system != nullptr && m_system->ApplyLinearImpulseAtWorldPoint(m_worldHandle, m_bodyHandle, impulse, worldPoint);
    }

    bool StaticRigidBodyComponent::ApplyAngularImpulse(const AZ::Vector3& impulse)
    {
    ...
    void StaticRigidBodyComponent::Activate()
    {
        ...
        AZ_Error("Box3D", m_collider != nullptr, "Box3D StaticRigidBodyComponent requires a ColliderComponent.");
