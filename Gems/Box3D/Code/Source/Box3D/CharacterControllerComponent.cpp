...
namespace Box3D
{
    CharacterControllerComponent::CharacterControllerComponent(CharacterConfiguration configuration, AZ::Name worldName)
    ...
    {
        return m_system != nullptr && m_characterHandle.IsValid();
    }
    ...
    bool CharacterControllerComponent::UpdateConfiguration(const CharacterConfiguration& configuration)
    {
        if (m_system != nullptr && m_characterHandle.IsValid() &&
            !m_system->UpdateCharacter(m_worldHandle, m_characterHandle, configuration))
        {
            return false;
        }
    ...
    void CharacterControllerComponent::OnTransformChanged([[maybe_unused]] const AZ::Transform& local, const AZ::Transform& world)
    {
        if (m_syncingTransform || m_system == nullptr || !m_characterHandle.IsValid())
        {
            return;
        }
