...
static uint32 Hash32Len13to24(const char* s, size_t len)
{
    ...
    const uint32 h = static_cast<uint32>(len);
...
static uint32 Hash32Len0to4(const char* s, size_t len)
{
    ...
    {
        const signed char v = static_cast<signed char>(s[i]);
...
static uint64 HashLen0to16(const char* s, size_t len)
{
    ...
    if (len > 0)
    {
        const uint8 a = static_cast<uint8>(s[0]);
        const uint8 b = static_cast<uint8>(s[len >> 1]);
        const uint8 c = static_cast<uint8>(s[len - 1]);
        const uint32 y = static_cast<uint32>(a) + (static_cast<uint32>(b) << 8);
        const uint32 z = static_cast<uint32>(len) + (static_cast<uint32>(c) << 2);
