...
namespace AZStd
{
     ...
     *
     * \note only allocate/deallocate are thread safe. reset, leak_before_destroy and comparison operators are not thread safe.
     *
    ...
    class static_pool_concurrent_allocator
    {
        ...
        union pool_node
        {
            aligned_storage<sizeof(Node), alignment_of_v<Node>>::type m_node;
            index_type m_index;
        };
    ...
    public:
        ...
        AZ_FORCE_INLINE static_pool_concurrent_allocator(const static_pool_concurrent_allocator& rhs, const char* name)
            : m_name(name) { reset(); }
        AZ_FORCE_INLINE static_pool_concurrent_allocator& operator=(const static_pool_concurrent_allocator& rhs) { m_name = rhs.m_name; return *this; }

        AZ_FORCE_INLINE const char* get_name() const { return m_name; }
        AZ_FORCE_INLINE void set_name(const char* name) { m_name = name; }
        static constexpr size_type max_size() { return NumNodes * sizeof(Node); }
        AZ_FORCE_INLINE size_type get_allocated_size() const { return m_numOfAllocatedNodes.load(memory_order_relaxed) * sizeof(Node); }
        ...
        AZ_FORCE_INLINE static size_type resize(
            pointer ptr,
            size_type newSize)
        {
        ...
        AZ_FORCE_INLINE static constexpr size_type data_size() { return sizeof(Node) * NumNodes; }

    private:
        typename aligned_storage<sizeof(Node)* NumNodes, alignment_of_v<Node>>::type m_data;
        const char* m_name;
