...
namespace AZStd::ranges
{
    namespace Internal
    {
        ...
        /*concept*/ constexpr bool compatible_joinable_ranges =
    ...
    template<class View, class Pattern>
        requires input_range<View> && view<View> && input_range<range_reference_t<View>>
            && forward_range<Pattern> && view<Pattern>
    ...
    public:
        join_with_view() requires (default_initializable<View> && default_initializable<Pattern>) {}
        ...
        template<class R>
            requires forward_range<R> && constructible_from<View, views::all_t<R>>
        ...
        constexpr auto begin() const
            requires input_range<const View> && is_reference_v<range_reference_t<const View>>
    ...
    // to work with the AZStd range views
        ...
        iterator() requires (default_initializable<OuterIter> && default_initializable<InnerIter>) {}

        iterator(iterator<!Const> i)
            requires (Const && convertible_to<iterator_t<View>, OuterIter>
        ...
        constexpr iterator& operator--()
            requires (ref_is_glvalue && bidirectional_range<Base>
                && Internal::bidirectional_common<InnerBase> && Internal::bidirectional_common<PatternBase>)
        {
        ...
        friend constexpr decltype(auto) iter_move(const iterator& x)
        {
            ...
            return visit<rvalue_reference>(std::ranges::iter_move, x.m_innerIter);
        }


        friend constexpr void iter_swap(const iterator& x, const iterator& y)
        {
            visit(std::ranges::iter_swap, x.m_innerIter, y.m_innerIter);
        }
