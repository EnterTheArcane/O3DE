...
def do_tests(compiler, silent):
    ...
    if verify_binding_dependencies_1(os.path.join(work_dir, "EntryDependencies.azsl"), compiler, silent):
        result += 1
    else:
        result_failed += 1

    if result and verify_binding_dependencies_2(os.path.join(work_dir, "../Semantic/StandardpbrForwardpass.azsl"), compiler, silent):
        result += 1
    else:
        result_failed += 1
