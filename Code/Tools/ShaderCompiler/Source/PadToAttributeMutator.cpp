...
namespace AZ::ShaderCompiler
{
    void PadToAttributeMutator::ProcessPadToAttribute(const AttributeInfo& attrInfo)
    {
        ...
        if (attrInfo.m_argList.size() != 1)
        {
            ...
            throw AzslcIrException{IR_INVALID_PAD_TO_ARGUMENTS, errorMsg, attrInfo.m_sourceLocation};
        }
        ...
        if (curScopeKindInfo.IsKindOneOf(Kind::Struct, Kind::Class, Kind::ShaderResourceGroup))
        {
            ...
            if (varUid.IsEmpty())
            {
                ...
                throw AzslcIrException{IR_INVALID_PAD_TO_LOCATION, errorMsg, attrInfo.m_sourceLocation};
            }
        ...
        for (size_t idx = 0; idx < memberFields.size(); idx++)
        {
            ...
            if (padToBoundary < nextMemberOffset)
            {
                ...
                if (!IsPowerOfTwo(padToBoundary))
                {
                    ...
                    throw AzslcIrException{IR_PAD_TO_CASE_REQUIRES_POWER_OF_TWO, errorMsg, varInfoPtr->GetOriginalSourceLocation()};
                }
