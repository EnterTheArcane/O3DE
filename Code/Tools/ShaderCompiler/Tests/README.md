...
These Python tests run directly against an engine-built `azslc`; they are not
registered with CTest.

The runner requires Python 3 and PyYAML. O3DE's Python environment normally provides
both. If a different environment is used, install PyYAML there first.
...
On Windows, use the corresponding `azslc.exe` path. A successful complete run may
include tests reported as `TODO`; it must report no `FAIL` results.
...
Paths are interpreted relative to this `Tests` directory. Omit `--silent` when
compiler stdout is useful while diagnosing a failure.
