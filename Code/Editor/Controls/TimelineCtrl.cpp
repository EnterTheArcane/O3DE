...
void TimelineWidget::mouseMoveEvent(QMouseEvent* event)
{
    ...
    {
        float start = AZStd::min(ClientToTime(m_lastPoint.x()), ClientToTime(event->position().x()));
        float end = AZStd::max(ClientToTime(m_lastPoint.x()), ClientToTime(event->position().x()));
